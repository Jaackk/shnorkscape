...
The native1475 saved-layout editor is mounted at the cache-stable host1477:692. Its onLoad8748 calls2462 and therefore owns entry into global Edit Mode; the server must not manually write varc3477. Save & Exit1475:43 is routed through native wrapper8743. Revision950 script8754 has two integer arguments but never reads its first argument: it reads the selected save target from client varc139445, so the wrapper's required callback argument is a neutral ignored placeholder rather than a server-selected layout. Exact950 script8746 can re-enter component1475:20 for its client-owned dirty-layout confirmation; live testing showed that retiring the server-owned mount after this branch strands its dimming/input shield. Therefore an explicit server-handled X uses exact950 wrapper8745(0), whose argument is the proven close-without-saving flag (also corroborated by Project Undercut949.1), while Save & Exit remains8743. Both paths reach8754, whose teardown includes2464,8751(0),6593(0) and8164. The server sends IF_CLOSESUB after the ordered wrapper call and unregisters only its own1477:692 mount. It never manually clears3477. Escape is deliberately not intercepted because no revision950 mapping from the server-visible root action to the discard lifecycle has been established.
