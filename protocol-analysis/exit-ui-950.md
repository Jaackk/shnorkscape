...
Native950ExitUiAcceptance passed35 checks, including a real native session's same-tick final backpack mutation saved to a newly created temporary profile, followed by player removal and slot release. The passed checks include menu response assertions for22/43/66; root's final shared build reruns the suite after integrating202. No real user's profile was opened by the probe.

The1475 saved-layout editor was inspected but not exposed: its current cache has59 components with several new custom widget types, native slot2000, onLoad8748 and varc3477. Its Save/Load transport and editor lifecycle need a separate complete implementation. Simply changing3477 would leave an incomplete global layout mode.
