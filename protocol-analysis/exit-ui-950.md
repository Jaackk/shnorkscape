...
Native950ExitUiAcceptance passed57 checks, including the editor mount/mode lifecycle and a real native session's same-tick final backpack mutation saved to a newly created temporary profile, followed by player removal and slot release. The remaining local menu response assertions cover43/66; component22 now owns the editor transition. No real user's profile was opened by the probe.

The first saved-layout milestone exposes the native editor without changing persistence. Options component22 closes1433, mounts interface1475 at the cache-stable root host1477:692 and sets edit-mode varc3477 to1. This matches the earlier project path while remaining independently pinned to the paired950 host, interface, onLoad8748, initializer19711/19712, enum7716 key2000 and struct21136. Native CLOSE_MODAL reconciles ownership and resets3477; an active server-side modal replacement also closes the owned mount. The client still owns the editor's59-component Save/Load workflow. Visual opening and all subsequent Save/Load behavior require live verification before any persistence claim.
