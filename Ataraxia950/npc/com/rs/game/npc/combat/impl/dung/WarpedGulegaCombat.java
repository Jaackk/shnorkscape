	...
	public int attack(final NPC npc, final Entity target) {
					...
					for (Entity t : boss.getPossibleTargets(true, true)) {
						tileLoop: for (WorldTile tile : attackTiles) {
							if (t.getX() == tile.getX() && t.getY() == tile.getY()) {
								delayHit(npc, 0, t, getMeleeHit(npc, getRandomMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitionConstants.MELEE) * 0.75), NPCCombatDefinitionConstants.MELEE, t)));
								break tileLoop;
							}
						}
					}
		...
		case 1://reg range aeo
			...
			for (Entity t : npc.getPossibleTargets(true, true)) {
				...
				delayHit(npc, 2, t, getRangeHit(npc, getRandomMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitionConstants.MELEE) * 0.75), NPCCombatDefinitionConstants.RANGE, t)));
			}
			break;
		case 2://reg magic aeo
			...
			for (Entity t : npc.getPossibleTargets(true, true)) {
				...
				delayHit(npc, 2, t, getMagicHit(npc, getRandomMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitionConstants.MELEE) * 0.75), NPCCombatDefinitionConstants.MAGE, t)));
			}
			break;
		case 0:
			npc.setNextAnimation(new Animation(15004));
