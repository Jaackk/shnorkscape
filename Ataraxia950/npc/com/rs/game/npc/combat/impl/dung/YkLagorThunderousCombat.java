...
public class YkLagorThunderousCombat extends CombatScript {
			...
			if (boss.getNextAttack() == 0) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						cycles++;
						if (cycles == 3) {
							boss.setNextAnimation(new Animation(19588));
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						if (cycles == 2) {
							boss.setNextAnimation(new Animation(19587));
	...
	public static void sendMagicalAttack(YkLagorThunderous npc, boolean specialAttack) {
		npc.setNextAnimation(new Animation(19590));
		...
		if (npc.getPossibleTargets().size() > 0) {
			for (Player player : npc.getManager().getParty().getTeam()) {
				...
				delayHit(npc, 1, player, getMagicHit(npc, getMaxHit(npc, NPCCombatDefinitionConstants.MAGE, player)));
