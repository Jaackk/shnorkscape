...
public class IcyBonesCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (Utils.random(10) == 0) {
			...
			if (mage)
				delayHit(npc, 2, target, getMagicHit(npc, getMaxHit(npc, NPCCombatDefinitionConstants.MAGE, target)));
			else
		...
		for (Player player : manager.getParty().getTeam()) {
			if (Utils.isOnRange(player.getX(), player.getY(), player.getSize(), npc.getX(), npc.getY(), npc.getSize(), 0)) {
				int damage = getMaxHit(npc, NPCCombatDefinitionConstants.MELEE, player);
		...
		if (onRange) {
			npc.setNextAnimation(new Animation(10816, 20));
