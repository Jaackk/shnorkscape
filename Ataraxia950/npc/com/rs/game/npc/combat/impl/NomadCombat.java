...
public class NomadCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (nomad.isMeleeMode()) {
			...
			delayHit(npc, 0, target, getRegularHit(npc, getRandomMaxHit(npc, 322, NPCCombatDefinitionConstants.MELEE, target)));
			return 2;
		} else {
			if (target instanceof Player && nomad.useSpecialSpecialMove()) {
				...
				int damage = getRandomMaxHit(npc, 322, NPCCombatDefinitionConstants.MAGE, target);
