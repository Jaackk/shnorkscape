...
public class DreadnautCombat extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		if (Utils.random(5) == 0) {
			...
			int damage = getRandomMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitionConstants.MELEE, target);
			if (damage > 0) {
			...
			for (Entity t : boss.getPossibleTargets()) {
				if (!t.withinDistance(target, 2))
					continue;
				int damage = getRandomMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitionConstants.MELEE, t);
