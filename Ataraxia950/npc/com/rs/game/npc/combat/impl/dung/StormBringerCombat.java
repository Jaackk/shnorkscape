...
public class StormBringerCombat extends CombatScript {
	...
	public int attack(NPC npc, final Entity target) {
		...
		int damage = 0;
		if (usingSpecial) {
			damage = getRandomMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitionConstants.MAGE) * (1.05 * tier)), NPCCombatDefinitionConstants.MAGE, target);
			if (Utils.random(11 - tier) == 0)
				target.setFreezeDelay(8); // Five seconds cannot move.
		} else
			damage = getMaxHit(npc, NPCCombatDefinitionConstants.MAGE, target);
