	...
	public int attack(final NPC npc, Entity target) {
		...
		if (special) {
			...
			if (meleeAttack) {
				...
				delayHit(npc, 0, target, getMeleeHit(npc, getRandomMaxHit(npc, 200, NPCCombatDefinitionConstants.MELEE, target)));
			} else {
				npc.setNextAnimation(new Animation(16990));
				World.sendProjectile(npc, target, 2546, 16, 16, 41, 30, 0, 0);
				delayHit(npc, 1, target, getMagicHit(npc, getRandomMaxHit(npc, 200, NPCCombatDefinitionConstants.MAGE, target)));
			}
