		...
		case 2://Multi magic
			...
			for (Entity t : npc.getPossibleTargets()) {
				if ((attack == 0 || attack == 1) && t != target)
					continue;
				World.sendProjectile(new NewProjectile(new WorldTile(npc.getCoordFaceX(npc.getSize()), npc.getCoordFaceY(npc.getSize()), 0), target, attack == 0 ? 2533 : 2535, 65, 50, 35, 5, 54, 0));
				if (attack == 0)
					delayHit(npc, 1, t, getRangeHit(npc, getMaxHit(npc, NPCCombatDefinitionConstants.RANGE, t)));
				else
					delayHit(npc, 1, t, getMagicHit(npc, getMaxHit(npc, NPCCombatDefinitionConstants.MAGE, t)));
			}
			break;
		case 3://Bind attacks
		case 4:
			...
			public void run() {
				...
				if (cycles == 2) {
					...
					targetL: for (Entity target : boss.getPossibleTargets()) {
						...
						int hit = (int) (boss.getMaxHit() * .1 + getRandomMaxHit(boss, (int) (boss.getMaxHit() * .90), NPCCombatDefinitionConstants.RANGE, player));
						player.applyHit(new Hit(boss, hit * 2, HitLook.REGULAR_DAMAGE));
					}
