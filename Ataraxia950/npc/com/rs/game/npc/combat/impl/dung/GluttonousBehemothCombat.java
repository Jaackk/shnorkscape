...
public class GluttonousBehemothCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (lessThanHalf && npc.getTemporaryAttributtes().get(Key.GLUTTONOUS_HEALING) == null) {
		...
		if (attackStyle == 2) {
			if (!Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), target.getX(), target.getY(), target.getSize(), 0))
				...
				delayHit(npc, 0, target, getMeleeHit(npc, getMaxHit(npc, NPCCombatDefinitionConstants.MELEE, target)));
				return npc.getAttackSpeed();
			}
		}
		if (attackStyle == 0) {
			npc.setNextAnimation(new Animation(13719));
			World.sendProjectile(new NewProjectile(/*npc*/new WorldTile(npc.getCoordFaceX(npc.getSize()), npc.getCoordFaceY(npc.getSize()), 0), target, 2612, 5, 16, 35, 16, 41, 0));
			int damage = getMaxHit(npc, NPCCombatDefinitionConstants.MAGE, target);
			...
			World.sendProjectile(new NewProjectile(new WorldTile(npc.getCoordFaceX(npc.getSize()), npc.getCoordFaceY(npc.getSize()), 0), target, 2610, 10, 16, 35, 16, 41, 0));
			delayHit(npc, 2, target, getRangeHit(npc, getMaxHit(npc, NPCCombatDefinitionConstants.RANGE, target)));
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
