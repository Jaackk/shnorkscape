...
public class LakkTheRiftSplitterCombat extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		for (Player player : manager.getParty().getTeam()) {
			if (Utils.colides(player.getX(), player.getY(), player.getSize(), npc.getX(), npc.getY(), npc.getSize())) {
				...
				delayHit(npc, 0, player, getRegularHit(npc, getRandomMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitionConstants.MELEE) * .85), NPCCombatDefinitionConstants.MELEE, player)));
				delayHit(npc, 0, player, getRegularHit(npc, getRandomMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitionConstants.MELEE) * .60), NPCCombatDefinitionConstants.MELEE, player)));
			}
		}
		if (smash) {
			...
			case 2:
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						boss.setNextAnimation(new Animation(16990));
	...
	private void regularMagicAttack(Entity target, NPC npc) {
		...
		if (target instanceof Player) {
			...
			int damage = getMaxHit(npc, NPCCombatDefinitionConstants.MAGE, player);
