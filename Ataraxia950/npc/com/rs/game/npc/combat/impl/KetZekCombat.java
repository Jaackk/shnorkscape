...
public class KetZekCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		switch (attackStyle) {
		case 0:
			hit = getRandomMaxHit(npc, defs.getMaxHit(), NPCCombatDefinitionConstants.MELEE, target);
	...
	private void commenceMagicAttack(final NPC npc, final Entity target, int hit) {
		hit = getRandomMaxHit(npc, npc.getCombatDefinitions().getMaxHit() - 50, NPCCombatDefinitionConstants.MAGE, target);
