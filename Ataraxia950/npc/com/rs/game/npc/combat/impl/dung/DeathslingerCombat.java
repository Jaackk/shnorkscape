...
public class DeathslingerCombat extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		if (usingSpecial) {
			...
			damage = getRandomMaxHit(npc, (int) (npc.getMaxHit(NPCCombatDefinitionConstants.RANGE) * (1.05 * tier)), NPCCombatDefinitionConstants.RANGE, target);
			if (Utils.random(11 - tier) == 0)
				target.getPoison().makePoisoned(100);
		} else
			damage = getMaxHit(npc, NPCCombatDefinitionConstants.RANGE, target);
