...
public class NightGazerKhighorahkCombat extends CombatScript {
	...
	public void sendRangeAoe(final NightGazerKhighorahk gazer) {
		if (gazer.isDead())
			return;
		gazer.setNextAnimationForce(new Animation(13425));
		for (Entity target : gazer.getPossibleTargets()) {
			World.sendProjectile(gazer, target, 2385, 60, 16, 41, 30, 0, 0);
			delayHit(gazer, 1, target, getRangeHit(gazer, getRandomMaxHit(gazer, (int) (gazer.getMaxHit() * 0.6), NPCCombatDefinitionConstants.RANGE, target)));
		}
		...
		if (!gazer.isSecondStage()) {
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					if (gazer.isDead())
						return;
					gazer.setNextAnimationForce(new Animation(13422));
				}
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (!gazer.isUsedSpecial()) {
			...
			if (success) {
				WorldTasksManager.schedule(new WorldTask() {
					...
					private int ticks;
					private final List<WorldTile> tiles = new LinkedList<WorldTile>();

					@Override
					public void run() {
						ticks++;
						if (ticks == 1) {
							npc.setNextAnimationForce(new Animation(gazer.isSecondStage() ? 13426 : 13428));
						} else if (ticks == 3) {
							for (Entity t : targets) {
								if (Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), t.getX(), t.getY(), t.getSize(), 1)) {
		...
		if (Utils.random(10) == 0) { //range aoe
			if (!gazer.isSecondStage()) {
				npc.setNextAnimationForce(new Animation(13423));
				WorldTasksManager.schedule(new WorldTask() {
			...
			if (Utils.random(3) == 0) { //range single target
				npc.setNextAnimationForce(new Animation(gazer.isSecondStage() ? 13433 : 13434));
				World.sendProjectile(npc, target, 2385, gazer.isSecondStage() ? 60 : 40, 16, 41, 90, 0, 0);
				delayHit(npc, 3, target, getRangeHit(npc, getMaxHit(npc, NPCCombatDefinitionConstants.RANGE, target)));
				return npc.getAttackSpeed() + 1;
			} else { //magic
				npc.setNextAnimationForce(new Animation(gazer.isSecondStage() ? 13430 : 13431));
				...
				delayHit(npc, 1, target, getMagicHit(npc, getMaxHit(npc, NPCCombatDefinitionConstants.MAGE, target)));
