...
public class HopeDevourerCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					for (Entity t : npc.getPossibleTargets()) {
						...
						int damage = (int) Utils.random(npc.getMaxHit(NPCCombatDefinitionConstants.MAGE) * .85, npc.getMaxHit(NPCCombatDefinitionConstants.MAGE));
		...
		if (Utils.random(5) == 0) {
			...
			final int damage = (int) Utils.random(npc.getMaxHit(NPCCombatDefinitionConstants.MELEE) * .85, npc.getMaxHit(NPCCombatDefinitionConstants.MELEE));
			...
			int damage = (int) Utils.random(npc.getMaxHit(NPCCombatDefinitionConstants.MELEE) * .75, npc.getMaxHit(NPCCombatDefinitionConstants.MELEE));
			if (target instanceof Player) {
				Player player = (Player) target;
