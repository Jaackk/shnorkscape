...
public class KalGerWarmongerCombat extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		if (boss.isUsingMelee()) {
			...
			if (smash) {
				boss.setNextAnimation(new Animation(19561));
		...
		if (boss.getType() == 1) {//NO WEAPONS HUR
			npc.setNextAnimation(new Animation(16990));//14392
			delayHit(npc, 0, target, getMeleeHit(npc, getRandomMaxHit(npc, boss.getMaxHit(), NPCCombatDefinitionConstants.MELEE, target)));
			...
			for (Entity t : boss.getPossibleTargets()) {
				...
				delayHit(npc, 0, t, getMagicHit(npc, getRandomMaxHit(npc, boss.getMaxHit(), NPCCombatDefinitionConstants.MAGE, t)));
			}
		} else if (boss.getType() == 4) {//2H
			npc.setNextAnimation(new Animation(19562));
			delayHit(npc, 0, target, getMeleeHit(npc, getRandomMaxHit(npc, boss.getMaxHit(), NPCCombatDefinitionConstants.MELEE, target)));
		} else if (boss.getType() == 5) {//BOW
			npc.setNextAnimation(new Animation(19568));
			npc.setNextGraphics(new Graphics(2885));
			for (Entity t : boss.getPossibleTargets()) {
				World.sendProjectile(boss, t, 2886, 75, 30, 50, 55, 2, 0);
				delayHit(npc, 2, t, getRangeHit(npc, getRandomMaxHit(npc, boss.getMaxHit(), NPCCombatDefinitionConstants.RANGE, t)));
			}
		} else if (boss.getType() == 6) {//MAUL
			npc.setNextAnimation(new Animation(19556));
			delayHit(npc, 0, target, getMeleeHit(npc, getRandomMaxHit(npc, boss.getMaxHit(), NPCCombatDefinitionConstants.MELEE, target)));
