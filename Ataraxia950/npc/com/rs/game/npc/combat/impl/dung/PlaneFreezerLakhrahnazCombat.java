...
public class PlaneFreezerLakhrahnazCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (Utils.random(3) == 0) {
			...
			case 0:
				...
				for (Entity t : npc.getPossibleTargets()) {
					...
					delayHit(npc, 1, t, getMagicHit(npc, getRandomMaxHit(npc, 100, NPCCombatDefinitionConstants.MAGE, target)));
				}
				break;
			case 1:
				...
				delayHit(npc, 0, target, getMeleeHit(npc, getRandomMaxHit(npc, 100, NPCCombatDefinitionConstants.MELEE, target)));
				break;
			}
		...
		delayHit(npc, 1, target, getRangeHit(npc, getRandomMaxHit(npc, 100, NPCCombatDefinitionConstants.RANGE, target)));
		return npc.getAttackSpeed();
	}
}
