		...
		if (atDistance || Utils.random(3) == 0) {
			...
			if (rangeAttack) {
				if (manager.getParty().getTeam().size() > 1 || Utils.random(3) == 0) {
							...
							if (cycles == 1) {
								...
								for (WorldTile tile : knifeTargets) {
									...
									entityLoop: for (Entity t : boss.getPossibleTargets()) {
										...
										delayHit(boss, delay, t, getRangeHit(boss, getRandomMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitionConstants.RANGE, t)));
									}
								}
					...
					delayHit(boss, 1, target, getRangeHit(boss, getRandomMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitionConstants.RANGE, target)));
				}
			} else {
				if (Utils.random(7) == 0) {
				...
				int damage = getRandomMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitionConstants.MAGE, target);
			...
			delayHit(boss, 0, target, getMeleeHit(boss, getRandomMaxHit(boss, boss.getMaxHit(), NPCCombatDefinitionConstants.MELEE, target)));
			return 4;
		}
	}
}
