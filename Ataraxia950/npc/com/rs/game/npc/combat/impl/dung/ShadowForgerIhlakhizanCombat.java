...
public class ShadowForgerIhlakhizanCombat extends CombatScript {
	...
	public int attack(final NPC npc, final Entity target) {
		...
		if (Utils.random(4) == 0) {
			if (Utils.random(3) == 0) {
						...
						public void run() {
							...
							for (Player player : forger.getManager().getParty().getTeam()) {
								if (player.isDead() || player.getX() != tile.getX() || player.getY() != tile.getY())
									continue;
								player.applyHit(new Hit(npc, Utils.random(npc.getMaxHit(NPCCombatDefinitionConstants.RANGE)) + 1, HitLook.RANGE_DAMAGE));
							}
						}
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					@Override
					public void run() {
						npc.setNextAnimationForce(new Animation(13016));
						for (Entity t : npc.getPossibleTargets()) {
								...
								WorldTasksManager.schedule(new WorldTask() {
									...
									public void run() {
										...
										if (ticks == 1) {
											...
											target.setNextAnimationForce(new Animation(10070));
		...
		case 0:
			...
			delayHit(npc, 3, target, getRegularHit(npc, getMaxHit(npc, NPCCombatDefinitionConstants.MAGE, target)));
			break;
		case 1:
			...
			for (Entity t : npc.getPossibleTargets()) {
				if (!Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), t.getX(), t.getY(), t.getSize(), 0))
					continue;
				delayHit(npc, 0, t, getMeleeHit(npc, getMaxHit(npc, NPCCombatDefinitionConstants.MELEE, t)));
			}
			break;
		}
