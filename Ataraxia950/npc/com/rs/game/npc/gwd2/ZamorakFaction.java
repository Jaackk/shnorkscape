...
public class ZamorakFaction extends FactionNPC {

    private static final long serialVersionUID = -8756034130489770019L;

    public ZamorakFaction(final int id, final WorldTile tile, final int mapAreaNameHash,
        ...
        for (final int regionId : getMapRegionsIds()) {
            ...
            if (playerIndexes != null) {
                for (final int playerIndex : playerIndexes) {
                    ...
                    if (player == null || player.isDead() || player.isUnderCombat() || player.getFindTargetDelay() > Utils.currentTimeMillis()
                            || player.hasFinished() || !player.isRunning() || player.getAppearence().isHidden() || player.getHeart().getActiveInsignia() == HeartOfGielinor.ZAMORAK
                            || !Utils.isOnRange(getX(), getY(), size, player.getX(), player.getY(), player.getSize(), getForceTargetDistance() > 0 ? getForceTargetDistance() : 1)
            ...
            if (npcsIndexes != null) {
                for (final int npcIndex : npcsIndexes) {
                    ...
                    if (npc == null || npc == this || npc instanceof Familiar || npc.getId() == 16980 || npc.isDead() || npc.hasFinished() || npc instanceof ZamorakFaction
                            || !Utils.isOnRange(getX(), getY(), size, npc.getX(), npc.getY(), npc.getSize(), getForceTargetDistance() > 0 ? getForceTargetDistance() : 1)
                            || !npc.getDefinitions().hasAttackOption() || !clipedProjectile(npc, false)) {
                        continue;
                    }
    ...
    public void sendDeath(final Entity source) {
        ...
        if (killer != null) {
            if (killer.getHeart().getBountyTargetFaction() == HeartOfGielinor.ZAMORAK) {
            ...
            public void run() {
                if (loop == 0) {
                    ...
                    if (source instanceof Player) {
                        ...
                        player.getHeart().incrementKillcount(HeartOfGielinor.ZAMORAK);
                    }
                    drop();
                    reset();
