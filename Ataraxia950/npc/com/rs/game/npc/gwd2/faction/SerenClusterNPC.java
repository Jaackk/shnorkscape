...
public class SerenClusterNPC extends SerenFaction {

	private static final long serialVersionUID = -3001494180936458041L;
	private final FactionCluster cluster;
	
	
	public SerenClusterNPC(int id, WorldTile tile, int mapAreaNameHash, boolean canBeAttackFromOutOfArea, boolean spawned, final FactionCluster cluster) {
	...
	public void processNPC() {
		if (isDead() || isLocked())
			return;
		if (getState() == DEFAULT || getCombat().process() || getAttackedBy() != null && getAttackedByDelay() > Utils.currentTimeMillis()) {
			super.processNPC();
			return;
		}
	...
	public void setRespawnTask() {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				try {
					if (cluster.getNode() != null && cluster.getNode().getType() == HeartOfGielinor.SEREN)
						spawn();
				} catch (Throwable e) {
