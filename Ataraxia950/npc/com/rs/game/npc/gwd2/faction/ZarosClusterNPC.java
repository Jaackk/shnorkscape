...
public class ZarosClusterNPC extends ZarosFaction {

	private static final long serialVersionUID = -2142436293202783723L;
	private final FactionCluster cluster;
	
	public ZarosClusterNPC(int id, WorldTile tile, int mapAreaNameHash, boolean canBeAttackFromOutOfArea, boolean spawned, final FactionCluster cluster) {
	...
	public void setRespawnTask() {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				try {
					if (cluster.getNode() != null && cluster.getNode().getType() == HeartOfGielinor.ZAROS)
						spawn();
				} catch (Throwable e) {
