...
public class SliskeFaction extends FactionNPC {

    private static final long serialVersionUID = 4335736123256761663L;

    public SliskeFaction(final int id, final WorldTile tile, final int mapAreaNameHash,
        ...
        for (final int regionId : getMapRegionsIds()) {
            ...
            if (playerIndexes != null) {
                for (final int playerIndex : playerIndexes) {
                    ...
                    if (player == null || player.isDead() || player.isUnderCombat() || player.getFindTargetDelay() > Utils.currentTimeMillis()
                            || player.hasFinished() || !player.isRunning() || player.getAppearence().isHidden() || player.getHeart().getActiveInsignia() == HeartOfGielinor.SLISKE
            ...
            if (npcsIndexes != null) {
                for (final int npcIndex : npcsIndexes) {
                    ...
                    if (npc == null || npc == this || npc instanceof Familiar || npc.getId() == 16980 || npc.isDead() || npc.hasFinished() || npc instanceof SliskeFaction
    ...
    public void sendDeath(final Entity source) {
        ...
        if (killer != null) {
            if (killer.getHeart().getBountyTargetFaction() == HeartOfGielinor.SLISKE) {
            ...
            public void run() {
                if (loop == 0) {
                    ...
                    if (source instanceof Player) {
                        ...
                        player.getHeart().incrementKillcount(HeartOfGielinor.SLISKE);
                    }
                    drop();
                    reset();
