...
public class ZarosFaction extends FactionNPC {

    private static final long serialVersionUID = -427599020299125906L;

    public ZarosFaction(final int id, final WorldTile tile, final int mapAreaNameHash,
        ...
        for (final int regionId : getMapRegionsIds()) {
            ...
            if (playerIndexes != null) {
                for (final int playerIndex : playerIndexes) {
                    ...
                    if (player == null || player.isDead() || player.isUnderCombat() || player.getFindTargetDelay() > Utils.currentTimeMillis()
                            || player.hasFinished() || !player.isRunning() || player.getAppearence().isHidden() || player.getHeart().getActiveInsignia() == HeartOfGielinor.ZAROS
            ...
            if (npcsIndexes != null) {
                for (final int npcIndex : npcsIndexes) {
                    ...
                    if (npc == null || npc == this || npc instanceof Familiar || npc.isDead() || npc.getId() == 16980 || npc.hasFinished() || npc instanceof ZarosFaction
    ...
    public void sendDeath(final Entity source) {
        ...
        if (killer != null) {
            if (killer.getHeart().getBountyTargetFaction() == HeartOfGielinor.ZAROS) {
            ...
            public void run() {
                if (loop == 0) {
                    ...
                    if (source instanceof Player) {
                        ...
                        player.getHeart().incrementKillcount(HeartOfGielinor.ZAROS);
                    }
                    drop();
                    reset();
