...
public class FactionCombatHandler extends NPCCombat {

	private static final WorldTile CENTER = new WorldTile(3199, 6944, 1);
	
	public FactionCombatHandler(NPC npc) {
		super(npc);
	}
	...
	public int combatAttack() {
		...
		if (distanceX > size + maxDistance || distanceX < -size - maxDistance || distanceY > size + maxDistance || distanceY < -size - maxDistance)
			return 0;
	...
	public boolean checkAll() {
		...
		if (target == null || npc.isDead() || npc.hasFinished() || npc.isForceWalking() || target.isDead() || target.hasFinished() || npc.getPlane() != target.getPlane() || npc.isCantInteract() || npc.isCannotMove())
			return false;
		if (npc.getFreezeDelay() >= Utils.currentTimeMillis())
			return true;
		int distanceX = target.getX() - npc.getX();
		...
		if (target.withinDistance(CENTER, 10)) {
			npc.forceWalkRespawnTile();
			return false;
		}
		...
		if (!npc.isCantFollowUnderCombat()) {
			...
			if ((!npc.clipedProjectile(target, maxDistance == 0)) || distanceX > size + maxDistance || distanceX < -1 - maxDistance || distanceY > size + maxDistance || distanceY < -1 - maxDistance) {
				int steps = RouteFinder.findRoute(RouteFinder.WALK_ROUTEFINDER, npc.getX(), npc.getY(), npc.getPlane(), npc.getSize(), new FixedTileStrategy(target.getX(), target.getY()), true);
