...
public final class DivineSkinweaver extends DungeonBoss {
	...
	public DivineSkinweaver(final int id, final WorldTile tile, final DungeonManager manager, final RoomReference reference) {
		super(id, tile, manager, reference);
		holeClosed = new boolean[5];
		skeletons = Collections.synchronizedList(new ArrayList<DungeonSkeletonBoss>());
	}

	public void removeSkeleton(final DungeonSkeletonBoss skeleton) {
		...
		if (!requestedClose && count < holeClosed.length) {
			...
			if (killedCount == 3) {
				...
				for (final Player p2 : getManager().getParty().getTeam()) {
	...
	private int[] getOpenHole() {
		final List<int[]> holes = new ArrayList<int[]>();
		for (final int[] hole : HOLES) {//49289
			final WorldObject object = getManager().getObjectWithType(getReference(), 49289, 0, hole[0], hole[1]);
	...
	public void processNPC() {
		...
		final int distance = (4 - Utils.getDistance(this, healTarget));
		if (distance == 4 || distance < 0) {
			return;
		}
		final int maxHeal = (int) (healTarget.getMaxHitpoints() * 0.25);
	...
	public void talkTo(final Player player) {
	...
	public void blockHole(final Player player, final WorldObject object) {
		...
		final WorldObject closedHole = new WorldObject(object);
