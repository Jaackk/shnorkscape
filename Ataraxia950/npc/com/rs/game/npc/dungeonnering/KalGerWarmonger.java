...
public class KalGerWarmonger extends DungeonBoss {
	...
	public boolean isUsingMelee() {
		return getAttackStyle() == NPCCombatDefinitionConstants.MELEE;
	}
	...
	private void processNextType() {
		...
		if (typeTicks == 1) {
			setNextAnimation(new Animation(19558));
			...
			setNextAnimation(new Animation(19581));
		} else if (typeTicks == 13) {
			setNextAnimation(new Animation(19582 + (type - 1)));
	...
	private void submitPullAttack() {
		...
		setNextAnimation(new Animation(19566));
		final NPC boss = this;
		WorldTasksManager.schedule(new WorldTask() {
				...
				if (ticks == 1) {
					...
					for (Entity t : possibleTargets) {
						...
						if (ticks == 8) {
							...
							setNextAnimation(new Animation(19559));
						}
	...
	private void stealPlayerEffects() {
		...
		for (Player player : getManager().getParty().getTeam()) {
			...
			boolean usingTurmoil = player.getPrayer().usingPrayer(1, 32);
