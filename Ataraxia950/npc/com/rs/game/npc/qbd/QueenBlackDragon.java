...
public final class QueenBlackDragon extends NPC {
    ...
    private static final int[][] REWARDS = { { 1149, 1, 1, 57 }, // Dragon helm
            ...
            { 11286, 1, 1, 4 } // Draconic visage current
    };
    ...
    public void setPhase(int phase) {
        ...
        case 5:
            ...
            attacker.sendMessage("<col=33FFFF>The enchantment is restored! " + "Queen Black Dragon falls back into her cursed slumber.</col>");
            handlePetDrop(attacker, getName());
            if (attacker.isGroupIronman()) {
               attacker.gimTracker.incrementBpGained(3);
            }
            attacker.getActivityTimersManager().finishBossTimer(QueenBlackDragon.this);
            break;
        }
    }
    ...
    public void handleIngoingHit(Hit hit) {
        if (hit != null && hit.getDamage() > 1250) {
    ...
    public void openRewardChest(boolean replace) {
        ...
        attacker.getPackets().sendInterSetItemsOptionsScript(1284, 7, 99, 8, 3, "Take", "Bank", "Discard", "Examine");
        attacker.getPackets().sendUnlockIComponentOptionSlots(1284, 7, 0, 10, 0, 1, 2, 3);
        attacker.getPackets().sendItems(99, rewards);
        if (replace) {
    ...
    private void prepareRewards() {
        rewards.add(new Item(537, Utils.random(5, 15)));
        ...
        for (Item i : rewards.getItems()) {
             if (i != null)
            attacker.getDropCollectionHandler().handleBossKills(i, DropCollectionConstants.QBD_ID);
        }
    }
    ...
    public void processNPC() {
        ...
        if (ticks == -20) {
            attacker.killStats[7] += 1;
            attacker.sendMessage("You've killed a total of " + Colors.RED + attacker.increaseKillStatistics("queen black dragon", true) + "" + "</col> x " + Colors.RED + "queen black dragons</col>.", true);
            ContractHandler.updateContract(attacker, this);
            HybridTokenDistributor.rollForToken(attacker, HybridTokenDistributor.Activity.QUEEN_BLACK_DRAGON);
            ...
            World.removeObject(new WorldObject(70778, 10, 0, base.transform(33, 31, 0)));
        ...
        if (spawningWorms) {
            ...
            List<QueenAttack> attackList = new ArrayList<>(Arrays.asList(attacks));
            Collections.shuffle(attackList);
            for (QueenAttack attack : attackList) {
                if (attack.canAttack(this, attacker)) {
    ...
    public void setHitpoints(int hitpoints) {
        ...
        if (lastHitpoints != hitpoints) {
            attacker.getPackets().sendGlobalConfig(1923, (getMaxHitpoints() - hitpoints) * 10);
            attacker.getPackets().sendIComponentText(1285, 32, ""+ Utils.formatNumber(hitpoints));
