...
public final class TorturedSoul extends NPC {
	...
	public void specialAttack(WorldTile teleport) {
        if (dragon == null || victim ==null || dragon.hasFinished() || victim.hasFinished() || dragon.isDead() || victim.isDead())
            return;
			...
			public void run() {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						if (target != null) {
							stop();
			                if(!(dragon.isDead() || dragon.hasFinished())) 
							target.applyHit(new Hit(dragon, Utils.random(100, 130), HitLook.REGULAR_DAMAGE));
							return;
						}
						if (x > victim.getX()) {
							x--;
