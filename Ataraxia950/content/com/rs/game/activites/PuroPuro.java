...
public class PuroPuro extends Controller {
	...
	private static final Item[][] REQUIRED = { { new Item(11238, 3), new Item(11240, 2), new Item(11242, 1) },
			...
			{ new Item(11246, 3), new Item(11248, 2), new Item(11250, 1) }, { null } };
	...
	public static void pushThrough(final Player player, WorldObject object) {
		...
		player.sendMessage(Utils.getRandom(2) == 0
		...
		for (int i = 0; i < 5; i++) {
			for (int index = 0; index < (FlyingEntities.values().length-4); index++) {
				...
				World.spawnNPC(FlyingEntities.values()[index].getNpcId(),
						new WorldTile(Utils.random(2558 + 3, 2626 - 3), Utils.random(4285 + 3, 4354 - 3), 0), -1,
						false);
			}
		}
	}
	...
	@Override
	public void start() {
		player.setNextWorldTile(new WorldTile(2591, 4318, 0));
	...
	public boolean processObjectClick1(WorldObject object) {
		...
		case 25014:
			player.getControlerManager().forceStop();
			Magic.sendTeleportSpell(player, 6601, -1, 1118, -1, 0, 0, new WorldTile(2427, 4446, 0), 9, false,
	...
	public boolean processNPCClick4(NPC npc) {
		if (npc.getId() == 6070) {
			...
			player.getDialogueManager().startDialogue("SimpleNPCMessage", 6070, "You already have the right equipment; "
