...
public final class RequestController extends Controller {
			...
			public void run() {
				...
				if (target == null || target.hasFinished() || !target.withinDistance(player, 14)
					...
					player.getPackets().sendGameMessage(
							"Unable to find " + (target == null ? "your target" : target.getDisplayName()));
					return;
				}
	...
	public boolean processObjectClick1(WorldObject object) {
		player.sendMessage(Colors.RED+"Clan wars is currently disabled pending fixes!");
		/*switch (object.getId()) {
		case 28213:
	...
	public void stop() {
		removeControler();
		decline();
		// player.getInterfaceManager().closeInterface(player.getInterfaceManager().hasRezizableScreen()
		// ? 1 : 11);
