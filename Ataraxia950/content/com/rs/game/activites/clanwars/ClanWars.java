...
public final class ClanWars implements Serializable {
	 ...
	 *            The second team.
	 */
	public ClanWars(final FriendChatsManager first, final FriendChatsManager second) {
		firstTeam = first;
		secondTeam = second;
	}
	 ...
	 *            The value.
	 */
	public static void sendConfig(final Player player, final Player other, final int configId, final int value) {
	...
	public static void enter(final Player p) {
		...
		p.getInterfaceManager().sendMinigameHudInterface(265);
		...
		if (hasWar) {
			...
			if (c.firstTeam == p.getCurrentFriendChat()) {
				...
				final WorldTile northEast = c.baseLocation.transform(
			...
			CoresManager.getServiceProvider().executeNow(() -> {
				for (final Player player1 : c.firstPlayers) {
					c.timer.refresh(player1, true);
				}
				for (final Player player2 : c.secondPlayers) {
					c.timer.refresh(player2, false);
				}
			});
			return;
		}
	...
	public void switchRule(final Rules rule, final Player player) {
		...
		if (other == null || player.getTemporaryAttributtes().get("clan_wars") != other.getTemporaryAttributtes()
				.get("clan_wars")) {
			return;
		}
		...
		if (magicRuleCount != 0 && ((rule == Rules.NO_MELEE && get(Rules.NO_RANGE))
				|| (rule == Rules.NO_RANGE && get(Rules.NO_MELEE)))) {
			player.getPackets().sendGameMessage("You can't activate all combat style rules, how would you fight?");
		} else {
	...
	private void sendVictoryConfiguration(final Player p) {
		switch (victoryType) {
		case -1:
			p.getPackets().sendConfigByFile(4095, 0);
			break;
		case 25:
			p.getPackets().sendConfigByFile(4095, 1);
			break;
		case 50:
			p.getPackets().sendConfigByFile(4095, 2);
			break;
		case 100:
			p.getPackets().sendConfigByFile(4095, 3);
			break;
		case 200:
			p.getPackets().sendConfigByFile(4095, 4);
			break;
		case 400:
			p.getPackets().sendConfigByFile(4095, 5);
			break;
		case 750:
			p.getPackets().sendConfigByFile(4095, 6);
			break;
		case 1_000:
			p.getPackets().sendConfigByFile(4095, 7);
			break;
		case 2_500:
			p.getPackets().sendConfigByFile(4095, 8);
			break;
		case 5_000:
			p.getPackets().sendConfigByFile(4095, 9);
			break;
		case 10_000:
			p.getPackets().sendConfigByFile(4095, 10);
			break;
		case -2:
			p.getPackets().sendConfigByFile(4095, 15);
			break;
		}
	}
	...
	private void sendTimeConfiguration(final Player p) {
		switch (timeLeft) {
		case 500:
			p.getPackets().sendConfigByFile(4096, 1);
			break;
		case 1_000:
			p.getPackets().sendConfigByFile(4096, 2);
			break;
		case 3_000:
			p.getPackets().sendConfigByFile(4096, 3);
			break;
		case 6_000:
			p.getPackets().sendConfigByFile(4096, 4);
			break;
		case 9_000:
			p.getPackets().sendConfigByFile(4096, 5);
			break;
		case 12_000:
			p.getPackets().sendConfigByFile(4096, 6);
			break;
		case 15_000:
			p.getPackets().sendConfigByFile(4096, 7);
			break;
		case 18_000:
			p.getPackets().sendConfigByFile(4096, 8);
			break;
		case 24_000:
			p.getPackets().sendConfigByFile(4096, 9);
			break;
		case 30_000:
			p.getPackets().sendConfigByFile(4096, 10);
			break;
		case 36_000:
			p.getPackets().sendConfigByFile(4096, 11);
			break;
		case 48_000:
			p.getPackets().sendConfigByFile(4096, 12);
			break;
		case -1:
			p.getPackets().sendConfigByFile(4096, 0);
			break;
		}
	}
	...
	public void accept(final Player player) {
		final Player other = (Player) player.getTemporaryAttributtes().get("clan_request_p");
		if (other != null && other.getTemporaryAttributtes().get("accepted_war_terms") == Boolean.TRUE) {
			CoresManager.getServiceProvider().executeNow(() -> {
				player.getTemporaryAttributtes().remove("accepted_war_terms");
				other.getTemporaryAttributtes().remove("accepted_war_terms");
				...
				for (final Player p1 : firstTeam.getPlayers()) {
					if (p1 != player && p1 != other) {
						p1.getPackets()
								.sendGameMessage("<col=FF0000>Your clan has been challenged to a clan war!</col>");
						p1.getPackets().sendGameMessage(
								"<col=FF0000>Step through the purple portal in the Challenge Hall.</col>");
						p1.getPackets().sendGameMessage("<col=FF0000>Battle will commence in 2 minutes.</col>");
					}
				}
				for (final Player p2 : secondTeam.getPlayers()) {
					if (p2 != player && p2 != other) {
						p2.getPackets()
								.sendGameMessage("<col=FF0000>Your clan has been challenged to a clan war!</col>");
						p2.getPackets().sendGameMessage(
								"<col=FF0000>Step through the purple portal in the Challenge Hall.</col>");
						p2.getPackets().sendGameMessage("<col=FF0000>Battle will commence in 2 minutes.</col>");
					}
				}
				firstTeam.setClanWars(ClanWars.this);
				secondTeam.setClanWars(ClanWars.this);
				final int width = (areaType.getNorthEastTile().getX() - areaType.getSouthWestTile().getX()) / 8 + 1;
				final int height = (areaType.getNorthEastTile().getY() - areaType.getSouthWestTile().getY()) / 8 + 1;
				...
				CoresManager.getServiceProvider().scheduleFixedLengthTask(timer = new ClanWarsTimer(ClanWars.this), 600, 600, TimeUnit.MILLISECONDS);
				enter(player);
				enter(other);
	...
	public void leave(final Player p, final boolean ingame) {
		...
		p.getInterfaceManager().removeMinigameHudInterface();
	...
	public void updateWar() {
		if ((timer != null &&timer.isStarted()) && isKnockOut()) {
			if (firstPlayers.size() < 1 || secondPlayers.size() < 1) {
				endWar();
				timer.stopNow(false);
			}
		} else if ((timer != null &&timer.isStarted()) && !isMostKills()
	...
	public void endWar() {
		...
		for (final Player player : firstTeam.getPlayers()) {
			player.getPackets().sendGameMessage(firstMessage);
		}
		for (final Player player : secondTeam.getPlayers()) {
	 ...
	 *            The other player.
	 */
	public void setVictoryType(final int victoryType, final Player p, final Player other) {
	 ...
	 *            The other player.
	 */
	public void setTimeLeft(final int timeLeft, final Player p, final Player other) {
	 ...
	 */
	public enum Rules {
		NO_FOOD(5288), NO_POTIONS(5289), NO_PRAYER(5290), NO_MAGIC(-1), NO_MELEE(5284), NO_RANGE(5285), NO_FAMILIARS(
				5287), ITEMS_LOST(4098);
