package com.rs.game.activities.gambling;
...
public class Gambling {
    ...
    private static final int INTERFACE = 9;
     ...
     */
    private final ArrayList<Rune> playerRunes;
    private final ArrayList<Rune> opponentRunes;
    /**
    ...
    private void draw() {
        ...
        if (status == Status.PLAYER_TURN) {
            incrementPlayerScore(rune.getValue());
            playerRunes.add(rune);
        } else if (status == Status.OPPONENT_TURN) {
            incrementOpponentScore(rune.getValue());
    ...
    private void rewardWinner() {
        switch (status) {
            case PLAYER_WIN:
                player.getInventory().addItem(995, gambleAmount * 2);
                ...
                break;
            case OPPONENT_WIN:
                opponent.getInventory().addItem(995, gambleAmount * 2);
                ...
                break;
            case DRAW:
                player.getInventory().addItem(995, gambleAmount);
                opponent.getInventory().addItem(995, gambleAmount);
    ...
    public void endAbruptly(Player abrupter) {
        destroy();
        for (Player player : players) {
    ...
    /**
     * This handles server shutdowns. This is in a player-by-player bases, so it ends it for each player, not
     * a session. That is, this is called for each player, not for each session, in order to avoid duplicates.
     *
     * If this had a for loop for each player in the gamble, when the server would shut down it would give
     * each player the gambleAmount multiple times because then refund would be called for each
     * person in the gamble in their realFinish method. Therefore, it would cause a dupe.
     */
    public void refund(Player player) {
        if (player.getInventory().hasFreeSlots()) {
            player.getInventory().addItem(995, gambleAmount);
        } else {
            player.getBank().addItem(new Item(995, gambleAmount), true);
        }
    }
    ...
    private void updateScore() {
        if (status == Status.PLAYER_TURN) {
            for (int i = 0; i < playerRunes.size(); i++) {
                player.getPackets().sendItemOnIComponent(INTERFACE, PLAYER_COMPONENTS[i], playerRunes.get(i).getId(), 1);
                opponent.getPackets().sendItemOnIComponent(INTERFACE, OPPONENT_COMPONENTS[i], playerRunes.get(i).getId(), 1);
            }
            ...
            for (int i = 0; i < opponentRunes.size(); i++) {
                player.getPackets().sendItemOnIComponent(INTERFACE, OPPONENT_COMPONENTS[i], opponentRunes.get(i).getId(), 1);
                opponent.getPackets().sendItemOnIComponent(INTERFACE, PLAYER_COMPONENTS[i], opponentRunes.get(i).getId(), 1);
            }
    ...
    private void updateStatus() {
        player.getPackets().sendIComponentText(INTERFACE, PLAYER_STATUS_COMPONENT, status.getPlayerStatus());
        player.getPackets().sendIComponentText(INTERFACE, OPPONENT_STATUS_COMPONENT, status.getOpponentStatus());
        opponent.getPackets().sendIComponentText(INTERFACE, PLAYER_STATUS_COMPONENT, getOtherStatus().getPlayerStatus());
        opponent.getPackets().sendIComponentText(INTERFACE, OPPONENT_STATUS_COMPONENT, getOtherStatus().getOpponentStatus());
    }
