...
public class FairyRing {
	...
	private final static String[][] LETTERS = new String[][] { { "a", "b", "c", "d" }, { "i", "j", "k", "l" },
			{ "p", "q", "r", "s" } };
	...
	public static boolean openRingInterface(Player player, WorldTile tile, boolean source) {
		...
		if (source) {
			player.getInterfaceManager().sendCentralInterfaceLargeInterface(734);
...
//		player.getPackets().sendIComponentText(735, 62,
...
//		player.getPackets().sendIComponentText(735, 76, "          Islands: Poison Waste south of Isafdar");
	...
	public static boolean sendRingTeleport(Player player, String hash) {
		...
		if (ring == null || ring.getTile() == null || (ring == Rings.BKQ && !player.isDonator())) {
	...
	private static void resetRingHash(Player player) {
		...
		for (int i = 0; i < 3; i++)
			player.getVarBitManager().sendVarBit(12072 + i, 0);
	}

	private static void sendTeleport(final Player player, final WorldTile tile) {
		Magic.sendTeleportSpell(player, FIRST_ANIMATION, SECOND_ANIMATION, FIRST_GRAPHICS, SECOND_GRAPHICS, 0, 0, tile,
				2, false, Magic.OBJECT_TELEPORT);
	}
	...
	public static void handleDialButtons(final Player player, int componentId) {
		...
		for (int i = 0; i < 3; i++)
			player.getVarBitManager().sendVarBit(12072 + i,
					locationArray[i] == 1 ? 3 : locationArray[i] == 3 ? 1 : locationArray[i]);
	...
	public enum Rings {
		AIQ(new WorldTile(2996, 3114, 0)), AJQ(new WorldTile(2735, 5221, 0)), AJR(new WorldTile(2780, 3613, 0)), AKQ(
				new WorldTile(2319, 3619, 0)), AKS(new WorldTile(2571, 2956, 0)), ALP(new WorldTile(2468, 4189,
						0)), ALQ(new WorldTile(3597, 3495, 0)), ALS(new WorldTile(2644, 3495, 0)), BIP(
								new WorldTile(3410, 3324, 0)), BIQ(new WorldTile(3251, 3095, 0)), BJQ(new WorldTile(
										1737, 5342,
										0)), BJR(new WorldTile(2650, 4730, 0)), BKP(new WorldTile(2385, 3035, 0)), BKR(
												new WorldTile(3469, 3431, 0)), BLP(new WorldTile(4622, 5147, 0)), BLR(
														new WorldTile(2740, 3351, 0)), CIP(new WorldTile(2513, 3884,
																0)), CIQ(new WorldTile(2528, 3127, 0)), CJR(
																		new WorldTile(2705, 3576, 0)), CKR(
																				new WorldTile(2801, 3003, 0)), CKS(
																						new WorldTile(3447, 3470,
																								0)), CLR(new WorldTile(
																										2735, 2742,
																										0)), CLS(
																												new WorldTile(
