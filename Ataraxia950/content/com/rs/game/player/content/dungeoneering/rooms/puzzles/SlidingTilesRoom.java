...
public class SlidingTilesRoom extends PuzzleRoom {
	...
	public static boolean handleSlidingBlock(Player player, NPC npc) {
		if (!npc.getName().equals("Sliding block") || player.getControlerManager().getControler() == null || !(player.getControlerManager().getControler() instanceof DungeonController)) {
			return false;
		}
		DungeonManager manager = player.getDungeoneeringManager().getParty().getDungeon();
