...
public class DungeoneeringMining extends MiningBase {
	...
	public enum DungeoneeringRocks {
		...
		CRUMBLING_WALL(1, 10, 17415, 20, 5);
	...
	public boolean start(Player player) {
		axeDefinitions = Mining.getPickaxeDefinitions(player, true);
		if (!checkAll(player))
			return false;
		player.getPackets().sendGameMessage("You swing your pickaxe at the " + (definitions.equals(DungeoneeringRocks.CRUMBLING_WALL) ? "crumbling wall." : "rock."));
	...
	public int processWithDelay(Player player) {
		if (definitions.equals(DungeoneeringRocks.CRUMBLING_WALL)) {
			player.getInventory().addItem(definitions.getOreId(), 1);
			player.getSkills().addXp(Skills.MINING, definitions.getXp());
			player.sendMessage("You mine a stone block from the crumbling wall.");
	...
	private void addOre(Player player) {
		...
		if (Utils.random(100) < player.getRingOfKinship().getBoost(RingOfKinship.GATHERER))
			if (player.getInventory().addItemDrop(definitions.oreId, 1))
				player.getPackets().sendGameMessage("You manage to gather another " + oreName + ".");
