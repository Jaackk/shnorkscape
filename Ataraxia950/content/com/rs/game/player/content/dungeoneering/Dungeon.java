...
public final class Dungeon {
	...
	public static void main(String[] args) {
		...
		while (true) {
			Logger.getGlobal().info("TOOK: " + (Utils.currentTimeMillis() - lastDung) + ", ");
			...
			for (int i = test.map[0].length - 1; i >= 0; i--) {
				for (int j = 0; j < test.map.length; j++) {
					...
					if (test.map[j][i] != null) {
						...
						s.append(")").append(test.map[j][i].getRoom().getDoorDirections().length).append(" ");
						if (key != -1) {
							s.append(" K").append((key - 18202) / 2);
						}
						for (int l = 0; l < test.map[j][i].getRoom().getDoorDirections().length; l++) {
							...
							if (lock0 >= 0 && lock0 < 64) {
								s.append(" L").append(rotation).append("_").append(lock0);
							}

						}
					}
					System.out.print(padRight(s.toString(), 30));
				}
				System.out.println();
			}
...
//					g.drawString("\033" + tier, x * 20 + 36, (7 - y) * 20 + 49);
	...
	public Dungeon(DungeonManager manager, int floorId, int complexity, int size) {
		...
		DungeonStructure structure = new DungeonStructure(size, random, complexity, manager == null || manager.getParty().isKeyShare());
		...
		children.add(base);
		long eligiblePuzzleRooms = getEligiblePuzzleRoomsCount(children);
		...
		for (RoomNode node : children) {
			...
			else if (node.children.size() > 0 && childrenHaveNoLock(node) && puzzleChance > random.nextDouble()) {
				puzzle = true;
		...
		if (Settings.DEBUG)
			Logger.getGlobal().info("Dungeon roomCount: " + creationCount);
	}

	private long getEligiblePuzzleRoomsCount(List<RoomNode> children) {
		long count = 0;
		for (RoomNode r : children) {
			if (r.children.size() > 0 && childrenHaveNoLock(r)) {
				count++;
			}
		}
		return count;
	...
	private boolean childrenHaveNoLock(RoomNode node) {
