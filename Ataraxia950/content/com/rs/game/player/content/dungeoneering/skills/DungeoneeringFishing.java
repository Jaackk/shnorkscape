...
public class DungeoneeringFishing extends Action {
	...
	private void addFish(Player player) {
		...
		if (Utils.random(100) < player.getRingOfKinship().getBoost(RingOfKinship.GATHERER))
			if (player.getInventory().addItemDrop(spot.getFish().id, 1))
				player.getPackets().sendGameMessage("You manage to catch another " + ItemDefinitions.getItemDefinitions(spot.getFish().id).getName().toLowerCase() + ".");
	...
	private boolean checkAll(Player player) {
		...
		if (!player.getInventory().containsItem(FLY_FISHING_ROAD, 1) && !player.getDungeoneeringToolbelt().containsTool(FLY_FISHING_ROAD)) {
