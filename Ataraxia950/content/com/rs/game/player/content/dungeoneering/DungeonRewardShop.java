...
public class DungeonRewardShop {
	...
	public enum DungeonReward {
		...
		private static final Map<Integer, DungeonReward> monsters = new HashMap<Integer, DungeonReward>();
	...
	public static void refresh(Player player) {
		player.getPackets().sendIComponentText(940, 31, "" + player.getDungeoneeringManager().getTokens());
	}

	public static void handleButtons(Player player, int componentId, int slotId, int packetId) {
		if (componentId == 64 && packetId == PacketRepository.ACTION_BUTTON1_PACKET) {
		...
		if (componentId == 2) {
			...
			if (reward == null) {
				...
				player.sm(reward.getName() + " requires " + reward.getRequirement() + " dungeoneering and costs " + reward.getCost() + " dungeoneering tokens.");
			}
		}
	}
