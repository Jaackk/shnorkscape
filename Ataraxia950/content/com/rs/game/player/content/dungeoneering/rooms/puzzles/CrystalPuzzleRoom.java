...
public class CrystalPuzzleRoom extends PuzzleRoom {
	...
	private void resetPosition(final int color) {
		active[color] = true;
		...
		public void run() {
				synchronized (manager) {
					...
					outer: for (int color = 0; color < 4; color++) {
						...
						for (final Player player : manager.getParty().getTeam()) {
							final WorldTile tile = manager.getTile(reference, POS_PLATE[color][0], POS_PLATE[color][1]);
							
							if(tile == null) {
								return;
							}
							if (player.getTileHash() == tile.getTileHash()) {
								continue outer;
							}
						}
					...
					if (complete) {
						if (manager.getTile(reference, POS_CENTER[0], POS_CENTER[1]) == null) {
							return;
						}
