		...
		if (object.getDefinitions().name.equals("Locked chest")) {
			...
			for (Connector connector : connectors) {
				if (connector.blocked) {
					World.spawnObject(new WorldObject(49063, 10, 0, manager.getTile(reference, connector.x, connector.y)));
				}
			}
			if (!expired) {
				manager.getRoom(reference).setThiefChest(0);
				return true;
			} else {
				...
				if (connector.blocked) {
					if (connector.left != null && connector.right != null) {
						if (connector.left.door.visited ^ connector.right.door.visited) {
							...
							progress = true;
							//Logger.getGlobal().info("unblocked: "+connector.x+" "+connector.y);
						}
					}
				}
