...
final class LightPathMechanic extends FixedLengthRunnable {
	...
	private final Map<Player, List<Hit>> hitsTaken;
	...
	public boolean repeat() {
		...
		if (tick == 0) {
			...
			World.spawnObject(new WorldObject(91664, 10, 0, new WorldTile(lightTile.getX(), lightTile.getY() + 1, lightTile.getPlane())));
		...
		if (tick == 4) {
			fight.getBoss().setLocation(new WorldTile(fight.getBoss().getX(), fight.getBoss().getY(), fight.getBoss().getPlane() + 1));
			fight.getBoss().setCantInteract(false);
		}
		if (tick == duration - 25) {
			fight.getPlayers().forEach(player -> player.getPackets().sendPlayerMessageBox("Araxxor " + "begins to come down and charge..."));
		}
		if (tick % 5 == 0) {
			fight.getPlayers().forEach(p -> {
				if (p != null && !p.isDead() && !p.hasFinished()) {
					NewProjectile projectile = new NewProjectile(new WorldTile(p, 4), p, attackTypeFlag ? 4979 : 4997, 25, 25, 0, 35, 15, 0);
					World.sendProjectile(projectile);
					CoresManager.getServiceProvider().executeWithDelay(() -> p.applyHit(new Hit(fight.getBoss(), Utils.random(50, 300), attackTypeFlag ? Hit.HitLook.MAGIC_DAMAGE : Hit.HitLook.RANGE_DAMAGE)), projectile.getTime() / 335);
			...
			fight.getPlayers().forEach(player -> {
				if (player != null && !player.isDead() && !player.hasFinished()) {
					if (!config.onLightTile(player, lightTile)) {
						Hit hit = new Hit(fight.getBoss(), currentDamage += 2, Hit.HitLook.POISON_DAMAGE);
						player.applyHit(hit);
		...
		if (tick % 25 == 0) {
			if (lightTile != null) {
				...
				World.removeObject(World.getObject(new WorldTile(lightTile.getX(), lightTile.getY() + 1, lightTile.getPlane())));
			}
			lightTile = config.generateLightTile();
			fight.getPlayers().forEach(player -> {
		...
		if (tick % 15 == 0 && Utils.random(5) == 0) {
			...
			NewProjectile projectile = new NewProjectile(fromTile, target, 5073, 40, 40, 10, NewProjectile.DEFAULT_DELAY);
			projectileDuration = Utils.getDistance(target.getX(), target.getY(), fromTile.getX(), fromTile.getY()) / (projectile.getSpeed() / 10) / 2;
			...
			for (Player target : fight.getPlayers()) {
				if (target != null && !target.isDead() && !target.hasFinished()) {
					if (targetOnEggs(target)) {
						Hit scaledHit = fight.getBoss().scaleBaseHit(target, target == fight.getBoss().getTarget() ? 100 : 50, Hit.HitLook.CRITICAL_DAMAGE);
						target.applyHit(scaledHit);
						target.sendMessage("Araxxor becomes more enraged with you...");
						blowUp = true;
					} else {
						Hit scaledHit = fight.getBoss().scaleBaseHit(target, target == fight.getBoss().getTarget() ? 300 : 200, Hit.HitLook.CRITICAL_DAMAGE);
						target.applyHit(scaledHit);
					}
				}
			}
	...
	private boolean targetOnEggs(Player target) {
		return eggDrops.stream().anyMatch(tile -> tile.getTileHash() == target.getTileHash());
	}
}
