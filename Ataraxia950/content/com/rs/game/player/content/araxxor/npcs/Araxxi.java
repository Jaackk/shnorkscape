...
/**
 * The {@code Araxxi} class defines NPC spawning mechanics and is directly linked to an {@link AraxxorFight} object. The Araxxi combat script will
 * refer directly to the {@code Araxxi} object to perform special attacks. All special attacks are self-contained in private methods in the class, or
...
public final class Araxxi extends Araxyte {
	...
	/**
	 * Generate the special attacks Araxxi should use based on the fight configuration.
	...
	/**
	 * Picks and performs a random Araxxi special attack. The same special attack will never be performed twice in a row, nor repeated until Araxxi has
	...
	/**
	 * Increments the global attack tick by 1 and returns its value. Araxxi performs a special attack every 6 global attack ticks.
	 ...
	 * @return <tt>true</tt> if Araxxi's next auto attack is a mage attack, false otherwise
	 */
	boolean shouldMage() {
	...
	/**
	 * Calculates the scaled attack delay due to the enrage effect.
	 * @return Araxxi's current attack delay scaled to enrage
	 */
	int getAttackDelay() {
		return enrage >= 75 ? 7 : 8;
	}

	/**
	 * Perform the parent {@code processNPC()}, but also check to see if it is time to cap damage at 500, or to send (or resend) the sludge attack.
	 */
	@Override
	...
	/**
	 * Perform the parent {@link Araxyte#processHit(Hit)}, but also adjust the enrage level and update the player interfaces.
	 * @param hit the {@code Hit} to process
	 */
	@Override
	public void processHit(Hit hit) {
		if (Utils.random(8) == 0 && enrage < 300)
			enrage += 2;
	...
	/**
	 * Perform the parent {@code sendDeath(Entity source)}, but send custom death animations, and notify the fight instance that it is time to finish and
	 * send the rewards.
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0)
					...
					World.spawnObject(new WorldObject(91673, 10, rot, new WorldTile(Araxxi.this)));
					fight.setRewarded(fight.getBoss().getMostDamageReceivedSourcePlayer());
					fight.getPlayers().forEach(p -> {
						if (p != null && !p.isDead() && !p.hasFinished()) {
							ContractHandler.updateContract(p, Araxxi.this);
							HybridTokenDistributor.rollForToken(p, HybridTokenDistributor.Activity.ARAXXOR);
							p.getActivityTimersManager().finishBossTimer(Araxxi.this);
						}
					});
					Araxxi.this.reset();
					Araxxi.this.finish();
	...
	/**
	 * Performs the sludge attack.<br/>
	 * <br/>
	 * This isn't a direct attack from Araxxi, but rather a separate mechanic that begins along-side Araxxi's combat. A bouncing black sludge ball takes a
	 * random number of bounces before honing in on its target. In group difficulty, there will be a separate sludge attack per player.
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			private void generateBouncePath() {
				...
				for (int i = 0; i < bounces; i++) {
					...
					projectiles[i] = new NewProjectile(i == 0 ? Araxxi.this : bounceLocations[i - 1], bounceLocations[i], 5012, 25, 25, 0, 35, 15, 0);
			...
			public void run() {
				...
				if (sent == bounces && finalProjectile == null) {
					finalProjectile = new NewProjectile(bounceLocations[bounces - 1], new WorldTile(target), 5012, 25, 25, 0, 35, 15, 0);
					finalStart = tick;
				...
				if (finalProjectile != null && tick == finalStart + finalDelay) {
					if (target.withinDistance(finalProjectile.getTo(), 2)) {
						fight.getPlayers().forEach(p -> {
							p.applyHit(new Hit(Araxxi.this, p == target ? 250 : 150, Hit.HitLook.POISON_DAMAGE));
