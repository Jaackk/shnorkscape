...
public class AraxxiCombat extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		final NPCCombatDefinition defs = araxxi.getCombatDefinitions();
		boolean mage = araxxi.shouldMage();
		...
		if (target instanceof Player) {
			...
			if (player.getPrayer().isMeleeProtecting() && araxxi.shouldMage())
				mage = true;
			else if (player.getPrayer().isRangeProtecting() && !araxxi.shouldMage())
				mage = false;
		}
		if (mage)
	...
	public Object[] getKeys() {
		return new Object[] { /* Araxyte.ARAXXI */"old araxxi" };
	}

	private void meleeAutoAttack(Araxxi araxxi, Entity target, NPCCombatDefinition defs) {
		Hit unscaledHit = getMeleeHit(araxxi, getRandomMaxHit(araxxi, defs.getMaxHit(), NPCCombatDefinitionConstants.MELEE, target));
	...
	private void mageAutoAttack(Araxxi araxxi, Entity target, List<Entity> possibleTargets, NPCCombatDefinition defs) {

		for (Entity entity : possibleTargets) {
			if (entity != null && !entity.isDead() && !entity.hasFinished()) {
				if (entity.withinDistance(araxxi, 1) && Utils.random(3) == 2) {
					meleeAutoAttack(araxxi, entity, defs);
					return;
				}
				float dmg = getRandomMaxHit(araxxi, defs.getMaxHit(), NPCCombatDefinitionConstants.MAGE, entity);
	...
	private void rangeAutoAttack(Araxxi araxxi, Entity trueTarget, List<Entity> possibleTargets, NPCCombatDefinition defs) {
		...
		for (Entity entity : possibleTargets) {
			if (entity != null && !entity.isDead() && !entity.hasFinished()) {
				...
				float totalDamage = getRandomMaxHit(araxxi, defs.getMaxHit(), NPCCombatDefinitionConstants.RANGE, entity);
