...
/**
 * The hub of all Araxxor fights. Calculates the current rotation at startup time, and tracks and processes all the live instances.
 *
 * <br/>
 * <br/>
...
public final class AraxxorManager {
	...
	private static final AraxxorEnvironment.Paths[] CURRENT_PATHS = new AraxxorEnvironment.Paths[2];
	...
	/**
	 * Synchronize the Araxxor rotation for today's date by setting the current rotation paths for the environment configurations.
	...
	public static void init(Calendar calendar) {
		 ...
		 * 
		 * if (ROT_2.contains(day)) { Logger.getGlobal().info(Logger.DEBUG, "Current Araxxor rotation: 1 --> Minions and Acid"); CURRENT_PATHS = new
		 * AraxxorEnvironment.Paths[] { AraxxorEnvironment.Paths.MINIONS, AraxxorEnvironment.Paths.ACID }; OFF_PATH = AraxxorEnvironment.Paths.LIGHT; } else
		 * if (ROT_1.contains(day)) { Logger.getGlobal().info(Logger.DEBUG, "Current Araxxor rotation: 2 --> Minions and Acid"); CURRENT_PATHS = new
		 * AraxxorEnvironment.Paths[] { AraxxorEnvironment.Paths.MINIONS, AraxxorEnvironment.Paths.ACID }; OFF_PATH = AraxxorEnvironment.Paths.LIGHT; } else
		 * if (ROT_3.contains(day)) { Logger.getGlobal().info(Logger.DEBUG, "Current Araxxor rotation: 3 --> Minions and Acid"); CURRENT_PATHS = new
		 * AraxxorEnvironment.Paths[] { AraxxorEnvironment.Paths.MINIONS, AraxxorEnvironment.Paths.ACID }; OFF_PATH = AraxxorEnvironment.Paths.LIGHT; }
		 * 
		 * Logger.getGlobal().info(Logger.DEBUG, "AraxxorManager initialized.");
		 */
	}

	public static AraxxorEnvironment.Paths[] getCurrentPaths() {
		return CURRENT_PATHS;
	}

	/**
	 * Safely finish the final internal steps of an {@link AraxxorFight}.<br/>
	 * Send the players a congratulatory message, informing them of their new Araxxor kill count, and warn them they have 3 minutes left to loot the
	 * corpse and leave the instance. Starts a {@link FixedLengthRunnable} to perform player removal if necessary after 3 minutes.
		...
		CoresManager.getServiceProvider().scheduleFixedLengthTask(new FixedLengthRunnable() {
			...
			public boolean repeat() {
				...
				if (time == 180) {
					...
					fight.notifyPlayers(Colors.GREEN + "You have 2 minutes " + "to claim your loot and exit " + "before you are forcefully removed.");
				} else if (time == 120) {
					fight.notifyPlayers(Colors.YELLOW + "You have 1 minute " + "to claim your loot and exit " + "before you are forcefully removed.");
				} else if (time == 150) {
					fight.notifyPlayers(Colors.RED + "You have 30 seconds" + "to claim your loot and exit " + "before you are forcefully removed.</col>");
				}
				time++;
				return true;
			}
