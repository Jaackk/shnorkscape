...
/**
 * The {@code AraxxorFight} is a data structure which represents the state of an Araxxor instance for a group of players. It is responsible for
 * properly scheduling phases of the fight, and transitioning between phases. It also controls mechanics which are independent of the boss NPCs, such
 * as the light path mechanic, and the suffocation effect after defating phase two of acid path.
 *
 * <br/>
 * <br/>
...
public final class AraxxorFight {
	...
	private Player rewarded;

	private Araxyte boss;

	/**
	 * Constructs a new Araxxor fight instance. The environment is generated internally in this constructor call.
	...
	/**
	 * Accessor to the {@link Araxyte} boss NPC currently spawned in the fight.
	 ...
	 * @return a {@code boolean} indicating if one of the webs has begun burning
	 */
	...
	/**
	 * Negates the suffocation flag. When this method is called, the {@code WorldTask} responsible for suffocating the players will invoke
	 * {@link WorldTask#stop()}.
	 */
	void stopSuffocate() {
	 ...
	 * @return {@code true} if the fight is shutting down, {@code false} otherwise
	 */
	...
	/**
	 * Retruns one of the players' rewards from this fight instance.
	 * @param player the player whose reward to pull from the rewards map.
	 * @return the {@link AraxxorReward} mapped to the supplied player
	 */
	...
	/**
	 * Increments the {@code stage} variable, and transitions to the appropriate next stage of the fight.<br/>
	 * <br/>
	 * All necessary environment, player, or boss changes are handled by this method.
	 */
	public void nextPhase() {
		...
		if (phase == 3) {
			...
			if (leftover > 2000) {
				...
				Araxxor rax = (Araxxor) boss;
				rax.setToHeal(Math.round(healed));
			}
			if (config.getActivePath() != AraxxorEnvironment.Paths.ACID)
				notifyPlayersMsgBox("Araxxor is healed by the spiders above!");
			boss.applyHit(new Hit(null, boss.getMaxHitpoints() - boss.getHitpoints(), Hit.HitLook.HEALED_DAMAGE));
		}
		if (phase == 2 && config.getActivePath() == AraxxorEnvironment.Paths.LIGHT) {
			players.forEach(AraxxorInterfaces::sendDarkness);
			CoresManager.getServiceProvider().scheduleFixedLengthTask(new LightPathMechanic(this), 0, 600, TimeUnit.MILLISECONDS);
	...
	private void startAraxxi() {
		...
		players.forEach(p -> {
			p.setNextWorldTile(config.getRelativeTile(new WorldTile(4574, 6263, 1)));
			p.setNextFaceWorldTile(new WorldTile(p.getX() + 1, p.getY(), p.getPlane()));
		});
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			final byte[] dirPlayer = Utils.getDirection(players.get(Utils.random(players.size())).getDirection());
			...
			public void run() {
				...
				if (tick == 1) {
					...
					boss.setNextForceMovement(new ForceMovement(boss, 0, config.getRelativeTile(new WorldTile(4589, 6265, 1)), 4, angleBoss));
					players.forEach(p -> {
						p.getCutscenesManager().play("AraxxiStartCS");
						p.setNextForceMovement(new ForceMovement(p, 0, config.getRelativeTile(new WorldTile(4588, 6260, 1)), 4, anglePlayer));
					});
				} else if (tick == 2) {
					players.forEach(p -> p.setNextAnimation(new Animation(19502)));
					boss.setNextAnimation(new Animation(-1));
				} else if (tick == 4) {
					players.forEach(player -> player.setNextWorldTile(config.getRelativeTile(new WorldTile(4588, 6260, 1))));
					boss.setNextWorldTile(config.getRelativeTile(new WorldTile(4589, 6265, 1)));
	...
	private void spawnAraxxi() {
		final int deathId = NPCCombatDefinitionsDataParser.getNPCCombatDefinitions(Araxyte.ARAXXI).getDeathEmote();
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (tick == 0) {
					...
					boss = new Araxxi(Araxyte.ARAXXI, initialTile, AraxxorFight.this);
	...
	/**
	 * Indicate the fight is over (phase 5), initialize the rewards map, increment player Araxxor enrage level, spawn the exit tile, and pass this fight
	 * instance to the {@link AraxxorManager} for post-fight processing. The player gains 20% more enrage. If the player has full enrage, they gain no
	 * enrage and if they have no starting enrage, they gain an automatic 20 enrage + 20% that first kill.
	 */
	public void finish() {
		phase = 5;
		...
		players.forEach(p -> {
			...
			rewardMap.put(p, new AraxxorReward(p));
			int percent = (int) Math.ceil(p.getAraxxorEnrage() * 0.2);
			p.addAraxxorEnrage(p.getAraxxorEnrage() >= 300 ? 0 : percent < 1 ? (20 + (int) Math.ceil(p.getAraxxorEnrage() * 0.2)) : percent);
		});
	...
	/**
	 * Safely remove a specified player from this fight. The fight is unmapped from the player in the {@link AraxxorManager}, the player is removed from
	 * this fight's player list, and all interfaces are closed.<br/>
	 * <br/>
	 * If the player list is empty after this removal, the fight will enter shutdown mode.
	...
	/**
	 * Enter shutdown state. Set the {@code shutdown} flag to {@code true}, causing all {@code WorldTask} objects associated with the fight to invoke
	 * {@link WorldTask#stop()}, finishing the boss NPC if required, and unmapping the environment chunks.
	 */
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (tick == 1) {
					...
					AraxxorFight.this.boss.setNextFaceWorldTile(AraxxorFight.this.config.getCutsceneAnchorOne());
					...
					Araxxor.SwingDirection swingDirection = Araxxor.SwingDirection.values()[Utils.random(Araxxor.SwingDirection.values().length)];
					...
					players.forEach(p -> {
						if (!(correctKey = AraxxorManager.controllerOf(p).checkCorrectKey())) {
							CoresManager.getServiceProvider().executeWithDelay(() -> {
								p.applyHit(new Hit(AraxxorFight.this.boss, 200, Hit.HitLook.CRITICAL_DAMAGE));
								p.setNextAnimation(new Animation(10070));
								p.getDialogueManager().startDialogue("SimpleMessage", "You fail to judge Araxxor's attack as he slams through you!");
					...
					AraxxorFight.this.boss.setNextWorldTile(new WorldTile(AraxxorFight.this.boss.getX(), AraxxorFight.this.boss.getY(), AraxxorFight.this.boss.getPlane() + 1));
					if (lpWallHealth > 0) {
						CoresManager.getServiceProvider().executeWithDelay(AraxxorFight.this::callChargeCutscene, 5, TimeUnit.SECONDS);
						players.forEach(p -> p.getPackets().sendPlayerMessageBox("Araxxor climbs " + "back up, and plans to charge again!"));
					} else {
...
//						AraxxorFight.this.players.forEach(p -> p.getInterfaceManager().closeSecondaryOverlay());
						CoresManager.getServiceProvider().executeWithDelay(AraxxorFight.this::nextPhase, 1200, TimeUnit.MILLISECONDS);
					}
					stop();
				}
	...
	/**
	 * Suffocate the players in the fight, causing poison damage every two game ticks. The damage dealt increases steadily until the players navigate past
	 * the acid path ramp.
	 */
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (checkShutdown())
					return;
				if (tick == 0) {
					players.forEach(player -> player.sendMessage("The noxious fumes " + "from the leftover acid begin to suffocate you!"));
				}
				if (tick % 3 == 0) {
					players.forEach(player -> player.getPackets().sendPlayerMessageBox("The noxious fumes " + "from the leftover acid begin to suffocate you! Jump off the ramp!"));
					damage += 10;
				}
				if (suffocate && tick % 2 == 0) {
					players.forEach(player -> {
						if (player != null && !player.isDead() && !player.hasFinished())
							player.applyHit(boss.scaleBaseHit(player, damage, Hit.HitLook.POISON_DAMAGE));
					});
				}
				if (!suffocate)
					stop();
	...
	/**
	 * Performs the initial Araxxor spawn. Araxxor is randomly spawned in one of his three forms (melee, range, mage), unless the group leader (or just
	 * the player if solo mode) is holding an Araxyte pheremone.
	 */
	private void spawnAraxxor() {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			private boolean processPheromone() {
				final Player pheromoneHolder = players.stream().filter(p -> p.getInventory().containsItem(33870, 1)).findFirst().orElse(null);
				final boolean pheremonePresent = pheromoneHolder != null;
				if (pheremonePresent) {
					ItemDefinitions defs = ItemDefinitions.getItemDefinitions(pheromoneHolder.getEquipment().getWeaponId());
					if (defs.isMagicTypeWeapon())
						...
						araxxorId = rand == 0 ? Araxyte.ARAXXOR_NPC_ID_MELEE : rand == 1 ? Araxyte.ARAXXOR_NPC_ID_MAGE : Araxyte.ARAXXOR_NPC_ID_RANGE;
					}
					players.forEach(p -> p.sendMessage("The Araxytes smell your " + (players.size() > 1 ? "group's " : "") + "pheromone."));
				}
			...
			public void run() {
				...
				if (tick == 10) {
					...
					boss.setCantInteract(true);
					boss.setNextFaceWorldTile(new WorldTile(boss.getX() - 1, boss.getY(), boss.getPlane()));
	 ...
	 */
	public void setRewarded(Player rewarded) {
