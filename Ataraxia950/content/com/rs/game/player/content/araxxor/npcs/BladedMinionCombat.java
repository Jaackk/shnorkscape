...
public class BladedMinionCombat extends CombatScript {
	...
	private void meleeAutoAttack(BladedMinion minion, Entity target, NPCCombatDefinition defs) {

		if (target != null && !target.isDead() && !target.hasFinished()) {
			Hit hit = getMeleeHit(minion, getRandomMaxHit(minion, defs.getMaxHit(), NPCCombatDefinitionConstants.MELEE, target));
	...
	public Object[] getKeys() {
		return new Object[] { /* AraxyteMinion.MinionType.BLADED.npcId */ "old blad minion" };
	}
}
