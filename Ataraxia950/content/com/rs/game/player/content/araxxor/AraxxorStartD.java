 ...
 *        1.1 (beta, 08/19/2017)
 */
public class AraxxorStartD extends Dialogue {

	@Override
	public void start() {
		sendDialogue(Colors.RED + ("Beyond this point is the Araxyte hive."), Colors.RED + ("There is no way out other than death or Victory."), Colors.RED + ("Only those who can endure dangerous encounters should proceed."));
		stage = 0;
	...
	public void run(int interfaceId, int componentId) {
		if (stage == 0) {
			stage = 1;
			sendOptionsDialogue("What would you like to do?", "Enter the Araxyte Cave.", "Start a custom session.", "See Araxxor Info");
			...
			case OPTION_1:
				...
				break;
			case OPTION_2:
				player.sendInputName("Who would you like to " + "invite to your group?", new InputNameEvent() {
					@Override
					public void run(Player player) {
						...
						if (groupMember != null) {
							...
							player.sendMessage("No player " + username + " is online.");
					}
				});
				end();
				break;
			case OPTION_3:
				player.sendMessage(String.format("Today's paths are %s and %s. Your current enrage level is %d%%.", AraxxorManager.getCurrentPaths()[0].getDisplay(), AraxxorManager.getCurrentPaths()[1].getDisplay(), player.getAraxxorEnrage()));
				end();
				break;
			}
		}
