...
/**
 * An environment configuration object containing all instance region information and current rotation information. <br/>
 * <br/>
...
public final class AraxxorEnvironment {
	...
	private final AraxxorFight fight;
	 ...
	 * @param currentPaths the available paths in this environment configuration, determined by the current Araxxor cycle. See {@link AraxxorManager}.
	 */
	...
	/**
	 * Constructs the initial environment layout. This process binds the map chunks corresponding to the ground-truth Araxxor cave to an instance chunks
	 * array, spawns the rotation-appropriate path entrances, and sends the players in the fight to the instanced region.
	 */
	private void buildEnvironment() {

		CoresManager.getServiceProvider().executeNow(() -> {
			boundChunks = MapBuilder.findEmptyChunkBound(chunkDimX, chunkDimY);
			MapBuilder.copyAllPlanesMap(southWestChunks[0], southWestChunks[1], boundChunks[0], boundChunks[1], chunkDimX, chunkDimY);
			initAcidPool();
			initReferenceTiles();
			Arrays.stream(currentPaths).forEach(path -> {
				WorldObject web = new WorldObject(path.getWebId(), 10, 0, getRelativeTile(path.getEntranceTile()));
				WorldObject click = new WorldObject(path.getObjectId(), 10, 0, getRelativeTile(path.getEntranceClickTile()));
			...
			WorldObject rocks = new WorldObject(offPath.getBlockedId(), 10, 0, getRelativeTile(offPath.getEntranceTile()));
	...
	void destroyEnvironment() {
		CoresManager.getServiceProvider().executeWithDelay(() -> MapBuilder.destroyMap(boundChunks[0], boundChunks[1], chunkDimX, chunkDimY), 10000, TimeUnit.MILLISECONDS);
	...
	/**
	 * Accessor method to the first cutscene anchor. This anchor corresponds to the cutscene clip when looking down light path at Araxxor before he
	 * charges.
	...
	/**
	 * Accessor method to the second cutscene anchor. This anchor corresponds to the cutscene clip of Araxxor climbing back up onto the ceiling after
	 * hitting the wall.
	...
	/**
	 * Accessor method to the first "look" tile. This gives a quasi-first person view of Araxxor before charging.
	...
	/**
	 * Accessor method to the second "look" tile. This orients the camera to look at the wall while Araxxor climbs back up.
	...
	/**
	 * Accessor method to the third cutscene anchor. This anchor corresponds to the cutscene clip when looking at the platform Araxxor jumps from to get
	 * to the Araxxi platform.
	...
	/**
	 * Accessor method to the third "look" tile. This orients the camera to look at the platform Araxxor jumps from.
	...
	/**
	 * Accessor method to the light path wall location. This is the wall that Araxxor slams into during the LP cutscene.<br/>
	 * <br/>
	...
	/**
	 * Accessor method to the ramp location at the end of the acid pool.<br/>
	 * <br/>
	 ...
	 *
	 * 250 WorldTile locations are sampled in a 3 tile radius of the player, and candidate grids are extracted from the sampled points.
	 *
	 * If a 1x1 grid cannot be found, the 'grid' defaults to a single WorldTile underneath Araxxor's current target's feet.
	 * @return an array of 4 WorldTile objects forming a 1x1 grid, or 1-length array containing the current targets location.
	 */
	public WorldTile[] generateEggGrid(Player eggBombTarget, int radius) {
		...
		while (free.size() < 100) {
			...
			if (verifyAllTileHashes(tile, eggBombTarget) && World.canMoveNPC(tile, 1))
				free.add(tile);
		}
	...
	/**
	 * Verifies that a given WorldTile doesn't not collide with any of the important, pre-existing reference tiles in the instance.
	 * @param tile the tile to check for collisions
	 * @return a <tt>boolean</tt> indicating whether or not there were any conflicts
	 */
	...
	/**
	 * Begins burning down the selected web. The web will burn for one minute before it disappears, allowing the players to begin the next phase of the
	 * fight.
	 * @param webId the id corresponding to the selected web to burn
	 */
	void burnWeb(int webId) {
		fight.setWebBurning();
		activePath = Objects.requireNonNull(Arrays.stream(currentPaths).filter(p -> webId == p.getObjectId()).findFirst().orElse(null));
		World.spawnObject(new WorldObject(activePath.getBurningWebId(), 10, 0, fight.getConfig().getRelativeTile(activePath.getEntranceTile())));
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (checkShutdown())
					return;
				if (tick == 60) {
					fight.notifyPlayersMsgBox("The web burns down, " + "allowing access!");
	...
	/**
	 * Checks to see if the player is standing in the light beam during the light path mechanic.
	...
	/**
	 * Generates a new tile location for the next light beam during the light path mechanic.
	...
	/**
	 * Accessor method to acid pool verticies.<br/>
	 * <br/>
	 * In order to determine whether or not Araxxor is in the acid pool, a polygon is formed which encapsulates the acid pool, and then the NPC location
	 * is passed through a ray-crossing algorithm to see if the location is within said polygon. The verticies of the polygon are contained in the
	 * <code>acidPool</code> list.
	...
	/**
	 * Clips tiles in a square region spanned from the supplied clipping root. This is used for inducing clippling behaviour on the webs and rock walls
	 * (which for some reason aren't clippable in their definitions).
	 * @param clipRoot the root location to begin clipping tiles from
	 */
	private void clipTiles(WorldTile clipRoot) {
		/*
		 * CoresManager.getServiceProvider().executeWithDelay(() -> { for (int i = 0; i < 8; i++) { for (int j = 0; j < 8; j++) {
		 * World.getRegion(clipRoot.getRegionId()).forceGetRegionMap().clipTile(clipRoot.getPlane(), clipRoot.getXInRegion() + i, clipRoot.getYInRegion() +
		 * j); } } }, 1000, TimeUnit.MILLISECONDS);
		 */
	}
	...
	void unclipTiles(WorldTile clipRoot) {
		 ...
		 * World.getRegion(clipRoot.getRegionId()).forceGetRegionMap().unclipTile(clipRoot.getPlane(), clipRoot.getXInRegion() + i, clipRoot.getYInRegion() +
		 * j); } }
		 */
	}
	 ...
	 */
	public void unclipTiles(List<WorldTile> tiles) {//im guessing these methods are for instance fight not regular where is the main area? it was instance only p sure
		/* tiles.forEach(t -> World.getRegion(t.getRegionId()).forceGetRegionMap().unclipTile(t.getPlane(), t.getXInRegion(), t.getYInRegion())); */
	}
	...
	public enum Paths {
		...
		LIGHT(91667, 91511, 91507, 91512, "Light Path", new WorldTile(4507, 6248, 1), new WorldTile(4509, 6252, 1), new WorldTile(4509, 6246, 1)), ACID(91668, 91509, 91506, 91510, "Acid Path", new WorldTile(4512, 6261, 1), new WorldTile(4513, 6264, 1), new WorldTile(4514, 6261, 1)), MINIONS(91669, 91504, 91505, 91508, "Minions Path", new WorldTile(4504, 6272, 1), new WorldTile(4506, 6274, 1), new WorldTile(4506, 6274, 1));

		private final int objectId;
		private final int webId;
		private final int burningWebId;
		private final int blockedId;
		private final String display;
		private final WorldTile entranceTile;
		private final WorldTile entranceClickTile;
		private final WorldTile clipRoot;

		Paths(int objectId, int webId, int burningWebId, int blockedId, String display, WorldTile entranceTile, WorldTile entranceClickTile, WorldTile clipRoot) {
