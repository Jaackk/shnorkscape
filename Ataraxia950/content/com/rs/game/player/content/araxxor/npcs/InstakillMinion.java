...
final class InstakillMinion extends NPC {
	...
	InstakillMinion(int id, WorldTile tile) {
		...
		//Logger.getGlobal().info(Logger.DEBUG, "Instakill minion created");
	...
	void findIntelligentRoute() {
		int steps = RouteFinder.findRoute(RouteFinder.WALK_ROUTEFINDER, getX(), getY(), getPlane(), getSize(), new FixedTileStrategy(killTarget.getX(), killTarget.getY()), true);
	...
	void detonate() {
		...
		killTarget.applyHit(new Hit(this, Integer.MAX_VALUE, Hit.HitLook.INSTANT_KILL_TYPE));
