...
/**
 * Abstract base class for Araxyte NPCs. Araxxor and Araxxi share a lot of the same functionality, namely special attacks. <br/>
 * <br/>
...
public abstract class Araxyte extends NPC {
	...
	transient AraxxorFight fight;

	/**
	 * Constructs a new Araxyte NPC, binding the NPC to the fight instance.
	 * @param id    the NPC id, corresponding to any of the Araxxor forms, or the Araxxi NPC id
	...
	/**
	 * Takes information from a base {@code Hit} object and scales the output damage according to this NPC's difficulty, as well as the enrage mechanic.
	 * The scaled hit damage for a group can be represented as, {@code ds = (gm + ep) * db}, where {@code ds} is the scaled damage, {@code ep} is the
	 * enrage percentage modifier, {@code db} is the initial base hit damage, and {@code gm} is the group modifier.
	...
	/**
	 * Perform the parent {@code processNPC()}, but adjust the difficulty if the group size changes.
	 */
	@Override
	...
	/**
	 * Perform the parent {@code processHit(Hit hit)}, but also reflect damage back to attackers if inside the healing web.
	 * @param hit the {@code Hit} to process
	 */
	@Override
	public void processHit(Hit hit) {
		if (!(this instanceof Araxxi)) {
			if (hit.getDamage() > 5000)
				hit.setDamage(5000);
		}
		if (inHealingWeb) {
			final int damage = hit.getDamage();
			fight.getPlayers().forEach(player -> {
				if (player != null && !player.isDead() && !player.hasFinished() && hit.getLook() != Hit.HitLook.HEALED_DAMAGE)
					player.applyHit(new Hit(this, Math.round((float) (damage * 0.50)), Hit.HitLook.REFLECTED_DAMAGE));
			});
		}
	...
	@Override
	public void setNextGraphics(Graphics graphics) {
		if (!inHealingWeb) {
			super.setNextGraphics(graphics);
		}
	}
	...
	/**
	 * Performs the minion spawn attack.<br/>
	 * <br/>
	 *
	 * Araxxor will spawn a batch of spider minions (size 2-6 in solo, size 5-10 in group). The type of minion is completely random, but there are 5 types
	 * of minions that can spawn.<br/>
	 * <br/>
	 *
	 * Bladed, Spitting, and Imbued minions each simply autoattack the players in the fight with melee, ranged, and magic attacks respectively.<br/>
	 * <br/>
	 *
	 * Pulsing minions will primarily focus on significantly healing Araxxor every three game ticks, but will attack with magic and stop healing this
	 * Araxyte if they are attacked.<br/>
	 * <br/>
	 *
	 * Mirrorback minions do not fight back. When they take damage, 50% of the damage they take is reflected to this Araxyte. However, if Mirrorback
	 * minions are spawned, 100% of all damage dealt directly to this Araxyte will be reflected back to the attacking player.<br/>
	 * <br/>
	 */
	void performMinionSpawn() {
		final int batchSize = Utils.random(difficulty == Difficulty.SOLO ? 2 : 5, difficulty == Difficulty.SOLO ? 6 : 10);
		AraxyteMinion.MinionType type = AraxyteMinion.MinionType.values()[Utils.random(AraxyteMinion.MinionType.values().length)];
		...
		for (int i = 0; i < batchSize; i++) {
			...
			while (true) {
				...
				if (minionSpawnTile.getTileHash() != getTileHash() && World.canMoveNPC(minionSpawnTile.getPlane(), minionSpawnTile.getX(), minionSpawnTile.getY(), 1))
					break;
			}
			AraxyteMinion minion = minionFactory.spawnMinion(type.npcId, minionSpawnTile, this);
			minion.performRole();
		}
		fight.getPlayers().forEach(p -> p.getPackets().sendPlayerMessageBox(Colors.RED + "A batch of minions appears!"));
	...
	/**
	 * Performs the Cleave special attack.<br/>
	 * <br/>
	 * Players are sweeped into melee range of Araxxor, after which he performs a massive melee blow to all players inside a two square radius. This blow
	 * is potentially lethal depending on the difficulty and enrage level.<br/>
	 * <br/>
	...
	void performCleave() {
		...
		fight.getPlayers().stream().filter(player -> player != null && !player.isDead() && !player.hasFinished()).forEach(player -> {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (tick == 0) {
					...
					fight.getPlayers().forEach(player -> {
						if (player != null && !player.isDead() && !player.hasFinished() && player.withinDistance(dragTo, 2))
	...
	/**
	 * Performs the Cocoon special attack.<br/>
	 * <br/>
	 * Players are frozen in place, trapped in a tight spider web. They will suffer 200 damage per game tick for as long as they are in the cocoon. The
	 * only way to escpae is to rapidly click away from their current location. Even then, they are not guaranteed to escape immediately. The maximum
	 * duration is 5 game ticks, but players can escape after 2 game ticks if they are lucky.
	 */
	void performCocoon() {
				...
				if (tick == 0)
					...
					fight.getPlayers().forEach(player -> {
						if (player != null && !player.isDead() && !player.hasFinished()) {
							...
							player.getPackets().sendPlayerMessageBox("You manage to " + "break free from the thick" + "webbing!");
						}
					});
	...
	/**
	 * Performs the web heal special attack.<br/>
	 * <br/>
	 * Araxxor retreats into a web, covering himself and healing for 1000 (2000 in group difficulty) every 2 game ticks. Furthermore, 50% of the damage
	 * inflicted on Araxxor will be reflected back at the attackers while in the web.
	 */
	void performWebHeal() {
		setTarget(null);
		setCannotMove(true);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (tick % 2 == 0 && tick != 6 && Utils.random(2) == 1)
					applyHit(new Hit(null, difficulty == Difficulty.SOLO ? 200 : 400, Hit.HitLook.HEALED_DAMAGE));
				else if (tick == 6) {
					inHealingWeb = false;
					setCannotMove(false);
					Araxyte.this.setTarget(fight.getPlayers().get(Utils.random(fight.getPlayers().size())));
					stop();
				}
				tick++;
			}
	...
	/**
	 * Represents the difficulty of this {@code Araxyte} NPC. The NPC will by default deal 30% more damage to groups than it does to single players. This
	 * does not include enrage effects.
	 */
	...
	/**
	 * Small enum containing the special attack definitions, each enumeration containing the attack delay for that special attack.
	 */
	enum Specials {
		COCOON(10), CLEAVE(20), SPAWN_MINIONS(0), ACID_SPIDER(3), WEB_HEAL(35);
