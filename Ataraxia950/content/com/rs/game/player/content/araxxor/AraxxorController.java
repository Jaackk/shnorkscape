...
public class AraxxorController extends Controller {
	...
	public boolean sendDeath() {
		...
		if (player.isGroupIronman()) {
			player.gimTracker.incrementDeaths("<#player> died while fighting Araxxor");
		}
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int slotId2, int packetId) {
		if (fight.getPhase() == 5) {
			return AraxxorInterfaces.handleRewardsInterface(player, fight, interfaceId, componentId, slotId, slotId2, packetId);
		} else {
	...
	public boolean processCommand(String s, boolean b, boolean c) {
		if (player.isDev() || player.isOwner() || player.isFakeDev() || player.getRights() >= 2 || s.toLowerCase().contains("staffmenu"))
			return true;
		player.sendMessage(Colors.RED + "Commands cannot be used whilst inside the Araxyte cave.");
		return false;
	}

	@Override
	public boolean processPlayerOption1(Entity target) {
		if (player.isDev() || player.isOwner() || player.isFakeDev() || player.getRights() >= 2)
			return true;
		if (player.getEquipment().getWeaponId() == 25202 && fight.getPhase() < 4) {
			player.sendMessage("It is too early in the fight and Araxxor is immune to your " + ItemDefinitions.getItemDefinitions(player.getEquipment().getWeaponId()).getName().toLowerCase() + "for now.");
			return false;
		}
	...
	public boolean processObjectClick2(WorldObject object) {
		if (fight.getPhase() == 5) {
			player.sendOptionsDialogue("Leave the Araxyte cave?", new String[] { "Yes", "No", }, new DialogueOptionEvent() {
				@Override
	...
	private boolean processPostFightObjectClick(WorldObject object) {
		if (object.getId() == 91673) {
			if (fight.getRewarded() == player)
				AraxxorInterfaces.sendRewardsInterface(player, fight.getReward(player));
			else
				player.sendMessage("The loot belongs to " + fight.getRewarded().getDisplayName() + ".");
			return false;
	...
	private void phaseProcess(int nextX, int nextY) {
		if (fight.getPhase() < 2 && fight.getConfig().getActivePath() == AraxxorEnvironment.Paths.LIGHT && nextX > fight.getConfig().relativeX(4515) && nextY < fight.getConfig().relativeY(6262))
			fight.nextPhase();
		else if (fight.getPhase() < 2 && fight.getConfig().getActivePath() == AraxxorEnvironment.Paths.MINIONS && player.getY() > fight.getConfig().relativeY(6270) && player.getX() > fight.getConfig().relativeX(4510))
			fight.nextPhase();
		else if (fight.getPhase() < 2 && fight.getConfig().getActivePath() == AraxxorEnvironment.Paths.ACID && player.getY() > fight.getConfig().relativeY(6259) && player.getY() < fight.getConfig().relativeY(6271) && player.getX() > fight.getConfig().relativeX(4519))
			fight.nextPhase();
		else if (fight.getPhase() < 3 && fight.getConfig().getActivePath() == AraxxorEnvironment.Paths.MINIONS && player.getX() > fight.getConfig().relativeX(4567))
			fight.nextPhase();
	}
