...
/**
 * An interface utility class responsible for sending an updating player interfaces during the fight.
 *
 * <br/>
 * <br/>
...
public final class AraxxorInterfaces {
	...
	private static final int ACID_INTERFACE = 15;
	private static final int RAMP_HEALTH_CID = 10;
	private static final int RAMP_HEALTH_DISPLAY_CID = 17;
	private static final int ABSORBED_ACID_CID = 13;
	private static final int ABSORBED_ACID_DISPLAY_CID = 18;
	private static final int UNUSED = 16;

	private static final int ABSORBED_CONFIG_FILE = 651;
	private static final int GLOBAL_HEALTH_CONFIG = 816;
	private static final int HEALTH_CONFIG_FILE = 817;
...
//			player.getInterfaceManager().sendSecondaryOverlay(945);
			...
			player.getPackets().sendGlobalString(315, araxyte instanceof Araxxor ? "Araxxor's Health" : "Araxxi's Health");
			player.getPackets().sendGlobalConfig(1233, (int) (((double) araxyte.getHitpoints() / araxyte.getMaxHitpoints()) * 212) - 12);
		});
	}
	...
	public static void sendAcidInterface(List<Player> players) {
		players.forEach(player -> {
			...
			player.getPackets().sendIComponentText(ACID_INTERFACE, RAMP_HEALTH_DISPLAY_CID, "Ramp Health: " + 100 + "%");
			player.getPackets().sendIComponentText(ACID_INTERFACE, ABSORBED_ACID_DISPLAY_CID, "Acid Absorbed: " + 0 + "%");
			player.getPackets().sendGlobalConfig(GLOBAL_HEALTH_CONFIG, 2);
		});
	}
	...
	public static void updateAcidInterface(List<Player> players, int rampHealth, int acidAbsorbed) {
		if (rampHealth == 0 || acidAbsorbed == 0) {
			players.forEach(player -> player.getInterfaceManager().closeOverlay(false));
			return;
		}
		...
		players.forEach(player -> {
			...
			player.getPackets().sendIComponentText(ACID_INTERFACE, ABSORBED_ACID_CID, "Absorbed");
			player.getPackets().sendIComponentText(ACID_INTERFACE, UNUSED, "");
			player.getPackets().sendIComponentText(ACID_INTERFACE, RAMP_HEALTH_DISPLAY_CID, "Ramp Health: " + rampHealth + "%");
			player.getPackets().sendIComponentText(ACID_INTERFACE, ABSORBED_ACID_DISPLAY_CID, "Acid Absorbed: " + acidAbsorbed + "%");
			player.getPackets().sendGlobalConfig(GLOBAL_HEALTH_CONFIG, 2);
...
//		}, 2);
	...
	/**
	 * Updates the lighting overlay interface with the specified brightness (config).
...
//			player.getInterfaceManager().sendSecondaryOverlay(1140);
	...
	static void sendRewardsInterface(Player player, AraxxorReward reward) {
		...
		player.getPackets().sendItems(99, reward.getItems());
		reward.notifyWorld(player.getDisplayName());
		reward.notifyPetDrop(player);
	}

	static boolean handleRewardsInterface(Player player, AraxxorFight fight, int interfaceId, int componentId, int slotId, int slotId2, int packetId) {
		if (interfaceId == 1284) {
			...
			case 8:
				...
				Arrays.stream(reward.getItems()).filter(i -> i != null).forEach(i -> player.getBank().addItem(i, true));
				reward.discardItems();
				reward.setLooted(true);
			...
			case 10:
				...
				Arrays.stream(reward.getItems()).filter(i -> i != null).forEach(i -> {
					boolean freeUnstackable = !i.getDefinitions().isStackable() && player.getInventory().getFreeSlots() >= i.getAmount();
					boolean stackableExists = i.getDefinitions().isStackable() && player.getInventory().containsItem(i);
					boolean stackableFree = i.getDefinitions().isStackable() && player.getInventory().hasFreeSlots();
				...
				if (outOfSpace.get())
					player.sendMessage("You don't have enough free " + "space in your inventory to hold any more items..");
				reward.shift();
				player.getPackets().sendItems(99, reward.getItems());
				return true;
			case 7:
				...
				if (packetId == PacketRepository.ACTION_BUTTON1_PACKET) {
					...
					player.sendMessage(ItemExaminesDataParser.getExamine(i));
					player.sendMessage("Grand Exchange guide price:" + Utils.getFormattedNumber(GrandExchange.getPrice(i)) + ".");
				}
				break;
			default:
				return true;
