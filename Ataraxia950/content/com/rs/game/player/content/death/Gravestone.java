...
public class Gravestone extends NPC {
	...
	private static final long serialVersionUID = -2468255495099987328L;
	...
	private final WorldTile deathTile;
	...
	public static final void sendBuySelection(final Player player, final int option) {
		if (player.getGravestone() == option) {
		...
		player.getInventory().removeItemMoneyPouch(new Item(995, PRICES[option]));
		player.setGravestone(option);
	...
	public Gravestone(final Player player, final WorldTile deathTile, final Item[] items) {
		super(getNPCId(player.getGravestone()), deathTile, -1, true, true);
		...
		for (final Item item : items) {
			...
			final FloorItem i = World.addGroundItem(item, deathTile, player, true, -1, 0, false);
			if (i != null) {
				floorItems.add(i);
			}
		}
	...
	public static final void login(final Player player) {
		final Gravestone stone = getGraveStoneByUsername(player.getUsername());
		if (stone == null) {
			return;
		}
	...
	public final void decrementGrave(final int stage, final String message) {
		final Player player = getPlayer();
		if (player != null) {
	...
	public final void sendGraveInscription(final Player reader) {
		reader.getInterfaceManager().sendInterface(266);
		reader.getPackets().sendConfigByFile(4191, gravestone == 0 ? 0 : 1);
		if (ticks <= 50) {
			reader.getPackets().sendText(266, 22, "The inscription is too unclear to read.");
		} else if (reader == getPlayer()) {
			reader.getPackets().sendText(266, 22, "It looks like it'll survive another " + ((ticks / 100) + 1) + " minutes. Isn't there something a bit odd about reading your own gravestone?");
		} else {
			reader.getPackets().sendText(266, 22, inscription);
		}
	}

	public final void repair(final Player blesser, final boolean bless) {
	...
	public final void demolish(final Player demolisher) {
	...
	public final void addLeftTime(final boolean clean) {
		if (clean) {
	...
	public static final int getMaximumTicks(final int gravestone) {
		switch (gravestone) {
		case 0:
			return 175;
		case 1:
	...
	public static final Gravestone getGraveStoneByUsername(final String username) {
	...
	public static final int getNPCId(final int gravestone) {
		if (gravestone == 13) {
			return 13296;
		}
	...
	private static final String getInscription(String username, final int gravestone) {
