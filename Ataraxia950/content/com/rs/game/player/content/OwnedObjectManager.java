...
public class OwnedObjectManager {
	...
	public static Player getOwner(WorldObject object) {
		Iterator<Entry<String, OwnedObjectManager>> it = ownedObjects.entrySet().iterator();
	    while (it.hasNext()) {
	    	Map.Entry<String, OwnedObjectManager> pair = it.next();
			OwnedObjectManager v = pair.getValue();
			if (v == null)
				continue;
			if (v.getObjects() == null)
				continue;
			for (int i = 0; i < v.getObjects().length; i++) {
				if (v.getObjects()[i] == null)
					continue;
				if (v.getObjects()[i].getX() == object.getX() && v.getObjects()[i].getY() == object.getY()
						&& v.getObjects()[i].getPlane() == object.getPlane()
						&& v.getObjects()[i].getId() == object.getId())
					return v.getOwner();
			}
	    }
		return null;
	...
	public static boolean removeEntry(Player player, WorldObject object) {
		for (Iterator<String> it = player.getOwnedObjectManagerKeys().iterator(); it.hasNext();) {
			final OwnedObjectManager manager = ownedObjects.get(it.next());
			if (manager == null) {
				it.remove();
	...
	public static int getObjectsforValues(Player player, Collection<Integer> objectIds) {
		int count = 0;
		for (Iterator<String> it = player.getOwnedObjectManagerKeys().iterator(); it.hasNext();) {
			OwnedObjectManager manager = ownedObjects.get(it.next());
			if (manager == null) {
			...
			if (objectIds.contains(manager.getCurrentObject().getId()))
				count++;
		}
		return count;
	}
