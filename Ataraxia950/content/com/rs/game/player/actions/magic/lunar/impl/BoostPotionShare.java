...
public class BoostPotionShare implements ItemSpell {
	...
	public Item[] getRunes() {
		return new Item[] { new Item(ASTRAL_RUNE, 3), new Item(EARTH_RUNE, 12), new Item(WATER_RUNE, 10) };
	}
		...
		for (int regionId : player.getMapRegionsIds()) {
			...
			for (int playerIndex : playerIndexes) {
				...
				if (affectedPeopleCount >= doses2)
					break;
				Pots.shareEffect(player, p2, item, true);
				affectedPeopleCount++;
			}
		}
		Pots.shareEffect(player, item, player.getInventory().getItems().getThisItemSlot(item), affectedPeopleCount, true);
		return true;
	}
