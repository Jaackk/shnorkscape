 ...
 */
public class ChatIconPacker {

    public static void main(String[] args) throws IOException {
        Cache.init();

        IndexedColorImageFile iconsFile = new IndexedColorImageFile(Cache.STORE, 1455, 0);
        BufferedImage icon = ImageIO.read(new File("crowns/one.png"));
        iconsFile.addImage(icon);
        Logger.getGlobal().info(iconsFile.getImages().length);
        boolean result = Cache.STORE.getIndexes()[8].putFile(1455, 0, 2, iconsFile.encodeFile(), null, false, false, -1, -1);
        Logger.getGlobal().info("Added! " + result);
    }
...
class IndexedColorImageFile {
    ...
    public int getPalleteIndex(int rgb) {
        ...
        if (pallete.length == 256) {
            Logger.getGlobal().info("Pallete to big, please reduce images quality.");
            return 0;
        }
