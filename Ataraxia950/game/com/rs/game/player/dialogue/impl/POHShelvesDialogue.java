...
public class POHShelvesDialogue extends Dialogue {

	private static final int[] LARDER_ITEMS = { 7688, 7702, 1980, 1919, 1923, 2313, 1931, 1949 };
	private static final String[] LARDER_NAMES = { "Kettle", "Teapot", "Tea Cup", "Beer Glass", "Bowl", "Pie dish",
			"Empty pot", "Chef's Hat" };
	...
	public void run(int interfaceId, int componentId) {
		if (stage == 0 || stage == -1) {
			if (componentId != OPTION_5) {
				addItem(getOrdinal(componentId), stage == 0);
			} else {
				if (componentId == OPTION_5) {
