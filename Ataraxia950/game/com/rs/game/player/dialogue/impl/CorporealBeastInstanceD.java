...
import com.rs.game.activities.instances.CorporealBeastInstance;
...
public class CorporealBeastInstanceD extends Dialogue {
	...
	public void start() {
		...
		sendOptionsDialogue("What would you like to do?",
			...
			if (player.withinDistance(new WorldTile(2970, 4384, 2), 10))
			sendOptionsDialogue("What would you like to do?",
					"Start an instance",
					"Join an instance",
					"Enter my instance");
	...
	public void run(int interfaceId, int componentId) {
		...
		case 11:
			...
			case OPTION_5:
				player.sendInputInteger("How many players will you limit the instance to?", new InputIntegerEvent() {
					@Override
					public void run(Player player) {
						player.getDialogueManager().startDialogue("CorporealBeastInstanceD", bossId, speed, getInteger());
					}
				});
				end();
				break;
		...
		case 12:
			if (componentId == OPTION_1) {
				player.sendInputInteger("Select a password in digits.", new InputIntegerEvent() {
					@Override
					public void run(Player player) {
						player.getDialogueManager().startDialogue("CorporealBeastInstanceD", bossId, speed, limit, getInteger());
					}
				});
				end();
				return;
		...
		case 13:
			...
			sendOptionsDialogue("The instance will cost " + (player.getPerkManager().hasPerkActive(DonationPerk.THE_DISCOUNTER) ? 25000 : 37500) * duration + " coins to build.<br>Are you sure you wish to create this instance?",
					"Yes, create the instance.",
					"No, I've changed my mind.");
			break;
		case 14:
			if (componentId == OPTION_1) {
				if (player.getMoneyPouch().removeAmount((player.getPerkManager().hasPerkActive(DonationPerk.THE_DISCOUNTER) ? 25000 : 37500) * duration)) {
					instance = new CorporealBeastInstance(player, duration, speed, limit, password, bossId, false);
