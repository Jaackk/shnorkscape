...
public class PotteryFurnace extends Dialogue {
	...
	public void start() {
		SkillsDialogue.sendSkillsDialogue(player, SkillsDialogue.MAKE,
				"How many would you like to make?<br>Choose a number, then click the pot to begin.", 28, PRODUCTS,
				new ItemNameFilter() {
					int count = 0;
	...
	public void run(int interfaceId, int componentId) {
		...
		if (componentIndex > INGREDIENTS.length || componentIndex < 0) {
			end();
			return;
		}
			...
			public boolean start(Player player) {
				...
				if (ingredientAmount == 0) {
					...
					player.getPackets().sendGameMessage("You need a "
							+ ItemDefinitions.getItemDefinitions(INGREDIENTS[componentIndex]).getName().toLowerCase()
				...
				if (player.getSkills().getLevel(Skills.CRAFTING) < levelReq) {
					...
					player.getPackets()
							.sendGameMessage("You need a crafting level of " + levelReq + " in order to spin this.");
					return false;
				}
