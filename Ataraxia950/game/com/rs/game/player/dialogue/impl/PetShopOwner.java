...
public class PetShopOwner extends Dialogue {
	...
	private static final int[] DOGS = new int[] { 12522, 12518, 12514, 12512, 12520, 12516 };
	private int npcId;
	...
	public static void sellShards(Player player) {
		...
		player.sendInputInteger("How many will you sell? (25 each, you have " + Utils.formatNumber(shardsCount) + ")", new InputIntegerEvent() {
			@Override
			public void run(Player player) {
				sellShards(player, getInteger());
			}
		});
	}
	...
	public static void sellShards(Player player, int quantity) {
		...
		if ((player.getInventory().getAmountOf(995) + (quantity * 25l)) >= Integer.MAX_VALUE)
			quantity = (Integer.MAX_VALUE - player.getInventory().getAmountOf(995)) / 25;
		if (quantity <= 0)
			return;
		if (!player.getInventory().containsItem(995, 1) && !player.getInventory().hasFreeSlots()) {
			player.sendMessage("You need some free space to do this.");
			return;
		}
		player.getInventory().deleteItem(new Item(12183, quantity));
		final int money = quantity * 25;
		player.getInventory().addItem(new Item(995, money));
		player.getPackets().sendGameMessage("You sell " + Utils.getFormattedNumber(quantity) + " spirit shards for " + Utils.getFormattedNumber(money) + " coins.");
	}

	public static void openShop(Player player) {
		ShopsDataParser.openShop(player, 20);
	}

	@Override
	public void start() {
		this.npcId = (Integer) parameters[0];
		sendOptionsDialogue(SEND_DEFAULT_OPTIONS_TITLE, "Can I see your shop, please?",
				"How much is that puppy in the window?", "So, what sort of pets are available?",
	...
	public void run(int interfaceId, int componentId) {
		...
		case -1:
			...
			case OPTION_3:
				stage = 1;
				sendPlayerDialogue(NORMAL, "So, what short of pets are available?");
				break;
			default:
			case OPTION_4:
				stage = 20;
		...
		case 7:
			stage = 8;
			sendNPCDialogue(npcId, NORMAL,
					"After that, you just need to feed it ground fishing bait until it's old enough to eat it solid.");
			break;
		case 8:
			stage = 9;
		...
		case 14:
			stage = 15;
			sendNPCDialogue(npcId, NORMAL, "Monkeys are a different story, however!");
			break;
		case 15:
			stage = 16;
		...
		case 17:
			stage = 18;
			sendNPCDialogue(npcId, NORMAL, "They will hang on tight, and never let go, even when the trap closes!");
			break;
		case 18:
			stage = 19;
			sendPlayerDialogue(NORMAL, "Thanks a lot, you've been very helpful!");
			break;
		case 19:
			stage = -2;
			sendNPCDialogue(npcId, NORMAL,
		...
		case 23:
			stage = 24;
			this.sendPetDialogue(668, 668, "668");
			break;
		case 24:
			if (componentId >= 1 && componentId <= 6) {
				selectedDog = DOGS[componentId - 1];
		...
		case 28:
			stage = 29;
			sendNPCDialogue(npcId, NORMAL, "Dogs are a big responsability and you should be cared for.");
			break;
		case 29:
			stage = 30;
			sendNPCDialogue(npcId, NORMAL,
					"If a person is unwilling to invest 500 coins, then they don't deserve to have the puppy in the first place.");
			break;
		case 30:
			stage = 31;
		...
		case 31:
			stage = 32;
			sendOptionsDialogue(SEND_DEFAULT_OPTIONS_TITLE, "Okay, I'll take the " + getPetName() + ".", "No thanks.");
			break;
		case 32:
			switch (componentId) {
			case OPTION_1:
				stage = 33;
				sendPlayerDialogue(NORMAL, "Okay, I'll take the " + getPetName() + ".");
				break;
			case OPTION_2:
			default:
				stage = -2;
		...
		case 33:
			...
			stage = -2;
			sendNPCDialogue(npcId, NORMAL, "There you go! I hope you two get on.");
	...
	private String getPetName() {
		return ItemDefinitions.getItemDefinitions(selectedDog).getName().toLowerCase().replace(" puppy", "");
	}
