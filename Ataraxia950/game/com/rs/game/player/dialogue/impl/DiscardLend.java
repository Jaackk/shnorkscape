...
public class DiscardLend extends Dialogue {
	...
	public void start() {
		...
		if (minutes < 0) {
			...
			player.getPackets().sendIComponentText(1189, 4,
					"<col=00007f>~ Loan expires in " + (hours > 0 ? hours + " hour" + (hours > 1 ? "s " : "") : "") + ""
							+ (minutes > 0 ? minutes + " minute" + (minutes > 1 ? "s" : "") : "") + " ~</col><br>"
							+ "If you discard this item, it will disappear.<br>"
							+ "You won't be able to pick it up again.");
		}
	}
	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			if (componentId == 10) {
				sendOptionsDialogue("Really discard item?", "Yes, discard it. I won't need it again.",
