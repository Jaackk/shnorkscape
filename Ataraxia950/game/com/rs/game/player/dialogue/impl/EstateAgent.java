...
public class EstateAgent extends Dialogue {
	...
	private static final POHLocation[] LOCATIONS = { POHLocation.EDGEVILLE, POHLocation.RIMMINGTON, POHLocation.TAVERLY,
			POHLocation.POLLNIVNEACH, POHLocation.RELLEKKE, POHLocation.BRIMHAVEN, POHLocation.YANILLE,
	...
	public void run(int interfaceId, int componentId) {
		...
		if (stage == -1) {
			sendOptionsDialogue("SELECT AN OPTION", "Can you move my house please?",
					"Can you redecorate my house please?", "Could I have a Construction guidebook?",
			...
			sendOptionsDialogue("SELECT AN OPTION", "Edgeville (2'500)", "Rimmington (5,000)", "Taverly (5,000)",
					"Pollnivneach (7,500)", "More");
			stage = 3;
		} else if (stage == 3) {
			if (componentId >= OPTION_1 && componentId <= OPTION_4) {
				moveHouse(LOCATIONS[getOrdinal(componentId)]);
			} else if (componentId == OPTION_5) {
				sendOptionsDialogue("SELECT AN OPTION", "Relleka (10,000)", "Brimhaven (15,000)", "Yannile (25,000)",
						"Prifddinas (50,000)", "Previous");
				stage = 4;
			}
		} else if (stage == 4) {
			...
			if (componentId >= OPTION_1 && componentId <= OPTION_4) {
				redecorateHouse(getOrdinal(componentId));
			} else if (componentId == OPTION_5) {
			...
			sendNPCDialogue(npcId, NORMAL, "There are various other people across " + Settings.SERVER_NAME
					+ " who can help you furnish your house. You should start by buying planks from the construction shop.");
			stage = 30;
		} else if (stage == 14) {
			...
			sendNPCDialogue(npcId, NORMAL,
					"As you may know, skillcapes are only available to masters in a skill. I have spent my entire life building houses and now I spend my time selling them! As a sign of my abilities I wear this Skillcape of Construction. If you ever have");
			stage = 16;
		} else if (stage == 16) {
			sendNPCDialogue(npcId, NORMAL,
			...
			boolean hasEnoughCoins = player.hasMoney(99000);
			if (hasEnoughCoins) {
