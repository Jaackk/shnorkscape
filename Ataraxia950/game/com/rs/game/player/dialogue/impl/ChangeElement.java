...
public class ChangeElement extends Dialogue {
	...
	public static boolean isStaff(int item) {
		return item == AIR || item == WATER || item == EARTH || item == FIRE;
	}

	public static boolean hasStaff(Player player) {
		return player.getInventory().containsItem(AIR, 1) || player.getInventory().containsItem(WATER, 1)
	...
	public void start() {
		...
		sendDialogue("Select an Element", "Air", "Water", "Fire", "Earth");
		stage = -1;
	}
		...
		if (stage == -1) {
			if (componentId == OPTION_1) {
				if (player.getInventory().containsItem(556, 100) && hasStaff(player)) {
					...
					player.sendMessage("You successfully change your staff into an air staff!");
				} else {
					player.getInterfaceManager().closeChatBoxInterface();
					player.sendMessage(
							"You must have 100 of the element's runes and an elemental staff in your inventory.");
				}
				...
				if (player.getInventory().containsItem(555, 100) && hasStaff(player)) {
					...
					player.sendMessage("You successfully change your staff into a water staff!");
				} else {
					player.getInterfaceManager().closeChatBoxInterface();
					player.sendMessage(
							"You must have 100 of the element's runes and an elemental staff in your inventory.");
				...
				if (player.getInventory().containsItem(554, 100) && hasStaff(player)) {
					...
					player.sendMessage("You successfully change your staff into a fire staff!");
				} else {
					player.getInterfaceManager().closeChatBoxInterface();
					player.sendMessage(
							"You must have 100 of the element's runes and an elemental staff in your inventory.");
				...
				if (player.getInventory().containsItem(557, 100) && hasStaff(player)) {
					...
					player.sendMessage("You successfully change your staff into an earth staff!");
				} else {
					player.getInterfaceManager().closeChatBoxInterface();
					player.sendMessage(
							"You must have 100 of the element's runes and an elemental staff in your inventory.");
