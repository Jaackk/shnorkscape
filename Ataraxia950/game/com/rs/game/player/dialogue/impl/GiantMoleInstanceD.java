...
public class GiantMoleInstanceD extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		...
		case 11:
			...
			case OPTION_5:
				player.sendInputInteger("How many players will you limit the instance to?", new InputIntegerEvent() {
					@Override
					public void run(Player player) {
						player.getDialogueManager().startDialogue("GiantMoleInstanceD", bossId, speed, getInteger());
					}
				});
				end();
				break;
		...
		case 12:
			if (componentId == OPTION_1) {
				player.sendInputInteger("Select a password in digits.", new InputIntegerEvent() {
					@Override
					public void run(Player player) {
						player.getDialogueManager().startDialogue("GiantMoleInstanceD", bossId, speed, limit, getInteger());
					}
				});
				end();
				return;
		...
		case 13:
			...
			sendOptionsDialogue("Which mode would you like to enter?",
					"Normal mode",
					"<col=ff0000>Hard mode");
			break;
		case 14:
			...
			sendOptionsDialogue("The instance will cost " + 25000 * duration * (hardMode ? 2 : 1) + " coins to build.<br>Are you sure you wish to create this instance?",
					"Yes, create the instance.",
					"No, I've changed my mind.");
			break;
		case 15:
			if (componentId == OPTION_1) {
				if (player.getMoneyPouch().removeAmount(250 * duration * (hardMode ? 2 : 1))) {
					instance = new GiantMoleInstance(player, duration, speed, limit, password, bossId, hardMode);
