...
public class HelwyrsChosen extends Dialogue {

	private static final int NPC = 22435;
	private int option;
	...
	public void start() {
		...
		if (option == 2) {
			...
			sendNPCDialogue(NPC, NORMAL, "Adventurer.");
	}
	...
	public void run(int interfaceId, int componentId) {
		...
		case 1:
			sendNPCDialogue(NPC, NORMAL, "You find yourself in the Heart of Gielinor, adventurer. "
					+ "This place is a brutal battlefield, but we elves must face it if we are "
					+ "to fight back the hordes of Zamorak, Zaros and Sliske in the name of Seren!");
			break;
		case 2:
			sendNPCDialogue(NPC, NORMAL, "Our leader is the ancient Cywir hunter Helwyr. With his unrivalled "
					+ "shapeshifting power at the helm we cannot fail.");
			break;
		case 3:
		...
		case 4:
			sendNPCDialogue(NPC, NORMAL, "We are not all savages. Upon this platform a truce "
					+ "has been established, allowing us - the envoys of each faction - "
					+ "to use words rather than violence to sway fresh recruits. For below "
					+ "the battle rages on,");
			break;
		case 5:
			sendNPCDialogue(NPC, NORMAL, "so we must do what we can to persuade the "
					+ "mercenaries that join the fray to fight for our cause. Take a look "
					+ "at what I have to offer. I assure you, I'll make it worth your while "
					+ "to fight for Seren.");
			stage = -1;
			return;
		case 10:
			HeartOfGielinor.sendReputationInterface(player, HeartOfGielinor.SEREN);
			end();
			return;
		case 20:
			sendNPCDialogue(NPC, NORMAL, "There are a few ways you can improve your standing. "
					...
					+ "Ingression fragments to call in reinforcements for us. "
		...
		case 21:
			sendNPCDialogue(NPC, NORMAL, "Speak to him if you'd like to know more. "
					...
					+ "other factions' generals that would certainly earn you a lot of "
					+ "favour.");
			stage = -1;
			return;
		case 30:
