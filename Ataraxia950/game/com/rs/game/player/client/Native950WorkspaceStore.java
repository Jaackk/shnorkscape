...
final class Native950WorkspaceStore {
    ...
    byte[] encode(Record r) throws IOException {
        ...
        out.writeUTF(Native950LayoutFixture.SCHEMA_SHA);out.writeUTF(IMAGE);out.writeUTF(r.account);
    ...
    Record decode(byte[] bytes,String account) throws IOException {
        ...
        if(in.readInt()!=MAGIC||in.readInt()!=VERSION||in.readInt()!=950
                ||!in.readUTF().equals(Native950LayoutFixture.SCHEMA_SHA)||!in.readUTF().equals(IMAGE)
                ||!in.readUTF().equals(Native950Save.canonicalUsername(account)))throw new IOException("Workspace identity/schema mismatch");
        ...
        try {Record r=new Record(account,revision,values);Native950LayoutFixture.workspacePlan(schema,r.snapshot());return r;}
