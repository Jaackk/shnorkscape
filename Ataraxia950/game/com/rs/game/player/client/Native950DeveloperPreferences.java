...
final class Native950DeveloperPreferences {
    ...
    static Set<String> load(String account)throws IOException{
        ...
        for(String id:lines.subList(1,lines.size()))if(Native950DeveloperActions.find(id)!=null&&result.size()<128)result.add(id);
        return result;
    }
    static void save(String account,Set<String> favourites)throws IOException{
        ...
        for(String id:favourites){if(Native950DeveloperActions.find(id)==null)throw new IOException("Unknown favourite");data.append(id).append('\n');}
