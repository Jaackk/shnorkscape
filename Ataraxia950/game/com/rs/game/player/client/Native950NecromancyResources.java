...
final class Native950NecromancyResources {
    ...
    private static final class State { int necrosis,souls,scythe; long lastCombat,scytheUntil,clock,publishedSoulTimer=Long.MIN_VALUE; boolean engaged; }
    ...
    void gainSoul(Player player,long tick){State s=states.computeIfAbsent(player,p->new State());s.souls=Math.max(s.souls,Math.min(soulCap(player),s.souls+1));s.lastCombat=tick;s.clock=tick;s.engaged=true;publish(player,s);}
    ...
    void cast(Player player,int struct,long tick,boolean livingDeath){
        ...
        s.lastCombat=tick;s.clock=tick;s.engaged=true;
        publish(player,s);
    }
    void basicAttack(Player player,long tick,boolean livingDeath){
        if(!livingDeath)return;
        State s=states.computeIfAbsent(player,p->new State());
        s.necrosis=Math.min(12,s.necrosis+2);s.lastCombat=tick;s.clock=tick;s.engaged=true;publish(player,s); // CS18671.
    }
    ...
    private void publishSoulTimer(Player p,State s){
        long end=s.souls==0?Long.MIN_VALUE:s.engaged?0:s.lastCombat+10;
        if(end==s.publishedSoulTimer)return;
        s.publishedSoulTimer=end;
        if(p.getRealChannel()==null||s.souls==0)return;
        // CS4252/11073 use varc7246; CS10886 renders no duration when its end is0.
        if(s.engaged)p.getRealChannel().write(com.rs.network.protocol.modern950.Native950Packets.varc(7246,0));
        else p.getRealChannel().write(com.rs.network.protocol.modern950.Native950Packets.runClientScript(4252,48334,(int)Math.max(0,end-s.clock)));
    }
