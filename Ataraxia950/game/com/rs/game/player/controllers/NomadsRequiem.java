...
public class NomadsRequiem extends Controller {
	...
	public void enterDoor(final int doorIndex) {
		...
		if (currentPart == DungeonPart.THRONE) {
			startThroneScene();
				...
				&& player.getQuestManager().getQuestStage(Quests.NOMADS_REQUIEM) == 0) {
			sendFirstScene();
		} else {
	...
	public boolean sendDeath() {
		if (currentPart == DungeonPart.THRONE) {
			...
			final List<Integer> indexes = World.getRegion(player.getRegionId()).getNPCsIndexes();
		...
		if (player.isGroupIronman()) {
			player.gimTracker.incrementDeaths("<#player> died while doing Nomad's Requiem");
		}
		player.lock(7);
		player.stopAll();
		WorldTasksManager.schedule(new WorldTask() {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (stage == 0) {
					 ...
					 */
					final WorldTile walk = new WorldTile(getBaseX() + 35, getBaseY() + 11, 0);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (stage == 1) {
					...
					final WorldTile throne = new WorldTile(getBaseX() + 16, getBaseY() + 20, 0);
					final Nomad nomad = (Nomad) World.spawnNPC(8528, throne, -1, true, true);
	...
	public void destroyPart(final int[] mapBaseChunks, final DungeonPart part) {
		if (mapBaseChunks == null || part == null) {
			return;
		}
	...
	public enum DungeonPart {
		...
		DungeonPart(final int chunkX, final int chunkY, final int sizeX, final int sizeY, final int... doorPositions) {
			this.chunkX = chunkX;
			this.chunkY = chunkY;
			this.sizeX = sizeX;
			this.sizeY = sizeY;
