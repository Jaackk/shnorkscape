...
public class HouseController extends Controller {
    ...
    public boolean processNPCClick1(NPC npc) {
        /**
         * if (npc instanceof ServantNPC) { npc.faceEntity(player); if
         * (!house.isOwner(player)) {
         * player.getDialogueManager().startDialogue("SimpleNPCMessage", npc.getId(),
         * "Sorry, I only serve my master."); return false; }
         * player.getDialogueManager().startDialogue("NewServantD", npc, false); return
         * false; }
         */ // TODO
        return true;
    }
    ...
    public boolean processNPCClick2(NPC npc) {
         ...
         * (!house.isOwner(player)) {
         * player.getDialogueManager().startDialogue("SimpleNPCMessage", npc.getId(),
         * "The servant ignores your request."); return false; }
         * player.getDialogueManager().startDialogue("NewServantD", npc, true); return
         * false; }
         */ // TODO
        return true;
    }
    ...
    public boolean processItemOnNPC(NPC npc, Item item) {
         ...
         * item.getId(), house.getServant().isSawmill()); return false; }
         */ // TODO
        return false;
    }
    ...
    public boolean processObjectClick1(WorldObject object) {
        ...
        if (object.getId() == HouseConstants.HObject.EXIT_PORTAL.getId())
            ...
            if (!house.isOwner(player)) {
                player.sendMessage("I'd better not do this...");
                return false;
            }
            house.callServant(true);
            return false;
        } else if (object.getId() == 13523) {
            // TODO ItemTransportation.transportationDialogue(player, new
            // Item(1712, 1), false);
            return false;
            ...
            if (object.getId() == 13214) {
                ...
                // player.getPackets().sendMusicEffectOld(90); //TODO music
                // effects
                player.setNextAnimation(new Animation(3674));
            }
            ...
            player.sendMessage("You use your tinderbox to light the torches.");
            player.lock(2);
            ...
            TabletMaking.openTabInterface(player, object, object.getId() - 13642);
            ...
            if (object.getDefinitions().getOption(1).equals("Climb")) {
                player.getDialogueManager().startDialogue("ClimbHouseStairsD", object, house);
            } else
            ...
            if (!house.isOwner(player)) {
                ...
                player.sendMessage("Your house must be in build mode in order to change your portal's focus.");
                return false;
            }
    ...
    private boolean canEnterRing(byte ringType) {
        if (ringType == 1 || ringType == 2) {
            for (int slot : RESTRICTED_SLOTS) {
                ...
                if (item != null) {
                    player.sendMessage("You need to remove your " + item.getName() + " before entering the ring.");
                    return false;
                }
            }
            if (ringType == 1) {
                ...
                if (weapon == 7671 || weapon == 7673)
                    return true;
                player.sendMessage("You need to have boxing gloves equipped in order to box.");
                return false;
            }
        }
        return true;
     ...
     * @return
     */
    private boolean canAttack(boolean mainHand) {
        if (ringType == 1 || ringType == 2) {
            if (player.getEquipment().getWeaponId() == 22496 || player.getCombatDefinitions().getSpellId(!mainHand) > 0) {
                player.sendMessage("You may only use melee in this ring!");
                return false;
            }
    ...
    public boolean canEquip(int slot, int item) {
        if (ringType == 1) {
            player.sendMessage("You may only use boxing gloves in a boxing ring.");
            return false;
        } else if (ringType == 2) {
            if (slot != 3) {
                player.sendMessage("You may only equip weapons in a fencing ring.");
                return false;
            }
        }
        return true;
    ...
    /**
     * @Override public boolean canRemoveEquip(int slot, int itemId) { if (ringType
     *           == 1) { if (slot == Equipment.SLOT_WEAPON) {
     *           player.sendMessage("You can't remove your boxing gloves."); return
     *           false; } } return true; }
     */
        ...
        if (interfaceId == 182) {
            ...
            if (componentId == 6 || componentId == 13) {
                if (!player.hasFinished()) {
                    player.sendMessage(Colors.RED + "CAUTION: Please leave your house for security meassures.");
                    return false;
                }
                return false;
            }
    ...
    public boolean handleItemOnObject(WorldObject object, Item item) {
        if (object.getId() == HouseConstants.HObject.CLAY_FIREPLACE.getId() || object.getId() == HouseConstants.HObject.STONE_FIREPLACE.getId() || object.getId() == HouseConstants.HObject.MARBLE_FIREPLACE.getId()) {
            ...
            if (house.isBuildMode()) {
                player.sendMessage("You cannot place items on the table while in building mode.");
                return false;
            }
            player.faceObject(object);
            player.sendMessage("You place the " + item.getName() + " on the table.");
            ...
            player.sendMessage("You fill the glass with " + object.getDefinitions().name + ".");
            return false;
        } else if (HouseConstants.Builds.ALTAR.containsObject(object)) {
            ...
            if (bone != null) {
                ...
                // player.getActionManager().setAction(new BonesOnAltar(object,
                // bone, house.getBurnerCount()));
                // TODO replace bones on altar
                return false;
            }
        }
        return true;
    ...
    public boolean processObjectClick2(final WorldObject object) {
        if (object.getId() == HouseConstants.HObject.EXIT_PORTAL.getId())
            ...
            if (!house.isChallengeMode()) {
                player.sendMessage("The doors are only locked during challenge mode.");
                return false;
            }
            ...
            WorldTasksManager.schedule(new WorldTask() {
                ...
                public void run() {
                    if (Utils.random(player.getSkills().getLevel(Skills.THIEVING)) < Utils.random(HouseConstants.DUNGEON_DOOR_LEVELS[slot])) {
                        player.sendMessage("and fail to pick the lock.");
                        this.stop();
                        return;
                    }
                    player.sendMessage("and successfully pick the lock.");
                    passDoor(object);
                }
            }, 1);
            return false;
    ...
    public boolean processObjectClick3(final WorldObject object) {
        ...
        if (object.getId() == HouseConstants.HObject.EXIT_PORTAL.getId()) {
            ...
            player.sendMessage("You attempt to force the lock...");
            final int slot = HouseConstants.Builds.DUNGEON_DOOR1.getHObjectSlot(object.getId());
            WorldTasksManager.schedule(new WorldTask() {
                ...
                public void run() {
                    if (Utils.random(player.getSkills().getLevel(Skills.STRENGTH)) < Utils.random(HouseConstants.DUNGEON_DOOR_LEVELS[slot])) {
                        player.sendMessage("You fail to force the lock.");
                        this.stop();
                        return;
                    }
                    passDoor(object);
