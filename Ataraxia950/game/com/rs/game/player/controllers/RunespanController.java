...
import com.rs.game.player.dialogue.impl.RuneSpanLeaving;
...
public class RunespanController extends Controller {
	...
	private static final WorldTile WIZARDS_TOWER = new WorldTile(3107, 3164, 2);
	private static final int AIR_RUNE = RunespanData.AIR_RUNE;
	private static final int EARTH_RUNE = RunespanData.EARTH_RUNE;
	private static final int WATER_RUNE = RunespanData.WATER_RUNE;
	private static final int MIND_RUNE = RunespanData.MIND_RUNE;
	private static final int FIRE_RUNE = RunespanData.FIRE_RUNE;
	private static final int ELEMENTAL_RUNE = -1;
	private static final int BODY_RUNE = RunespanData.BODY_RUNE;
	private static final int RUNE_ESSENCE = RunespanData.RUNE_ESSENCE;
	private static final int CHAOS_RUNE = RunespanData.CHAOS_RUNE;
	private static final int NATURE_RUNE = RunespanData.NATURE_RUNE;
	private static final int COSMIC_RUNE = RunespanData.COSMIC_RUNE;
	private static final int ASTRAL_RUNE = RunespanData.ASTRAL_RUNE;
	private static final int LAW_RUNE = RunespanData.LAW_RUNE;
	private static final int BLOOD_RUNE = RunespanData.BLOOD_RUNE;
	private static final int DEATH_RUNE = RunespanData.DEATH_RUNE;
	private long startTime;
	private boolean starterRunesClaimed;
	private boolean exiting;
	private YellowWizard yellowWizard;

	/**
	 * Handles entering to Runespan.
	 *
	 * @param player
	 *            The player entering.
	 */
	public static void enterRunespan(final Player player) {
		enterRunespan(player, LOWER_LEVEL);
	}

	public static void enterMiddleRunespan(final Player player) {
		if (player.getSkills().getLevel(Skills.RUNECRAFTING) < 50) {
			player.getPackets().sendGameMessage("You need a Runecrafting level of 50 to use the mid-level Runespan portal.");
			return;
		}
		enterRunespan(player, HIGHER_LEVEL);
	}

	private static void enterRunespan(final Player player, final WorldTile destination) {
		if (player.isLocked())
			return;
		if (player.getControlerManager().getControler() != null)
			player.getControlerManager().forceStop();
		removeRunes(player);
		player.useStairs(-1, destination, 0, 2);
		WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run() {
				player.getControlerManager().startControler("RunespanController");
			}
		}, 2);
	}
	...
	private static void removeRunes(Player player) {
		if (player.getFamiliar() != null && player.getFamiliar().getBob() != null) {
			for (Item item : player.getFamiliar().getBob().getBeastItems().getItems()) {
				if (item == null || !RunespanData.isRunespanRuneOrEssence(item.getId()))
					continue;
	...
	private boolean handleCrossingPlataform(final WorldObject object, final Platforms plataform) {
		...
		boolean freeTravel = player.getUsername().equalsIgnoreCase("Noel") || RuneCrafting.hasEtherealOutfit(player);
		if (largeIsland)
			player.getPackets().sendGameMessage("You weren't charged for traveling to a larger island.");
		else if (freeTravel)
			player.getPackets().sendGameMessage("You cross without consuming runes.");
		else {
			for (int index = 0; index < plataform.runes.length; index++) {
				int runeId = plataform.runes[index];
				if (runeId == ELEMENTAL_RUNE) {
					if (player.getInventory().containsOneItem(AIR_RUNE))
						runes[index] = AIR_RUNE;
					else if (player.getInventory().containsOneItem(WATER_RUNE))
						runes[index] = WATER_RUNE;
					else if (player.getInventory().containsOneItem(EARTH_RUNE))
						runes[index] = EARTH_RUNE;
					else if (player.getInventory().containsOneItem(FIRE_RUNE))
						runes[index] = FIRE_RUNE;
					else {
			...
			player.getPackets().sendGameMessage(builder + " have been removed from your inventory.");
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (stage == 0) {
					...
					player.setNextForceMovement(new NewForceMovement(player, 1, toTile, 5, Utils.getAngle(toTile.getX() - object.getX(), toTile.getY() - object.getY())));
				} else if (stage == 1) {
	...
	public void start() {
		...
		player.getPackets().sendGlobalConfig(1918, 0);
		startTime = Utils.currentTimeMillis();
		player.getDialogueManager().startDialogue(
				"SimpleMessage",
				"Welcome To the Runespan ",
				Colors.RED + "WARNING</col>: When you leave all the runes you make here will be removed and",
	...
	public void exitRunespan() {
		if (exiting)
			return;
		exiting = true;
		int points = getRunespanTotalInventoryPoints();
		if (points > 0) {
			player.addRunespanPoints(points);
			player.getPackets().sendGameMessage("The Runecrafting Guild converts your runes into " + points + " Runespan points.");
		}
		removeRunes(player);
		if (yellowWizard != null)
			yellowWizard.finish();
	...
	public void leaveRunespan() {
		exitRunespan();
		player.useStairs(-1, WIZARDS_TOWER, 0, 2);
	}
	...
	public void handleWizzard(Player player) {
		startTime = Utils.currentTimeMillis();
		yellowWizard = new YellowWizard(new WorldTile(RANDOM_LOCATIONS[Utils.random(RANDOM_LOCATIONS.length)]), this, RunespanData.getRandomRuneId());
		player.getPackets().sendGameMessage("<col=ff0000>A wizard needs your help somewhere in the Runespan.");
		if (!player.getHintIconsManager().isEmpty())
			player.getHintIconsManager().removeUnsavedHintIcon();
	}
	...
	public void sendInterfaces() {
		...
		player.getPackets().sendGlobalConfig(1909, player.getRuneSpanPoints());
		refreshInventoryPoints();
	}

	public int getRunespanTotalInventoryPoints() {
		return RunespanData.getInventoryPointValue(player);
	}
	...
	public void refreshInventoryPoints() {
		player.getPackets().sendGlobalConfig(1916, getRuneSpanPoints());
	}
	...
	public boolean processCommand(String s, boolean b, boolean c) {
		...
		return s.equalsIgnoreCase("home");
	}
	...
	public boolean processNPCClick1(NPC npc) {
		...
		if (id == 15402 || name.contains("floating essence")) {
			collectFloatingEssence();
			return false;
		}
		if (id == 15418 || name.contains("wizard finix")) {
			player.getDialogueManager().startDialogue("WizardFinix", 0, npc);
			return false;
		}
		if (id == 15430 && yellowWizard != null) {
			yellowWizard.tellRequest(player);
			return false;
		}
		if (id == 15432 || id == 15433 || id == 15434 || id == 15435 || name.contains("wizard student")) {
			showYellowWizardHint();
			return false;
		}
	...
	public boolean processNPCClick2(NPC npc) {
		if (npc.getId() == 15402 || npc.getDefinitions().getName().toLowerCase().contains("floating essence")) {
			collectFloatingEssence();
			return false;
		}
		if (npc.getId() == 15418 || npc.getDefinitions().getName().toLowerCase().contains("wizard finix")) {
			player.getDialogueManager().startDialogue(RuneSpanLeaving.class.getSimpleName());
			return false;
		}
		if (npc.getId() == 15430 && yellowWizard != null) {
			yellowWizard.tellRequest(player);
			return false;
		}
	...
	public boolean processItemOnNPC(NPC npc, Item item) {
		if (npc.getId() == 15430 && yellowWizard != null && RunespanData.isRunespanRune(item.getId())) {
			yellowWizard.giveReward(player, item.getId());
			refreshInventoryPoints();
			return false;
		}
		return true;
	...
	private void collectFloatingEssence() {
		int have = player.getInventory().getAmountOf(RUNE_ESSENCE);
		if (have >= 25) {
			player.getPackets().sendGameMessage("You already have enough essence from the floating essence. Get more from an essence creature.");
			return;
		}
		int toAdd = 25 - have;
		if (!ItemDefinitions.getItemDefinitions(RUNE_ESSENCE).isStackable()) {
			int free = player.getInventory().getFreeSlots();
			if (free <= 0) {
				player.getPackets().sendGameMessage("You need a free inventory slot.");
				return;
			}
			toAdd = Math.min(toAdd, free);
		}
		player.setNextAnimation(new Animation(12832));
		player.getInventory().addItem(RUNE_ESSENCE, toAdd);
		player.getPackets().sendGameMessage("You collect " + toAdd + " rune essence.");
	}

	public void showYellowWizardHint() {
		if (yellowWizard == null) {
			player.getPackets().sendGameMessage("No wizard is calling for help right now.");
			return;
		}
		if (!player.getHintIconsManager().isEmpty())
			player.getHintIconsManager().removeUnsavedHintIcon();
		player.getHintIconsManager().addHintIcon(yellowWizard.getX(), yellowWizard.getY(), yellowWizard.getPlane(), 150, 2, 0, -1, true);
		player.getPackets().sendGameMessage("An arrow now points to the wizard who needs help.");
	}
	...
	public boolean processObjectClick1(final WorldObject object) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (stage == 0) {
							...
							player.setNextForceMovement(new NewForceMovement(player, 1, dest, 35, Utils.getAngle(dest.getX() - object.getX(), dest.getY() - object.getY())));
		...
		if (object.getId() == 70508) {
			if (player.getSkills().getLevel(com.rs.game.player.Skills.RUNECRAFTING) < 33) {
				player.getPackets().sendGameMessage("You need a Runecrafting level of 33 to reach the middle level of the Runespan.");
				return false;
			}
			...
			if (player.getSkills().getLevel(com.rs.game.player.Skills.RUNECRAFTING) < 66) {
				player.getPackets().sendGameMessage("You need a Runecrafting level of 66 to reach the upper level of the Runespan.");
				return false;
			}
	...
	public void exchangeStarterRunes() {
		if (starterRunesClaimed) {
			player.getPackets().sendGameMessage("Wizard Finix has already supplied you with runes for this visit.");
			return;
		}
		int level = player.getSkills().getLevel(com.rs.game.player.Skills.RUNECRAFTING);
		int[] runes = RunespanData.getAvailableRuneIds(level);
		if (runes.length == 0)
			return;
		int cost = RunespanData.getStarterRuneCost(level);
		if (player.getRuneSpanPoints() < cost) {
			player.getPackets().sendGameMessage("You need " + cost + " Runespan points to claim those runes.");
			return;
		}
		int freeSlotsNeeded = 0;
		for (int runeId : runes) {
			if (!player.getInventory().containsOneItem(runeId))
				freeSlotsNeeded++;
		}
		if (player.getInventory().getFreeSlots() < freeSlotsNeeded) {
			player.getPackets().sendGameMessage("You need more inventory space to claim those runes.");
			return;
		}
		player.setRuneSpanPoint(player.getRuneSpanPoints() - cost);
		for (int runeId : runes)
			player.getInventory().addItem(runeId, 10);
		starterRunesClaimed = true;
		player.getPackets().sendGlobalConfig(1909, player.getRuneSpanPoints());
		refreshInventoryPoints();
		player.getPackets().sendGameMessage("Wizard Finix exchanges " + cost + " points for 10 of each rune you can siphon.");
	}
	...
	private enum HandledPlatforms {
		...
		private final WorldTile smallIsland;
		private final WorldTile largeIsland;

		HandledPlatforms(int largeIslandX, int largeIslandY, int smallIslandX, int smallIslandY) {
	...
	private enum Platforms {
		...
		Platforms(int objectId, int startEmote, int startGraphic, int middleEmote, int middleGraphic, int endEmote, int endGraphic, boolean invisible, int... runes) {
