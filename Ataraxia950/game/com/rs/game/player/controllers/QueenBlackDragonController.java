...
public final class QueenBlackDragonController extends Controller {
    ...
    public boolean processButtonClick(final int interfaceId, final int componentId, final int slotId, final int slotId2, final int packetId) {
        ...
        switch (interfaceId) {
        case 506:
            player.getPackets().sendGameMessage("You cannot teleport away from Queen black dragon.");
            break;
        case 1284:
            switch (componentId) {
            case 8:
                if (npc.getRewards().getItems().length == 0) {
            ...
            case 7:
                ...
                case PacketRepository.ACTION_BUTTON2_PACKET:
                    player.getBank().addItems(new Item[] { npc.getRewards().toArray()[slotId] }, true);
                    npc.getRewards().toArray()[slotId] = null;
                    break;
    ...
    public boolean processObjectClick1(final WorldObject object) {
        ...
        if (object.getId() == 70813) {
            player.setNextWorldTile(new WorldTile(1194, 6498, 0));
            end(0);
            return false;
        }
        ...
        CoresManager.getServiceProvider().executeNow(() -> {
            try {
                ...
                FadingScreen.unfade(player, time, () -> {
                    ...
                    player.getInterfaceManager().sendOverlay(1285, false);
                    player.getPackets().sendIComponentText(1285, 32, "2,500");
