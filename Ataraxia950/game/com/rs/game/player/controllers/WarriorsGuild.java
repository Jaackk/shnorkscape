...
public class WarriorsGuild extends Controller {
    ...
    /**
     * Projectile source only (World.sendProjectile below); it is never interacted
     * with. The five-argument constructor registers it in World.getNPCs() as a side
     * effect of class initialisation, which the legacy server has always done and
     * which is left exactly as it was. On the flat 947 cache the same class
     * initialisation happens inside Native950Bootstrap.run(), where a stray
     * world NPC breaks the single-slot NPC_INFO invariant that
     * World.addNative950Npc enforces, so the native JVM builds the same NPC
     * without registering it (toSpawn = false). See notes/P5-router.md.
     */
    private static final NPC CATAPULT_PROJECTILE_BASE = new NPC(1957, new WorldTile(2842, 3550, 1), -1, false, false,
            !com.rs.cache.Cache.isFlatReadOnly());
    ...
    public static void init() {
        WorldTasksManager.schedule(WarriorsGuild.timer = new WarriorTimer(), 0, 0);
    }
    ...
    public boolean processPlayerOption1(Entity target) {
        if (target instanceof AnimatedArmor) {
            ...
            return player == npc.getCombat().getTarget();
        }
        return true;
    }
    ...
    public boolean processObjectClick1(WorldObject object) {
        if (object.getId() >= 15624 && object.getId() <= 15630) {
            ...
            player.setNextWorldTile(new WorldTile(object.getX(), inLobby ? object.getY() + (object.getId() == 15647 ? 1 : -1) : object.getY(), player.getPlane()));
            return false;
            ...
            if (object.getId() == 15653)
                ...
                resetKegBalance();
            player.setNextWorldTile(new WorldTile(player.getX() == object.getX() ? object.getX() + (object.getId() == 66758 ? -1 : 1) : object.getX(), object.getY(), player.getPlane()));
            return false;
            ...
            if (player.getX() >= 2847) {
                ...
                if (!withinArea) {
                    player.setNextWorldTile(new WorldTile(withinArea ? object.getX() + 1 : object.getX(), object.getY(), player.getPlane()));
                    inCyclopse = false;
                }
            } else {
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    player.setNextWorldTile(new WorldTile(player.getX() == object1.getX() ? object1.getX() + 1 : object1.getX(), object1.getY(), player.getPlane()));
            ...
            WorldTasksManager.schedule(new WorldTask() {
                ...
                public void run() {
                    ...
                    if (ticks == 0)
                        ...
                        player.getDialogueManager().startDialogue("SimpleMessage", "You get pushed away..");
    ...
    private void submitDummyHit(final WorldObject object) {
        player.setNextAnimation(new Animation(PlayerCombat.getAttackAnimation(player, true)));
        WorldTasksManager.schedule(new WorldTask() {
    ...
    private boolean isProperHit(WorldObject object) {
        ...
        List<Integer> xpGains = player.getCombatDefinitions().getSelectedXpGains();
        if (object.getId() == 15624)
            return xpGains.contains(Skills.ATTACK);
        else if (object.getId() == 15625)
            return attackStyle == Combat.SLASH_STYLE;
        else if (object.getId() == 15626)
            return xpGains.contains(Skills.STRENGTH);
        else if (object.getId() == 15627)
            return xpGains.contains(Skills.ATTACK) && xpGains.contains(Skills.STRENGTH) && xpGains.contains(Skills.DEFENCE);
        else if (object.getId() == 15628)
            return attackStyle == Combat.CRUSH_STYLE;
        else if (object.getId() == 15629)
            return attackStyle == Combat.STAB_STYLE;
        else if (object.getId() == 15630)
            return xpGains.contains(Skills.DEFENCE);
        return false;
    }
...
//                player.getPackets().sendConfigByFile(8668, 0);
    ...
    public boolean canEquip(int slot, int itemId) {
        if (itemId == 8856) {
            if (!inCatapultArea(player))
                ...
                sendShieldInterfaces();
        } else return slot != Equipment.SLOT_HAT || kegCount < 1;
        return true;
    }
    ...
    private void sendShieldInterfaces() {
        player.getPackets().sendConfigByFile(15391, defensiveStyle);
        player.getInterfaceManager().replaceMeleeAbilitesTab(411);
        player.getInterfaceManager().sendLockGameTab(InterfaceManager.INVENTORY_TAB, true);
        player.getInterfaceManager().sendLockGameTab(InterfaceManager.SKILLS_TAB, true);
        player.getInterfaceManager().sendLockGameTab(InterfaceManager.PRAYER_BOOK_TAB, true);
        player.getInterfaceManager().sendLockGameTab(InterfaceManager.MAGIC_BOOK_TAB, true);
        player.getInterfaceManager().openGameTab(InterfaceManager.MELEE_ABILITIES_TAB);
    }
    ...
    private void closeShieldInterfaces() {
        ...
        player.getInterfaceManager().sendLockGameTab(InterfaceManager.SKILLS_TAB, false);
        player.getInterfaceManager().sendLockGameTab(InterfaceManager.PRAYER_BOOK_TAB, false);
        player.getInterfaceManager().sendLockGameTab(InterfaceManager.MAGIC_BOOK_TAB, false);
    }
    ...
    private void balanceKeg(final WorldObject object) {
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                kegCount++;
                player.getPackets().sendConfigByFile(object.getDefinitions().configFileId, 1);
    ...
    public static class WarriorTimer extends WorldTask {
        ...
        /**
         * The projectile placeholder in order to properly check the "Catapult
         * Minigame".
         */
