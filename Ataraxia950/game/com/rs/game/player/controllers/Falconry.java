...
import com.rs.game.player.actions.hunter.FlyingEntityHunter.DynamicFormula;
...
public class Falconry extends Controller {
	...
	public static void beginFalconry(Player player) {
		int weapId = player.getEquipment().getWeaponId();
	...
	public boolean processButtonClick(int interfaceId, int componentId, int slotId, int slotId2, int packetId) {
		if (interfaceId == 387 || interfaceId == 667) {
			if (componentId == 9 || componentId == 15) {
				player.sendMessage("You cannot unequip your Falconeers glove.");
				return false;
			}
		}
		return true;
	...
	public void forceClose() {
		player.getEquipment().getItems().remove(new Item(10024));
		player.getEquipment().refresh(3);
	...
	public boolean processNPCClick1(final NPC npc) {
		...
		if (npc.getDefinitions().getName().toLowerCase().contains("kebbit")) {
							...
							WorldTasksManager.schedule(new WorldTask() {
								...
								public void run() {
									...
									player.getPackets().sendGameMessage(
											"The falcon successfully swoops down and captures the kebbit.", true);
			...
			player.getPackets().sendGameMessage("You retreive the falcon as well as the fur of the dead kebbit;"
					+ " creatures caught: " + Colors.RED + player.getCreaturesCaught() + "</col>.", true);
