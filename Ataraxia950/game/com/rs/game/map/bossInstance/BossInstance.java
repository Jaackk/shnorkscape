...
public abstract class BossInstance {
    ...
    public void enterInstance(Player player, boolean login) {
        synchronized (BossInstanceHandler.LOCK) {
            ...
            players.add(player);
            playMusic(player);
            if (!isPublic() && getBoss() != Boss.Death && getBoss() != Boss.Husband_Mich)
                player.getPackets().sendGameMessage("Welcome to this session against <col=00FFFF>" + getInstanceName() + "</col>. This arena will expire in " + (settings.getTimeRemaining() / 60000) + " minutes.");
            if (!login)
    ...
    public String getInstanceName() {
        return settings.getBoss().name().replace("_", " ") + (getSettings().isHardMode() ? " (Hard Mode)" : "");
    }
    ...
    public void leaveInstance(Player player, int type) {
        synchronized (BossInstanceHandler.LOCK) {
            ...
            if (players.isEmpty() && getBoss() != Boss.Evil_Tree) // public version
                finish();
        }
    }
