...
public class JsonObject<K, V> extends LinkedHashMap<K, V> {
	...
	void moveCharsToMate() {
		...
		if (items == null) {
			...
			throw new IllegalStateException("char[] should only have one String in the [], found " + items.length
					+ ", line " + line + ", col " + col);
		}
	}
		...
		if (containsKey("@keys")) {
			...
			if (value instanceof Object[]) {
				...
				throw new IllegalStateException(
						"JsonObject with @keys, but no array [] associated to it, line " + line + ", col " + col);
			}
			...
			if (value instanceof Object[]) {
				...
				throw new IllegalStateException(
						"JsonObject with @items, but no array [] associated to it, line " + line + ", col " + col);
			}
