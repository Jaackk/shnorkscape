...
import com.rs.cache.filestore.util.bzip2.BZip2Compressor;
import com.rs.cache.filestore.util.bzip2.BZip2Decompressor;
import com.rs.cache.filestore.util.crc32.CRC32HGenerator;
import com.rs.cache.filestore.util.gzip.GZipCompressor;
import com.rs.cache.filestore.util.gzip.GZipDecompressor;
...
public class Archive {
	...
	private void decompress(byte[] archive) {
		...
		length = compressedLength + (compression == 0 ? 5 : 9) + (id != 255 ? 2 : 0);
		switch (compression) {
		case Constants.NO_COMPRESSION: // no compression
		...
		case Constants.LZMA_COMPRESSION:
			...
			stream.readBytes(compresseddata, 0, compressedLength);

			byte[] temp = new byte[compresseddata.length + 8];
			System.arraycopy(compresseddata, 0, temp, 0, 5);
			temp[5] = (byte) (length >>> 0);
			temp[6] = (byte) (length >>> 8);
			temp[7] = (byte) (length >>> 16);
			temp[8] = (byte) (length >>> 24);
			temp[9] = temp[10] = temp[11] = temp[12] = 0;
			System.arraycopy(compresseddata, 5, temp, 13, compresseddata.length - 5);
			checkRevision(compressedLength, archive, stream.getOffset());
			data = new byte[length];
			LZMAInputStream streams;
			try {
				streams = new LZMAInputStream(new ByteArrayInputStream(temp));
				streams.read(data);
			} catch (IOException e) {
				e.printStackTrace();
			}
