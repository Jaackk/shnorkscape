...
public final class MainFile {
    ...
    MainFile(int id, FlatCacheRepository repository) {
        this.id = id;
    ...
    public Archive getArchive(int id, int[] keys) {
        if (isReadOnly()) {
            rejectEncryption(keys);
            try {
                byte[] decoded = flatRepository.readDecodedContainer(this.id, id);
                return decoded == null ? null : new Archive(id, 0, -1, decoded);
    ...
    public byte[] getCachedArchiveData(int id) {
        if (isReadOnly()) return getArchiveData(id);
        byte data[];
    ...
    public byte[] getArchiveData(int archiveId) {
        if (isReadOnly()) {
            try { return flatRepository.readContainer(id, archiveId); }
            catch (IOException e) { throw new UncheckedIOException(e); }
        }
        synchronized (data) {
            try {
        ...
        synchronized (data) {
            try {
                ...
                while (remaining > 0) {
                    ...
                    data.write(tempBuffer, ((long) block * (long) TOTAL_BLOCK_LEN));
                    remaining -= blockSize;
                    block = nextBlock;
                    chunk++;
                }
    ...
    public int getArchivesCount() throws IOException {
        if (isReadOnly()) {
            if (id == 255) return flatRepository.getIndexes().keySet().stream().mapToInt(Integer::intValue).max().orElse(-1) + 1;
            FlatCacheRepository.Index index = flatRepository.getIndexes().get(id);
            return index == null ? 0 : index.getGroups().keySet().stream().mapToInt(Integer::intValue).max().orElse(-1) + 1;
        }
        synchronized (index) {
    ...
    static void rejectEncryption(int[] keys) {
        if (keys != null) for (int key : keys) if (key != 0)
            throw new UnsupportedOperationException("Encrypted groups require an explicit modern cache decryptor");
    }
}
