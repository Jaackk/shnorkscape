...
public class GraphicDefinitions {
    ...
    public static void main(String... sss) {
        try {
          ...
          // 5799 blue shield like telos
          // 5893 telos shield (green)
        GraphicDefinitions defs = getGraphicDefinitions(3601);
        System.out.println(defs.modelId + ", " + defs.emoteId + ", client cycles=" + AnimationDefinitions.getAnimationDefinitions(defs.emoteId).getEmoteClientCycles());
        int[] emoteIds = { 1788, 1789, 1790, 1791, 1792 };
        for (int i=1788-20;i<= 1788 +20;i++) {
            int gfxId = getGfxByAnimation(i);
            if (gfxId != -1) {
                defs = getGraphicDefinitions(gfxId);
                int time = AnimationDefinitions.getAnimationDefinitions(defs.emoteId).getEmoteTime();
                System.out.println("gfx=" + gfxId + ", emoteId=" + i + ", time=" + time);
            }
        }
        int abilityId = 11;
        int type = ActionBar.MAGIC_ABILITY_SHORTCUT;
        GeneralRequirementMap data = GeneralRequirementMap.getMap(ClientScriptMap.getMap(ActionBar.CS_DATA_ID[type - 1]).getIntValue(abilityId == 200 ? 2 : abilityId));
        int currentWeapon = -1;

        // 2919 - mainhand
        // 2914 - offhand

        boolean twoHWep = ItemDefinitions.getItemDefinitions(currentWeapon).getEquipType() == 5;
        ...
        System.out.println("emoteId=" + emoteId + ", projId=" + projectileId);
        System.out.println(getGraphicByModel(49709));
...
//            continue;
    ...
    public static int getGfxByAnimation1(int id) {
        for (int i = 0; i < Utils.getGraphicDefinitionsSize(); i++) {
            GraphicDefinitions defs = getGraphicDefinitions(i);
            if (defs == null || defs.emoteId != id)
                continue;
    ...
    public static final void main2(String... s) {
        ...
        int offset2 = 50;
        for (int i = 0; i < Utils.getGraphicDefinitionsSize(); i++) {
            GraphicDefinitions def = GraphicDefinitions.getGraphicDefinitions(i);
            if (def == null)
                continue;
...
//			System.out.println("opcode="+opcode);
     ...
     * if ((opcode ^ 0xffffffff) == -3) emoteId = stream.readBigSmart(); else if
     * (opcode == 4) anInt1446 = stream.readUnsignedShort(); else if (opcode != 5) {
     * if ((opcode ^ 0xffffffff) != -7) { if (opcode == 7) anInt1440 =
     * stream.readUnsignedByte(); else if ((opcode ^ 0xffffffff) == -9) anInt1451 =
     * stream.readUnsignedByte(); else if (opcode != 9) { if (opcode != 10) { if
     * (opcode == 11) { // added opcode // aBoolean1442 = true; byteValue = (byte)
     * 1; } else if (opcode == 12) { // added opcode // aBoolean1442 = true;
     * byteValue = (byte) 4; } else if (opcode == 13) { // added opcode //
     * aBoolean1442 = true; byteValue = (byte) 5; } else if (opcode == 14) { //
     * added opcode // aBoolean1442 = true; // aByte2856 = 2; byteValue = (byte) 2;
     * intValue = stream.readUnsignedByte() * 256; } else if (opcode == 15) { //
     * aByte2856 = 3; byteValue = (byte) 3; intValue = stream.readUnsignedShort(); }
     * else if (opcode == 16) { // aByte2856 = 3; byteValue = (byte) 3; intValue =
     * stream.readInt(); } else if (opcode == 44) { //rs3 new
     * stream.readUnsignedShort(); //count } else if (opcode == 45) { //rs3 new
     * stream.readUnsignedShort(); //count } else if (opcode == 46) { //rs3 new
     * 
     * } else if (opcode != 40) { if ((opcode ^ 0xffffffff) == -42) { int i =
     * stream.readUnsignedByte(); aShortArray1455 = new short[i]; aShortArray1435 =
     * new short[i]; for (int i_0_ = 0; i > i_0_; i_0_++) { aShortArray1455[i_0_] =
     * (short) (stream.readUnsignedShort()); aShortArray1435[i_0_] = (short)
     * (stream.readUnsignedShort()); } } } else { int i = stream.readUnsignedByte();
     * aShortArray1438 = new short[i]; aShortArray1456 = new short[i]; for (int i_1_
     * = 0; ((i ^ 0xffffffff) < (i_1_ ^ 0xffffffff)); i_1_++) {
     * aShortArray1438[i_1_] = (short) stream.readUnsignedShort();
     * aShortArray1456[i_1_] = (short) stream.readUnsignedShort(); } } } else
     * aBoolean1448 = true; } else { // aBoolean1442 = true; byteValue = (byte) 3;
     * intValue = 8224; } } else anInt1454 = stream.readUnsignedShort(); } else
     * anInt1449 = stream.readUnsignedShort(); } else modelId =
     * stream.readBigSmart(); }
     */
