...
/** ISAAC stream for native947. Derived from the existing Graham Edgecombe implementation, verified against OpenNXT. Unlike the legacy development cipher, this stream is always enabled. Thread confinement is the caller's responsibility. */
public final class Native950Isaac implements java.util.function.IntSupplier {
	/** * The golden ratio. */
	...
	public Native950Isaac(int[] seed) {
        java.util.Objects.requireNonNull(seed, "seed");
        if (seed.length > SIZE) throw new IllegalArgumentException("ISAAC seed exceeds 256 words");
    ...
    /** Consumes one word, in the same reverse-block order as the native client. */
    @Override
    public int getAsInt() {
        if (count-- == 0) {
            isaac();
