...
public final class OutputStream extends Stream {
	...
	public void writeString(String s) {
		...
		offset += StringUtilities.encodeString(buffer, getOffset(), s, 0, s.length());
		writeByte(0);
	}
    ...
    public void write24BitIntegerV1(int i) {
        writeByte(i);
        writeByte(i >> 8);
        writeByte(i >> 16);
    }
	...
	public void writePacket(Player player, int id) {
		...
		if (player == null)
			writeSmart(id);
		else if (player.isBot() || player.getIsaacKeyPair() == null) {
			if (id >= 128) {
				writeByte((id >> 8) + 128);
				writeByte(id);
			} else {
