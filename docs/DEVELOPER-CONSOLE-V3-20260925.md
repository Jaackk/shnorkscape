...
Physical follow-up on runtime51eb4c3: fresh login/open succeeded; Home icons and compact header visible; first Boss row blue-selected without a click; deliberate Supreme over-zoom clipped at the preview edges; the long Shadow hand-wrap title stayed inside the inspector; Angel of Death search auto-selected Nex; Player occupied approximately230px rather than the old180px. Man distance650 and Supreme1350 were physically calibrated to approximately160-170px. The final refinement moves the crop below the heading and applies the common small/medium fallback distances while preserving native Beasts metadata and the accepted large Vorago distance. This remains PARTIAL for universal per-mesh size equality: no verified rendered-model-bounds API or complete NXT mesh-bounds decoder is established. Zoom/Reset remain available.

Final runtime e7db9bf was installed and physically reopened after fresh login. Supreme default now fills the preview (~170px); deliberate extreme zoom stayed below its heading and above the controls. Reset restored the default. Server/client left running. Full regression1594 (0 failures,2 skips) preceded the final small framing refinement; final focused console29 and script21 checks passed.


## 25 September follow-up checkpoint (staged, not deployed)
- Native NPC/player rotation hooks now belong to clipping pane1448:8 and model slot0. Previous overlay1448:6 sat behind the new preview pane. Accepted zoom/framing retained. AUTOMATED VERIFIED; Vulkan drag restoration LIVE TEST PENDING.
- Items/NPC pagination now says Previous/Next. Combat Loadouts is a dedicated page; Quick Controls retains toggles; Abilities renamed Combat Commands.
- Typed NPC/object/item favourites use the existing bounded per-account preferences sidecar; catalogue pages have Favourites filters. Selection keeps its epoch and native scroll state. Current favourite controls use a coloured asterisk, NOT the requested final native gold star.
- Verified bank/skill sprites replace Heal (Constitution heart13199), Objects (Construction13209), Items (scroll8123), Bosses (skull30936), Combat (sword13193). Native CS7918 proves active sprite setter834; source icons visually checked against library-native-tab-icons-full.png. Other requested icon replacements remain pending.
- REMAINING: actual gold-star controls across collections, cross-family boss favourites outside curated boss records (including Vorago), Quest/Passive/Aggressive/Slayer category expansion, additional NPC presentation coverage, object model previews, remaining Home icons, physical Vulkan drag/icon acceptance. These are not complete. Exact object-ID-to-native-preview contract remains unestablished by existing capability research.
- No server restart, deployment, client manipulation or character-save edits in this checkpoint.
