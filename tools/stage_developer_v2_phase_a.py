"""Package the already-built Phase-A candidate. Never install or restart.
...
NAME='developer-console-v2-phase-a-20260925'
...
def main():
    ...
    assert m['candidate']=='developer-console-ux-20260925','Only the audited predecessor is supported'
    old=ROOT/'dist'/m['candidate'];dest=ROOT/'dist'/NAME
    assert (dest/'cache-v4/12/21143.dat').is_file(),'Build the complete script delta first'
    for e in m['files']:
    ...
    m.update(candidate=NAME,sourceCommit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT).decode().strip(),
             validationReport='protocol-analysis/developer-console-v2-phase-a-950.json',
             liveChecklist='docs/DEVELOPER-CONSOLE-V2-PHASE-A-20260925.md',
             liveAcceptance='Phase A primitives only. AUTOMATED VERIFIED; physical Vulkan acceptance pending before final composition.')
    ...
    for gate in ('Start-950Server.ps1','Test-Bundle.ps1','Prepare-ClientCache.ps1'):
        ...
        edit(gate,patch)
    def installer(s):
        s=s.replace(",'developer-console-ux-20260925')",",'developer-console-ux-20260925','"+NAME+"')")
        at='if ($manifest.candidate -notin '
        s=s.replace(at,"if ($manifest.candidate -eq '"+NAME+"') { $expectedTargets += @(21131..21143 | ForEach-Object { \"cache/12/$_.dat\" }) }\n"+at,1)
        return s.replace('$planned = @()',"if ($manifest.candidate -eq '"+NAME+"') { $newTargets = @(21131..21143 | ForEach-Object { \"cache/12/$_.dat\" }) }\n$planned = @()",1)
    edit('Apply-PlayabilityUpdate.ps1',installer)
    def launcher(s):
        s=s.replace('Developer Console UX update.','Developer Console V2 Phase A test candidate.')
        s=s.replace('Native title and right-click actions; inline results; NPC and object browsers.','Native NPC scrolling, full model previews, drag rotation and integrated search.')
        s=s.replace('Includes named placement controls, owned spawn cleanup and cached searches.','Primitive acceptance checkpoint: final V2 page composition follows the live test.')
        return s.replace('docs/DEVELOPER-CONSOLE-UX-20260925.md',m['liveChecklist'])
