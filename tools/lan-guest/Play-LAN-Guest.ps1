...
$address=(Get-Content -LiteralPath (Join-Path $root 'LAN-HOST.txt') -Raw).Trim()
...
try { $config=(Invoke-WebRequest -Uri $uri -UseBasicParsing -TimeoutSec 10).Content }
catch { throw "Cannot reach SHNORKSCAPE at ${address}:8950. Ask Jack to start Play.cmd, check you are on the same home network (not guest Wi-Fi), and check LAN-HOST.txt if his address changed. $($_.Exception.Message)" }
