...
public final class Native950DeveloperReferenceCheck {
    ...
    public static void main(String[] args)throws Exception{
        ...
        require(after.getValidArchiveIds().length==before.getValidArchiveIds().length+1);
        require(after.getValidArchiveIds()[after.getValidArchiveIds().length-1]==21130);
        Set<Integer> changed=new HashSet<>(Arrays.asList(8286,21124,21125,21126,21127,21128,21129,21130));
        ...
        for(int id:changed){
            ...
            require(ref.getCompressed()==packed.length-2&&ref.getUncompressed()==archive.getData().length&&ref.getRevision()==(id==21130?1:before.getArchives()[id].getRevision()+1)&&Arrays.equals(new int[]{0},ref.getValidFileIds()));
            require(((packed[packed.length-2]&255)<<8|(packed[packed.length-1]&255))==(ref.getRevision()&65535));
        }
        System.out.println("PASS: unrelated references preserved; seven replacement payloads and new marker helper accepted by production JS5 reader.");
    }
}
