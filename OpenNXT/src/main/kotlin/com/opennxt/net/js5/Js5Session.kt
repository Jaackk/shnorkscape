...
class Js5Session(val channel: Channel) : AutoCloseable {
    ...
    internal fun formatArchivePayload(index: Int, raw: ByteBuffer): ByteArray {
        return if (index == 255) {
            ...
            val data = Js5ContainerPayload.archiveBody(raw)
    ...
    private fun writeResponseBlock(response: PendingResponse): Int {
        // A non-final wire block must contain exactly 102395 payload bytes. The client
        // derives that boundary from the container length; a scheduler budget cannot
        // shorten the block without making the next header appear inside its payload.
        val payloadBytes = minOf(MAX_RESPONSE_BLOCK_PAYLOAD_BYTES, response.remaining)
        if (payloadBytes <= 0) {
            return 0
        }
    ...
    fun process(limit: Int): Int {
        ...
        try {
            // Treat the per-pass limit as a soft budget: finish the current protocol
            // block before yielding, even if that exceeds the remaining allowance.
            while (bytesSent < limit) {
                val response = nextPendingResponse(priority = true) ?: break
                val sent = writeResponseBlock(response)
                bytesSent += sent
