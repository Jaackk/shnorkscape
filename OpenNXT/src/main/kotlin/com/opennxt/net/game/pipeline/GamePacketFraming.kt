...
class GamePacketFraming : ByteToMessageDecoder() {
    ...
    private fun shouldUseLobbyCompatVarByteFallback(
        ...
        if (OpenNXT.config.build == 950 || side != Side.CLIENT || bootstrapStage(channel) != "social-state") {
            return false
        }
    ...
    private fun lateWorldCompatClientSizeOverride(channel: Channel, opcode: Int): Int? {
        if (OpenNXT.config.build == 950 || side != Side.CLIENT || bootstrapStage(channel) != "late-default-varps") {
            return null
        }
