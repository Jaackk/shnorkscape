...
object Js5MsEndpoint {
    ...
    internal fun resolvePayload(index: Int, archive: Int): ResolvedPayload? {
        ...
        return ResolvedPayload(archiveHttpBody(data), "archive")
    }

    /** HTTP uses the same archive CRC boundary as JS5, excluding the on-disk version trailer. */
    internal fun archiveHttpBody(raw: ByteBuffer): ByteBuffer {
        return Js5ContainerPayload.archiveBody(raw)
    }

    internal fun resolveRetailFallbackPayload(
        request: FullHttpRequest,
        index: Int,
        archive: Int,
        offline: Boolean = OpenNXT.offlineModeEnabled(),
    ): ResolvedPayload? {
