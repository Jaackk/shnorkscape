...
internal object RetailUpstreamCookie {
    ...
    fun resolveJavConfigCookie(sourceUrl: String?, type: BinaryType,
                              offline: Boolean = OpenNXT.offlineModeEnabled()): String {
        if (offline) return RetailSessionCookie.current()
    ...
    fun resolveMsCookie(offline: Boolean = OpenNXT.offlineModeEnabled()): String {
        if (offline) return RetailSessionCookie.current()
        if (RetailSessionCookie.isPinned()) {
            return RetailSessionCookie.current()
        }
