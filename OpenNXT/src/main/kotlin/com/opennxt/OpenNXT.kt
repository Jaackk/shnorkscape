...
object OpenNXT : CliktCommand(name = "run-server") {
    ...
    internal fun offlineModeEnabled(envValue: String? = System.getenv("OPENNXT_OFFLINE")): Boolean =
        envValue?.trim()?.lowercase() in setOf("1", "true")

    internal fun retailRawChecksumPassthroughEnabled(
        build: Int,
        envValue: String? = System.getenv(ENABLE_RETAIL_RAW_CHECKSUM_PASSTHROUGH_ENV),
        offline: Boolean = offlineModeEnabled()
    ): Boolean {
        if (offline || build != RETAIL_RAW_CHECKSUM_PASSTHROUGH_BUILD) {
            return false
        }
    ...
    internal fun retailLoggedOutJs5ProxyEnabled(
        ...
        if (!retailLoggedOutJs5PassthroughEnabled(build, passthroughEnvValue, offline)) {
            return false
        }
    ...
    override fun run() {
        ...
        try {
            ...
            val generatedHttpChecksumTable = Container.wrap(
                ChecksumTable.create(
                    filesystem,
                    true,
                    httpIndices = if (config.build >= 947) setOf(40, 56) else setOf(40)
                )
                    .encode(rsaConfig.js5.modulus, rsaConfig.js5.exponent)
            ).array()
            ...
            logger.info { "Binding game server to ${config.bindAddress}:${config.ports.gameBackend}" }
            startupProbe("before-game-bind")
            val result = bootstrap.bind(config.bindAddress, config.ports.gameBackend).sync()
            networkChannel = result.channel()
            if (!result.isSuccess) {
                throw IllegalStateException("Failed to bind to ${config.bindAddress}:${config.ports.gameBackend}", result.cause())
            }
