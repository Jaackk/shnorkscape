...
class WorldPlayer(
    ...
    private var native947WalkTarget: TileLocation? = null
    private var native950WalkTarget: TileLocation? = null

    /**
     * Walking puts new decode logic on the receive path of the one thing that already works on
     * 950, so it stays opt-in until it has been exercised live. -Dopennxt.950.walk=true enables it.
     */
    private val walkEnabled950: Boolean =
        System.getProperty("opennxt.950.walk", "false").toBoolean()

    /**
     * The world-root bootstrap stage is on the critical world-entry path. -Dopennxt.950.ribbon=true
     * mounts the ribbon and the 947 interface policy on top of the verified minimal scene.
     */
    private val ribbonEnabled950: Boolean =
        System.getProperty("opennxt.950.ribbon", "false").toBoolean()

    /**
     * The native Settings window. Independent of the ribbon: the Escape binding lives on the root
     * component 1477:8, so this works with or without -Dopennxt.950.ribbon.
     */
    private val settingsEnabled950: Boolean =
        System.getProperty("opennxt.950.settings", "false").toBoolean()

    /**
     * Scene re-centring, so the player can walk beyond the login scene. Off by default because it
     * changes the scene geometry the whole world bootstrap depends on.
     */
    private val regionsEnabled950: Boolean =
        System.getProperty("opennxt.950.regions", "false").toBoolean()

    /**
     * Server-side collision. Off by default: a wrong clip map is worse than none, because it traps
     * the player rather than merely letting them walk through walls.
     */
    private val collisionEnabled950: Boolean =
        System.getProperty("opennxt.950.collision", "false").toBoolean()

    /** 0 when closed, otherwise one of the SETTINGS_* page constants. */
    ...
    companion object {
        /** 1477:715, the Management Windows slot from struct 21301 param 3505. */
        private const val SETTINGS_HOST = 715
        private const val SETTINGS_GAMEPLAY = 1
        private const val SETTINGS_GRAPHICS = 2
        private const val SETTINGS_CONTROLS = 3
        private const val SETTINGS_AUDIO = 4
        private const val SETTINGS_RIBBON = 5
        private const val SETTINGS_ACCESSIBILITY = 6
    ...
    /**
     * Routes a validated 950 ground (88) or minimap (78) click into the walk target.
     *
     * Never throws: a malformed frame must not break the receive path that world entry depends on,
     * so every length or range mismatch simply declines. The caller still releases the buffer.
     */
    private fun handleNative950WalkRequest(opcode: Int, buffer: io.netty.buffer.ByteBuf) {
        if (!walkEnabled950 || (opcode != 88 && opcode != 78)) return
        val bytes = ByteBufUtil.getBytes(buffer, buffer.readerIndex(), buffer.readableBytes(), false)
        val request = Native950WorldBootstrap.readWalkRequest950(opcode, bytes) ?: return
        val destination = TileLocation(request.x, request.y, entity.location.plane)
        if (!regionsEnabled950 &&
            !Native947WorldBootstrap.isWithinScene(destination, viewport.baseTile, viewport.mapSize)
        ) {
            // Without re-centring the scene cannot follow the player, so a destination outside it
            // is unreachable and the walk would run off the built map.
            client.traceBootstrap(
                "world-native950-walk-rejected name=$name x=${request.x} y=${request.y} reason=outside-scene"
            )
            return
        }
        native950WalkTarget = destination
        client.traceBootstrap(
            "world-native950-walk name=$name x=${request.x} y=${request.y} " +
                "mod=${request.modifier} minimap=${request.minimap}"
        )
    }

    private fun handleNative947Packet(packet: UnidentifiedPacket) {
        val buffer = packet.packet.buf
        try {
            if (packet.packet.opcode != 3 && packet.packet.opcode != 28) return
            val request = Native947WorldBootstrap.readWalkRequest(
                packet.packet.opcode,
                ByteBufUtil.getBytes(buffer, buffer.readerIndex(), buffer.readableBytes(), false)
            ) ?: return
            val destination = TileLocation(request.x, request.y, entity.location.plane)
            if (!Native947WorldBootstrap.isWithinScene(destination, viewport.baseTile, viewport.mapSize)) {
                client.traceBootstrap("world-native947-walk-rejected name=$name x=${request.x} y=${request.y} reason=outside-scene")
                return
            }
            native947WalkTarget = destination
            client.traceBootstrap(
                "world-native947-walk-request name=$name x=${request.x} y=${request.y} modifier=${request.modifier}"
            )
        } finally {
            buffer.release()
        }
    ...
    /**
     * Emits one 950 walk step and advances the server-side location.
     *
     * The client applies the delta to its own position and never re-reads an absolute coordinate
     * during a walk, so [entity] must be moved before the frame is written or the two drift apart
     * permanently. [Native947WorldBootstrap.nextWalkTile] and [Native947WorldBootstrap.isWithinScene]
     * are pure geometry and revision independent, so they are reused rather than duplicated.
     */
    /**
     * Picks the next step toward [destination], preferring the diagonal and sliding along an
     * obstruction when the diagonal is refused.
     *
     * This is deliberately NOT pathfinding. It cannot route around a building: if every candidate
     * is blocked the walk stops. The real answer is a breadth-first router over the clip map -
     * the Ataraxia engine already has one in WalkRouteFinder - but this alone turns "stops dead at
     * every corner" into "follows the wall", which is most of the felt difference in open terrain.
     */
    private fun nextNative950Step(previous: TileLocation, destination: TileLocation): TileLocation? {
        val dx = destination.x.compareTo(previous.x)
        val dy = destination.y.compareTo(previous.y)
        if (dx == 0 && dy == 0) return null
        val candidates = if (dx != 0 && dy != 0) {
            listOf(
                TileLocation(previous.x + dx, previous.y + dy, previous.plane),
                TileLocation(previous.x + dx, previous.y, previous.plane),
                TileLocation(previous.x, previous.y + dy, previous.plane)
            )
        } else {
            listOf(TileLocation(previous.x + dx, previous.y + dy, previous.plane))
        }
        for (candidate in candidates) {
            if (!regionsEnabled950 &&
                !Native947WorldBootstrap.isWithinScene(candidate, viewport.baseTile, viewport.mapSize)
            ) continue
            if (collisionEnabled950 && !Collision950.checkWalkStep(previous, candidate)) continue
            return candidate
        }
        return null
    }

    private fun walkNative950Step(): Boolean {
        val destination = native950WalkTarget ?: return false
        val previous = entity.location
        val next = nextNative950Step(previous, destination)
        if (next == null) {
            native950WalkTarget = null
            client.traceBootstrap(
                "world-native950-walk-blocked name=$name at=${previous.x},${previous.y} " +
                    "target=${destination.x},${destination.y} reason=no-legal-step"
            )
            return false
        }
        val payload = Native950WorldBootstrap.walkStep(next.x - previous.x, next.y - previous.y)
        entity.previousLocation = previous
        entity.location = next
        client.write(OpcodeWithBuffer(36, Unpooled.wrappedBuffer(payload)))
        if (collisionEnabled950) Collision950.ensureLoadedAround(next)
        maintainNative950Scene(next)
        if (next == destination) native950WalkTarget = null
        return true
    }

    /**
     * Re-centres the scene on [tile] with a header-only REBUILD_NORMAL.
     *
     * Never re-runs Viewport.init: it unconditionally writes the ~5.1 KB GPI bitstream, and the
     * client only consumes that while the gate byte at [[client]+0x198b8 + 0x49] is set. That byte
     * is cleared at the tail of the first rebuild (0x1400f7189) and only re-armed on a fresh world
     * entry, so a second rebuild carrying the bitstream would have its header read ~5 KB too far in.
     *
     * areaType must stay byte-identical: changing it routes through 0x1400f7067 and destroys every
     * entry in the client's player and npc containers. npcBits must be resent every time; it is
     * rewritten on each rebuild.
     */
    private fun recentreNative950(tile: TileLocation) {
        val bootstrap = OpenNXT.config.lobbyBootstrap
        val header = Native950WorldBootstrap.rebuildHeader950(
            chunkX = tile.chunkX,
            chunkY = tile.chunkY,
            npcBits = viewport.sceneRadius,
            areaType = bootstrap.rebuildNormalAreaType,
            hash1 = bootstrap.rebuildNormalHash1,
            hash2 = bootstrap.rebuildNormalHash2
        )
        client.write(OpcodeWithBuffer(63, Unpooled.wrappedBuffer(header)))
        viewport.moveToRegion(tile, MapSize.SIZE_256, sendUpdate = false)
        client.traceBootstrap(
            "world-native950-recentre name=$name chunkX=${tile.chunkX} chunkY=${tile.chunkY}"
        )
    }

    /**
     * Re-centres once the player nears the edge of the built scene.
     *
     * The client's hard constraint is 0 <= tile - base <= 255. The margin below is server policy:
     * it re-centres back to roughly local 128 with ~40 tiles of headroom, well clear of the 254
     * limit that applies to anything the server has to name by a single local byte.
     */
    private fun maintainNative950Scene(next: TileLocation) {
        if (!regionsEnabled950) return
        val localX = next.getLocalX(viewport.baseTile, viewport.mapSize)
        val localY = next.getLocalY(viewport.baseTile, viewport.mapSize)
        if (localX !in 40..215 || localY !in 40..215) recentreNative950(next)
    }

    private fun walkNative947Step(): Boolean {
        val destination = native947WalkTarget ?: return false
        val previous = entity.location
        val next = Native947WorldBootstrap.nextWalkTile(previous, destination)
        if (next == null || !Native947WorldBootstrap.isWithinScene(next, viewport.baseTile, viewport.mapSize)) {
            native947WalkTarget = null
            return false
        }
        // This is bounded straight-line movement; the server has no collision route finder yet.
        val payload = Native947WorldBootstrap.walkStep(next.x - previous.x, next.y - previous.y)
        entity.previousLocation = previous
        entity.location = next
        client.write(OpcodeWithBuffer(27, Unpooled.wrappedBuffer(payload)))
        if (next == destination) native947WalkTarget = null
        return true
    }
    ...
    fun handleIncomingPackets() {
        ...
        while (true) {
            ...
            if (OpenNXT.config.build == 950) {
                // Only validated 950 requests are enabled during the isolated world-login test.
                when (packet) {
                    is WorldlistFetch -> worldList.handleRequest(packet.checksum, client)
                    is NoTimeout -> NoTimeoutHandler.handle(this, packet)
                    is UnidentifiedPacket -> {
                        val buffer = packet.packet.buf
                        try {
                            client.traceBootstrap("world-native950-client opcode=${packet.packet.opcode} bytes=${buffer.readableBytes()} preview=${ByteBufUtil.hexDump(buffer, buffer.readerIndex(), minOf(buffer.readableBytes(), 32))}")
                            handleNative950WalkRequest(packet.packet.opcode, buffer)
                            handleNative950InterfaceButton(packet.packet.opcode, buffer)
                        } finally { buffer.release() }
                    }
                    else -> logger.debug { "Ignoring unverified native 950 world packet ${packet::class.simpleName}" }
                }
                continue
            }

            if (OpenNXT.config.build == 947) {
                // The inherited compatibility opcodes are from a different native protocol.
                // Keep verified requests working without triggering their speculative bootstrap replies.
                when (packet) {
                    is WorldlistFetch -> worldList.handleRequest(packet.checksum, client)
                    is NoTimeout -> NoTimeoutHandler.handle(this, packet)
                    is UnidentifiedPacket -> handleNative947Packet(packet)
                    else -> logger.debug { "Ignoring unverified native 947 world packet ${packet::class.simpleName}" }
                }
                continue
            }
    ...
    private fun openNative947Interface(slot: InterfaceSlot, interfaceId: Int) {
        // These slot positions moved in the native cache; inherited component numbers are stale.
        val slots = requireNotNull(OpenNXT.resources.get<EnumDefinition>(7716))
        val structId = slots.values[slot.id] as Int
        val struct = requireNotNull(OpenNXT.resources.get<StructDefinition>(structId))
        val hash = struct.getInt(3505, default = -1)
        check(hash >= 0) { "Missing native interface slot ${slot.name}" }
        interfaces.open(id = interfaceId, parent = hash ushr 16, component = hash and 65535, walkable = true)
        client.traceBootstrap(
            "world-native947-interface name=$name slot=${slot.name} id=$interfaceId parent=${hash ushr 16} component=${hash and 65535}"
        )
    }

    /**
     * Decodes an interface click and routes the Settings entry points.
     *
     * Client opcodes 18, 122, 89, 100, 81, 126, 49, 66, 31, 59 are options 1..10, all fixed size 9
     * (pointer table 0x140b63c00). Layout, confirmed against five live captures:
     ...
     * Live examples: ffffffc5050800ffff is 1477:8 slot -1 (Escape/Options),
     * ffffff970500000007 is 1431:0 slot 7 (the Settings gear), ffffffb9050f00ffff is 1465:15.
     *
     * Only option 1 is routed; every other option and any unrecognised component is ignored.
     */
    private fun handleNative950InterfaceButton(opcode: Int, buffer: io.netty.buffer.ByteBuf) {
        if (!settingsEnabled950 || opcode != Native950WorldBootstrap.CLIENT_IF_BUTTON1) return
        val bytes = ByteBufUtil.getBytes(buffer, buffer.readerIndex(), buffer.readableBytes(), false)
        val button = Native950WorldBootstrap.readInterfaceButton(bytes) ?: return
        if (button.item != -1) return
        val interfaceId = button.interfaceId
        val component = button.component
        val slot = button.slot
        client.traceBootstrap(
            "world-native950-button name=$name interface=$interfaceId component=$component slot=$slot"
        )

        val escape = interfaceId == 1477 && component == 8 && slot == -1
        val gear = interfaceId == 1431 && component == 0 && slot == 7
        if (escape || gear) {
            when {
                native950SettingsPage == 0 -> openNative950Settings()
                // The gear's own hook can close the frame before notifying us, so it always
                // refocuses Graphics rather than toggling.
                gear -> {
                    client.write(RunClientScript(8179))
                    client.write(RunClientScript(8180, arrayOf(1, 1)))
                    interfaces.events(1477, 8, -1, -1, 252)
                    renderNative950Settings(SETTINGS_GRAPHICS)
                }
                else -> closeNative950Settings()
            }
            client.flush()
            return
        }
        if (native950SettingsPage == 0 || interfaceId != 1477) return
        if (component == 717 && slot == 1) {
            closeNative950Settings()
            client.flush()
            return
        }
        if (component != 714) return
        val page = native950SettingsPageForActor(slot)
        if (page != 0) {
            renderNative950Settings(page)
            client.flush()
        }
    }

    /**
     * Opens the native management window and focuses Graphics.
     *
     * Ported from the 947 project's Native947Settings.open(). Host 1477:715 is the "Management
     * Windows" slot (struct 21301 param 3505); every id used here is byte-identical between the
     * 947 and 950 caches.
     */
    private fun openNative950Settings() {
        // Gear and Escape both run the native quick-options hook before their server action.
        // Retire that overlay, then restore the base Options key its cleanup can disable.
        client.write(RunClientScript(8179))
        client.write(RunClientScript(8180, arrayOf(1, 1)))
        // While management is visible, root 8's Escape hook emits close actor 717 itself. Suppress
        // only the second generic Options notification: cache mask 254 minus bit 1.
        interfaces.events(1477, 8, -1, -1, 252)
        interfaces.open(id = 1448, parent = 1477, component = SETTINGS_HOST, walkable = true)
        for (bit in intArrayOf(19029, 19031, 19032, 19033, 47565, 60056, 19004))
            sendNative950Varbit(bit, 0)
        sendNative950Varbit(18994, 9)
        sendNative950Varc(2911, 9)
        native950SettingsPage = 0
        renderNative950Settings(SETTINGS_GRAPHICS)
        client.traceBootstrap("world-native950-settings-open name=$name host=1477:$SETTINGS_HOST")
    }

    private fun renderNative950Settings(next: Int) {
        val replace = native950SettingsPage != next
        if (replace) removeNative950SettingsPage()
        native950SettingsPage = next
        sendNative950Varbit(19001, next)
        client.write(RunClientScript(8288, arrayOf(9)))
        client.write(RunClientScript(8193))
        // 365's Accessibility selector reads the visible management wrapper as well as page 19001,
        // so both must be available before its onLoad runs.
        interfaces.hide(1477, 708, false)
        if (replace && next == SETTINGS_ACCESSIBILITY) client.write(RunClientScript(20387))
        if (replace) {
            when (next) {
                SETTINGS_GRAPHICS -> {
                    interfaces.open(id = 1426, parent = 1448, component = 3, walkable = true)
                    // 742's onLoad 2594/2595 selects and opens its native renderer page.
                    interfaces.open(id = 742, parent = 1426, component = 0, walkable = true)
                }
                SETTINGS_AUDIO -> {
                    interfaces.open(id = 1426, parent = 1448, component = 3, walkable = true)
                    interfaces.open(id = 429, parent = 1448, component = 5, walkable = true)
                }
                else -> interfaces.open(
                    id = native950SettingsPageInterface(next), parent = 1448, component = 3, walkable = true
                )
            }
        }
        client.write(RunClientScript(8283, arrayOf(native950SettingsPageStruct(next), 0)))
        if (next == SETTINGS_AUDIO) client.write(RunClientScript(8283, arrayOf(21183, 1)))
        interfaces.hide(1448, 3, next == SETTINGS_AUDIO)
        if (next == SETTINGS_AUDIO) interfaces.hide(1448, 5, false)
        interfaces.hide(1448, 1, true)
        interfaces.hide(1477, 708, false)
        for (tab in SETTINGS_GAMEPLAY..SETTINGS_ACCESSIBILITY)
            interfaces.events(1477, 714, tab * 4 - 1, tab * 4 - 1, 2)
        interfaces.events(1477, 717, 1, 1, 2)
        client.traceBootstrap("world-native950-settings-page name=$name page=$next replaced=$replace")
    }

    private fun removeNative950SettingsPage() {
        when (native950SettingsPage) {
            0 -> return
            SETTINGS_GRAPHICS -> {
                interfaces.close(1426, 0)
                interfaces.close(1448, 3)
            }
            SETTINGS_AUDIO -> {
                interfaces.close(1448, 5)
                interfaces.close(1448, 3)
            }
            else -> interfaces.close(1448, 3)
        }
    }

    private fun closeNative950Settings() {
        if (native950SettingsPage == 0) return
        client.write(RunClientScript(8179))
        client.write(RunClientScript(8180, arrayOf(1, 1)))
        // The native close script reads the current client page for its cleanup.
        sendNative950Varc(2911, -1)
        client.write(RunClientScript(8290, arrayOf(1)))
        removeNative950SettingsPage()
        native950SettingsPage = 0
        interfaces.close(1477, SETTINGS_HOST)
        interfaces.hide(1477, 708, true)
        interfaces.events(1477, 8, -1, -1, 254)
        client.traceBootstrap("world-native950-settings-close name=$name")
    }

    private fun native950SettingsPageForActor(actor: Int): Int =
        if (actor < 3 || actor > 23 || (actor + 1) % 4 != 0) 0 else (actor + 1) / 4

    private fun native950SettingsPageInterface(page: Int): Int = when (page) {
        SETTINGS_GAMEPLAY, SETTINGS_ACCESSIBILITY -> 365
        SETTINGS_CONTROLS -> 1444
        SETTINGS_RIBBON -> 567
        else -> throw IllegalArgumentException("Not a single-interface settings page: $page")
    }

    private fun native950SettingsPageStruct(page: Int): Int = when (page) {
        SETTINGS_GAMEPLAY -> 21179
        SETTINGS_GRAPHICS -> 21182
        SETTINGS_CONTROLS -> 21181
        SETTINGS_AUDIO -> 21183
        SETTINGS_RIBBON -> 44487
        SETTINGS_ACCESSIBILITY -> 52418
        else -> throw IllegalArgumentException("Unknown settings page: $page")
    }

    /**
     * CLIENT_SETVARC_LARGE, opcode 119, fixed 6 bytes.
     *
     * Parser 0x140141DB0: the id is a big-endian u16 whose low byte carries +0x80
     * (lea eax,[r9-0x80] at 0x140141e1c), and the value is reassembled as
     * (b3 shl 24) or (b2 shl 16) or (b5 shl 8) or b4 (0x140141de5-0x140141e0f).
     * Registration table entry base + 119*80 + 0x10 = 0x140E97430, base 0x140E94EF0.
     */
    private fun sendNative950Varc(id: Int, value: Int) {
        client.write(
            OpcodeWithBuffer(119, Unpooled.wrappedBuffer(Native950WorldBootstrap.encodeVarc(id, value)))
        )
    }

    /**
     * VARBIT_SMALL, opcode 28, fixed 3 bytes.
     *
     ...
     *
     * Sent raw rather than through a packet class because there is no VarbitSmall class in
     * serverprot/variables, and adding one would mean adding a file to the fixed source list in
     * Build-950Lobby.ps1.
     *
     * Two silent-failure gates apply to every var write: the definition provider at
     * [owner+0x18D68] must report kind 4 (checked at 0x140142226), and a varbit's parent varp must
     * carry integer type tag 0 (0x1402EB3F2), which is why the parent must never be written with a
     * long-form varp packet first.
     */
    private fun sendNative950Varbit(id: Int, value: Int) {
        client.write(
            OpcodeWithBuffer(28, Unpooled.wrappedBuffer(Native950WorldBootstrap.encodeVarbit(id, value)))
        )
    }

    /**
     * The 947 interface policy, re-expressed for 950.
     *
     * Ported from Native947InterfaceBootstrap/Native947Ribbon in the 947 project. Every root slot
     * binding it relies on (enum 7716 -> struct -> param 3505/3503) is byte-identical between the
     * two caches, and all seven varbit definitions below are byte-identical too, so the 947 values
     * carry over unchanged. The one binding that did move is the run orb: 1465:14 on 947 is
     * 1465:15 on 950, because the 950 cache inserted "View Leagues" at 1465:9.
     */
    private fun native950InterfacePolicy() {
        // Modern combat, modern interfaces, modern skin, and a non-tutorial onboarding state.
        // Emitted before the panels whose onLoad hooks read these mode bits.
        sendNative950Varbit(27168, 0)
        sendNative950Varbit(27169, 0)
        sendNative950Varbit(22875, 0)
        sendNative950Varbit(39917, 98)
        sendNative950Varbit(49044, 100)
        sendNative950Varbit(60098, 1)
        sendNative950Varbit(18797, 1)

        openNative947Interface(InterfaceSlot.BUTTONS, 1431)
        openNative947Interface(InterfaceSlot.BACKPACK, 1473)
        openNative947Interface(InterfaceSlot.WORN_EQUIPMENT, 1462)
        openNative947Interface(InterfaceSlot.ALL_CHAT, 137)

        // Mask bit (n+1) enables component option n; the gate is `lea ecx,[opIndex+1]; shr edx,cl;
        // test dl,1` at 0x14016a752. With the bit clear the client sends no packet at all.
        interfaces.events(1473, 5, 0, 27, 2360382)
        interfaces.events(1465, 15, -1, -1, 2)
        // 1431:0 hosts the ribbon's dynamic children; option 1 on the gear actor.
        interfaces.events(1431, 0, 7, 7, 2)

        client.traceBootstrap("world-native950-interface-policy name=$name ribbon=1431 orb=1465:15")
    }

    private fun hideUnusedNative947Panels() {
        val slots = requireNotNull(OpenNXT.resources.get<EnumDefinition>(7716))
        // Anything not retained is hidden below, so this must stay in lockstep with the panels
        // opened in bootstrapNativeWorld or the ribbon would be opened and immediately hidden.
        val retainedSlots = if (ribbonEnabled950 && OpenNXT.config.build == 950)
            setOf(
                InterfaceSlot.GAME_VIEW, InterfaceSlot.MINIMAP, InterfaceSlot.BUTTONS,
                InterfaceSlot.BACKPACK, InterfaceSlot.WORN_EQUIPMENT, InterfaceSlot.ALL_CHAT
            )
        else setOf(InterfaceSlot.GAME_VIEW, InterfaceSlot.MINIMAP)
        val wrappers = InterfaceSlot.VALUES.mapNotNull { slot ->
            val structId = slots.values[slot.id] as? Int ?: return@mapNotNull null
            val struct = OpenNXT.resources.get<StructDefinition>(structId) ?: return@mapNotNull null
            slot to struct.getInt(3503, default = -1)
        }
        // Keep both supported panels and their shared ancestors; only dedicated slot wrappers are hidden.
        val retainedComponents = setOf(0, 27, 60) + wrappers
            .filter { (slot, hash) -> slot in retainedSlots && hash ushr 16 == 1477 }
            .map { (_, hash) -> hash and 65535 }
        val unusedComponents = wrappers
            .filter { (slot, hash) -> slot !in retainedSlots && hash ushr 16 == 1477 }
            .map { (_, hash) -> hash and 65535 }
            .filterNot { it in retainedComponents }
            .distinct()
        unusedComponents.forEach { component -> interfaces.hide(id = 1477, component = component, hidden = true) }
        client.traceBootstrap("world-native947-hide-unused-panels name=$name components=${unusedComponents.joinToString(",")}")
    }

    private fun bootstrapNativeWorld() {
        val build = OpenNXT.config.build
        check(build == 947 || build == 950)
        val rebuildOpcode = if (build == 950) 63 else 90
        val playerInfoOpcode = if (build == 950) 36 else 27
        native947WalkTarget = null
        runBootstrapStage("native$build-rebuild") {
            val registration = requireNotNull(PacketRegistry.getRegistration(Side.SERVER, RebuildNormal::class))
            check(registration.opcode == rebuildOpcode) { "Native $build requires REBUILD_NORMAL opcode $rebuildOpcode" }
            val payload = Unpooled.buffer(5140)
            val builder = GamePacketBuilder(payload)
            viewport.init(builder)
            viewport.slotFlags.fill(0)
            if (build == 950 && regionsEnabled950) {
                // The client's scene window is 256 tiles with base = (chunk - 16) * 8
                // ([scene+0x6a0] = 0x100 at 0x1401177c9, applied at 0x1400f6fc5). SIZE_104 makes
                // getLocalX/getLocalY disagree with the client by 80 tiles on each axis, which is
                // what confines walking today. moveToRegion assigns mapSize and baseTile before it
                // checks sendUpdate, so this is a pure state update and sends nothing.
                viewport.moveToRegion(entity.location, MapSize.SIZE_256, sendUpdate = false)
            }
            // Both native receive handlers require wire value 5 for a normal scene rebuild.
            val rebuild = viewport.createPacket().copy(mapSize = 5)
            @Suppress("UNCHECKED_CAST")
            (registration.codec as GamePacketCodec<GamePacket>).encode(rebuild, builder)
            client.traceBootstrap(
                "world-native$build-rebuild name=$name playerIndex=${entity.index} " +
                    "chunkX=${rebuild.chunkX} chunkY=${rebuild.chunkY} mapSize=${rebuild.mapSize} " +
                    "areaType=${rebuild.areaType} hash1=${rebuild.hash1} hash2=${rebuild.hash2}"
            )
            client.write(OpcodeWithBuffer(registration.opcode, payload))
        }
        runBootstrapStage("native$build-appearance") {
            val payload = if (build == 950) Native950WorldBootstrap.initialAppearance(entity.model.data)
                else Native947WorldBootstrap.initialAppearance(entity.model.data)
            client.write(OpcodeWithBuffer(playerInfoOpcode, Unpooled.wrappedBuffer(payload)))
            initialWorldSyncSent = true
            worldSyncFramesSent = 1
        }
        if (build == 950 && collisionEnabled950) {
            Collision950.ensureLoadedAround(entity.location)
            client.traceBootstrap("world-native950-collision-loaded name=$name around=${entity.location.x},${entity.location.y}")
        }
        runBootstrapStage("native$build-world-root") {
            interfaces.openTop(1477)
            openNative947Interface(InterfaceSlot.GAME_VIEW, 1482)
            openNative947Interface(InterfaceSlot.MINIMAP, 1465)
            if (build == 950 && ribbonEnabled950) native950InterfacePolicy()
            hideUnusedNative947Panels()
        }
        awaitingMapBuildComplete = false
        awaitingWorldReadySignal = false
        deferredBootstrap = null
    ...
    fun added() {
        ...
        if (OpenNXT.config.build == 947 || OpenNXT.config.build == 950) {
            stage = "native${OpenNXT.config.build}-world"
            bootstrapNativeWorld()
            return
        }
...
// ClientSetvarcSmall(id=2834, value=1)
    ...
    override fun tick() {
        if (OpenNXT.config.build == 950) {
            // Exactly one PLAYER_INFO per tick, as before: a step frame when walking, else idle.
            if (!walkNative950Step()) {
                client.write(OpcodeWithBuffer(36, Unpooled.wrappedBuffer(Native950WorldBootstrap.idle())))
            }
        ...
        if (OpenNXT.config.build == 947) {
            if (!walkNative947Step()) {
                client.write(OpcodeWithBuffer(27, Unpooled.wrappedBuffer(Native947WorldBootstrap.idle())))
            }
            // Native tick-end 195 releases pending scene/player work; 216 keeps the connection alive.
            client.write(ServerTickEnd)
            client.write(NoTimeout)
            return
        }
