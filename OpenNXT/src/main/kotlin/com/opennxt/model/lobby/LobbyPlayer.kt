...
class LobbyPlayer(client: ConnectedClient, name: String) : BasePlayer(client, name) {
    ...
    val worldList = if (OpenNXT.config.build == 950) WorldList(arrayOf(
        WorldListEntry(id = 1, location = WorldListLocation(161, "Local"),
            flag = WorldFlag.createFlag(), activity = "All content", host = worldHost(), playercount = 0)
    )) else WorldList(
        arrayOf(
            WorldListEntry(
                id = 1,
        ...
        get() {
            if (OpenNXT.config.build == 950) field.entries[0].playercount = com.rs.game.World.getPlayers().size
            return field
        }
    ...
    private fun handleUnidentifiedPacket(packet: UnidentifiedPacket): Boolean {
        ...
        if (OpenNXT.config.build == 947 || OpenNXT.config.build == 950) {
            logger.info { "Native lobby input opcode=$opcode bytes=${buf.readableBytes()}" }
            buf.release()
            return true
        }
    ...
    fun added() {
        ...
        if (OpenNXT.config.build == 950) {
            Native950LobbyLayout.verifyCache()
            interfaces.openTop(id = 906)
            // Enum941 tab1 is906:45.906:37 is a tab underlay, not the old814 summary host.
            interfaces.open(id = 910, parent = 906, component = 45, walkable = true)
            for (packet in Native950LobbyLayout.initialization(name))
                client.write(OpcodeWithBuffer(packet.type().opcode(), Unpooled.wrappedBuffer(packet.payload())))
            logger.info { "Sent clean950 lobby for $name: native World tab and local World1" }
            return
        }
        if (OpenNXT.config.build == 947) {
            interfaces.openTop(id = 906)
            // Older supplemental component IDs overlap the native 947 lobby controls.
            interfaces.open(id = 814, parent = 906, component = 37, walkable = true)
            client.write(RunClientScript(script = 10936, args = emptyArray()))
            logger.info { "Sent native lobby interface bootstrap for $name" }
            return
        }
...
/** Small native950 lobby presentation policy; world entry and authentication remain client-owned. */
internal object Native950LobbyLayout {
    private var verifiedFilesystem: com.opennxt.filesystem.Filesystem? = null
    fun initialization(name: String): List<com.rs.network.protocol.modern950.Native950Packets.Packet> {
        val displayName = name.replace("<", "").replace(">", "").take(24)
        return listOf(
            com.rs.network.protocol.modern950.Native950Packets.interfaceText(906,118,displayName),
            com.rs.network.protocol.modern950.Native950Packets.interfaceText(906,119,"Local server - all content enabled"),
            com.rs.network.protocol.modern950.Native950Packets.hideInterface(906,120,true),
            com.rs.network.protocol.modern950.Native950Packets.hideInterface(906,139,true),
            //3059/3060 select the authored World tab, reveal host45 and update selection/footer.
            com.rs.network.protocol.modern950.Native950Packets.runClientScript(3059,1)
        )
    }
    @Synchronized fun verifyCache() {
        val fs=OpenNXT.filesystem
        if(verifiedFilesystem===fs)return
        val pins=listOf(
            Triple(3,906,"ff04c4a3f47b03da3d8f7e8b128cba8e1d2508017286399146b62fa005ba8f06"),
            Triple(3,910,"997e4eb11dbd3949afe3fd24f3abf8e9f48cf362100b151070f4357d41b2dc0f"),
            Triple(12,3059,"d99a945a395266eb40fd5ab898cdc73206457adb543b18202615d7bd36830df1"),
            Triple(12,3060,"279fe594e06a0e337badc90e7768f6637b50dce26a3aac95bf78b48f88b50a0d")
        )
        for((index,group,expected) in pins){
            val raw=com.opennxt.filesystem.Container.decode(requireNotNull(fs.read(index,group))).data
            val actual=java.security.MessageDigest.getInstance("SHA-256").digest(raw).joinToString(""){"%02x".format(it.toInt() and 255)}
            check(actual==expected){"Changed950 lobby layout $index/$group; revalidate before showing it"}
        }
        verifiedFilesystem=fs
    }
}
