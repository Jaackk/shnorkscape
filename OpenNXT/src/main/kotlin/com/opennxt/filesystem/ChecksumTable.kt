...
class ChecksumTable(val entries: Array<TableEntry>) {
    ...
    companion object {
        ...
        fun create(fs: Filesystem, http: Boolean, httpIndices: Set<Int> = setOf(40)): ChecksumTable {
            // Keep the legacy HTTP table shape unless additional indices are requested.
            val entryCount = if (http && httpIndices == setOf(40)) 41 else fs.numIndices()
            val entries = Array(entryCount) { index ->
                if (http && index !in httpIndices) return@Array TableEntry.EMPTY
