...
class SqlJetFilesystem(path: Path) : Filesystem(path), Closeable, AutoCloseable {
    private val logger = KotlinLogging.logger { }

    var indices: Array<SqlJetIndexFile?>
    ...
    init {
        ...
        logger.info { "Opening SQLJet filesystem from $path" }
        var count = 0
        ...
        indices = Array(count) { i ->
            val file = SqlJetIndexFile(path.resolve("js5-$i.jcache"))
    ...
    override fun createIndex(id: Int) {
        ...
        val current = indices
        indices = Array(id + 1) { index ->
            if (index != id) {
                current[index]
            } else {
                SqlJetIndexFile(path.resolve("js5-$index.jcache"))
            }
        }
    }
